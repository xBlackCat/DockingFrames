    ...
    private boolean supports(String translucencyName) {
        try {
            ...
            Method isWindowTranslucencySupported =
                    GraphicsDevice.class.getMethod("isWindowTranslucencySupported", windowTransulcency);
            return (Boolean) isWindowTranslucencySupported.invoke(gd,
                    windowTransulcency.getField(translucencyName).get(null));
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
