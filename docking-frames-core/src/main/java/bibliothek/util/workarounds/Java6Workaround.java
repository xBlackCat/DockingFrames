...
public class Java6Workaround implements Workaround {
    ...
    public void markAsGlassPane(Component component) {
        try {
            Method componentMixingCutoutShape =
                    getAWTUtilitiesClass().getMethod("setComponentMixingCutoutShape", Component.class, Shape.class);
            componentMixingCutoutShape.invoke(null, component, new Rectangle());
        } catch (SecurityException | InvocationTargetException | IllegalAccessException | ClassNotFoundException |
                 NoSuchMethodException | IllegalArgumentException ex) {
            // ignore
        }
    }
    ...
    private boolean supports(String translucency) {
        ...
        try {
            ...
            Method isTranslucencySupported =
                    getAWTUtilitiesClass().getMethod("isTranslucencySupported", translucencyClass);

            return (Boolean) isTranslucencySupported.invoke(null, getTranslucency(translucency));
        } catch (SecurityException | NoSuchMethodError | NoClassDefFoundError ex) {
            // ignore
        } catch (Exception ex) {
    ...
    public boolean setTranslucent(Window window) {
        ...
        try {
            ...
            return true;
        } catch (NoClassDefFoundError | SecurityException | NoSuchMethodError ex) {
            // ignore
        } catch (Exception ex) {
            if (printWarnings && !invocationTargetException) {
                invocationTargetException = true;
