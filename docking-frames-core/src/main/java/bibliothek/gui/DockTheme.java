 ...
 * {@link ThemeManager#add(String, bibliothek.util.Path, bibliothek.gui.dock.util.TypedUIProperties.Type, UIValue)
 * add} must
...
public interface DockTheme {
     ...
     */
    PropertyKey<BackgroundPaint> BACKGROUND_PAINT = new PropertyKey<>("dock.background");

    /**
     * A unique identifier for the {@link DockProperties} to access the current {@link BorderModifier}.
     */
    PropertyKey<BorderModifier> BORDER_MODIFIER = new PropertyKey<>("dock.borderModifier");

    /**
     * The factory creating {@link Span}s and ultimately deciding how the animation looks like when dropping a
     * {@link Dockable}
     */
    PropertyKey<SpanFactory> SPAN_FACTORY =
            new PropertyKey<>("span factory", new ConstantPropertyFactory<>(new DefaultSpanFactory()), true);
     ...
     */
    PropertyKey<Combiner> COMBINER = new PropertyKey<>("dock.combiner",
     ...
     * value will be derived from the current {@link DockTheme}.
     */
    PropertyKey<StationPaint> STATION_PAINT = new PropertyKey<>("dock.paint",
            new DynamicPropertyFactory<>() {
     ...
     * This method should not be used directly, instead an {@link UIValue} of type {@link DefaultStationPaintValue}
     * should
     * be installed at the local {@link ThemeManager} to retrieve the value.
     *
     ...
     * value will be derived from the current {@link DockTheme}.
     */
    PropertyKey<DisplayerFactory> DISPLAYER_FACTORY = new PropertyKey<>("dock.displayerFactory",
            new DynamicPropertyFactory<>() {
     ...
     * This method should not be used directly, instead an {@link UIValue} of type
     * {@link DefaultDisplayerFactoryValue} should
     * be installed at the local {@link ThemeManager} to retrieve the value.
     *
     ...
     */
    PropertyKey<DockableMovingImageFactory> DOCKABLE_MOVING_IMAGE_FACTORY = new PropertyKey<>("dock.movingImageFactory",
            new DynamicPropertyFactory<>() {
                public DockableMovingImageFactory getDefault(PropertyKey<DockableMovingImageFactory> key,
     ...
     */
    PropertyKey<DockableSelection> DOCKABLE_SELECTION = new PropertyKey<>("dock.dockableSelection",
            new DynamicPropertyFactory<>() {
