...
public class DockController {
    /**
     * property telling whether this application runs in a restricted environment or not, the default value is the
     * result of {@link DockUI#isSecureEnvironment()}
     */
    public static final PropertyKey<Boolean> RESTRICTED_ENVIRONMENT =
            new PropertyKey<>("dock.restricted_environment",
                    new ConstantPropertyFactory<>(DockUI.getDefaultDockUI().isSecureEnvironment()), true);
     ...
     */
    private final List<DockableSelectionListener> dockableSelectionListeners = new ArrayList<>();
    /**
     * Listeners observing the bound-state of {@link DockTitle}s
     */
    private final List<DockTitleBindingListener> dockTitleBindingListeners = new ArrayList<>();
     ...
     */
    private final DockTitleObserver dockTitleObserver = new DockTitleObserver();
    /**
     * mapping tells which titles are currently active
     */
    private final Map<DockTitle, Dockable> activeTitles = new HashMap<>();
    /**
     ...
     */
    private final List<ActionOffer> actionOffers = new ArrayList<>();
    /**
     * A  list of sources for {@link DockActionSource DockActionSources}
     */
    private final List<ActionGuard> guards = new ArrayList<>();
    /**
     ...
     */
    private final MultiDockAcceptance acceptance = new MultiDockAcceptance();
     ...
     */
    private final Map<Component, DockElementRepresentative> componentToDockElements =
            new HashMap<>();
    /**
     * a list of listeners listening for changes in {@link #componentToDockElements}
     */
    private final List<DockControllerRepresentativeListener> componentToDockElementsListeners =
            new ArrayList<>();
     ...
     */
    private final DockHierarchyLock lock = new DockHierarchyLock();
    ...
    protected void showCoreWarning() {
        if (showCoreWarning) {
            EventQueue.invokeLater(CoreWarningDialog::showDialog);
        }
    }
    ...
    protected final void initiate(DockControllerFactory factory, ControllerSetupCollection setup) {
        ...
        final List<ControllerSetupListener> setupListeners = new LinkedList<>();
        if (setup == null) {
            setup = listener -> {
                if (listener == null) {
    ...
    protected UIScheme<String, TextValue, TextBridge> createDefaultTextScheme() {
        ResourceBundle bundle =
                ResourceBundle.getBundle("data.bibliothek.gui.dock.core.locale.text", Locale.getDefault(),
    ...
    public void addRepresentative(DockElementRepresentative representative) {
        DockControllerRepresentativeListener[] listeners = componentToDockElementsListeners
                .toArray(new DockControllerRepresentativeListener[0]);
     ...
     *
     *
     *      {@link FocusController#setFocusedDockable(DockElementRepresentative, Component, boolean, boolean, boolean)}.
     * @see #isOnFocusing()
     */
     ...
     */
    @Deprecated
    @Todo(compatibility = Compatibility.BREAK_MAJOR, description = "remove this method", priority =
     ...
     *                              optimizations are allowed. Clients normally can set this argument
     *                              to <code>false</code>.
     * @param ensureFocusSet        if <code>true</code>, then this method should make sure that either
     *                              <code>focusedDockable</code>
     *                              itself or one of its {@link DockElementRepresentative} is the focus owner
     * @param ensureDockableFocused if <code>true</code>, then this method should make sure that
     *                              <code>focusedDockable</code>
    ...
    @Todo(compatibility = Compatibility.BREAK_MAJOR, description = "remove this method", priority =
            ...
            target = Version.VERSION_1_1_3)
    public void setFocusedDockable(Dockable focusedDockable, Component component, boolean force,
                                   boolean ensureFocusSet, boolean ensureDockableFocused) {
        setFocusedDockable(new DefaultFocusRequest(focusedDockable, component, force, ensureFocusSet,
                ensureDockableFocused));
    }
    ...
    public DockActionSource listOffers(Dockable dockable) {
        List<DockActionSource> guards = new ArrayList<>();
        List<DockActionSource> parents = new ArrayList<>();
        ...
        return offer.getSource(dockable, dockable.getLocalActionOffers(), guards.toArray(new DockActionSource[0]),
                parentSource, parents.toArray(new DockActionSource[0]));
    }
    ...
    protected DockTitleBindingListener[] dockTitleBindingListeners() {
        return dockTitleBindingListeners.toArray(
                new DockTitleBindingListener[0]);
    }
    ...
    protected DockableSelectionListener[] dockableSelectionListeners() {
        return dockableSelectionListeners.toArray(new DockableSelectionListener[0]);
    }
    ...
    private class DockableSelectionObserver extends DockRegisterAdapter {
         ...
         */
        private final DockStationListener listener = new DockStationAdapter() {
            @Override
    ...
    private class DockTitleObserver extends DockRegisterAdapter implements DockTitleBindingListener {
         ...
         */
        private final Set<DockTitle> titles = new HashSet<>();

        /**
         * a listener added to each {@link Dockable}
         */
        private final DockableListener dockableListener = new DockableAdapter() {
            @Override
