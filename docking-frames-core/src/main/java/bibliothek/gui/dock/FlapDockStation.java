...
public class FlapDockStation extends AbstractDockableStation {
     ...
     */
    public static final PropertyKey<FlapLayoutManager> LAYOUT_MANAGER = new PropertyKey<>(
     ...
     */
    public static final PropertyKey<ButtonContent> BUTTON_CONTENT = new PropertyKey<>(
            "flap dock station button content", new ConstantPropertyFactory<>(ButtonContent.THEME_DEPENDENT), true);

    /**
     * Key for all elements that depend from {@link #BUTTON_CONTENT}, adds additional information to the
     * {@link ButtonContent}.
     */
    public static final PropertyKey<ButtonContentFilter> BUTTON_CONTENT_FILTER = new PropertyKey<>(
            "flap dock station button content connector",
            new ConstantPropertyFactory<>(new DefaultButtonContentFilter()), true);

    /**
     * Key for the minimum size of all {@link FlapDockStation}s.
     */
    public static final PropertyKey<Dimension> MINIMUM_SIZE = new PropertyKey<>("flap dock station empty size",
            new ConstantPropertyFactory<>(new Dimension(0, 0)), true);
    ...
    public static final PropertyKey<FlapWindowFactory> WINDOW_FACTORY =
            new PropertyKey<>("flap dock station window factory",
     ...
     */
    private final FlapLayoutManagerListener layoutManagerListener = (manager, station, dockable) -> {
     ...
     */
    private final PropertyValue<FlapLayoutManager> layoutManager = new PropertyValue<>(LAYOUT_MANAGER) {
        @Override
     ...
     */
    private final PropertyValue<ButtonContent> buttonContent = new PropertyValue<>(BUTTON_CONTENT) {
        @Override
     ...
     */
    private final PropertyValue<Dimension> minimumSize = new PropertyValue<>(MINIMUM_SIZE) {
     ...
     */
    private final PropertyValue<FlapWindowFactory> windowFactory = new PropertyValue<>(WINDOW_FACTORY) {
     ...
     */
    private final Listener dockableListener = new Listener();
     ...
     */
    private final List<FlapDockListener> flapDockListeners = new ArrayList<>();
    ...
    protected void init() {
        ...
        buttonPane.addHierarchyListener(e -> {
    ...
    public void setController(DockController controller) {
        if (getController() != controller) {
            ...
            if (controller != null) {
                ...
                titleVersion =
                        controller.getDockTitleManager().getVersion(WINDOW_TITLE_ID, ControllerTitleFactory.INSTANCE);
                buttonVersion =
                        controller.getDockTitleManager().getVersion(BUTTON_TITLE_ID, BasicButtonTitleFactory.FACTORY);
    ...
    protected DockTitle.Orientation orientation(Direction direction) {
        return switch (direction) {
            case NORTH -> DockTitle.Orientation.SOUTH_SIDED;
            case SOUTH -> DockTitle.Orientation.NORTH_SIDED;
            case EAST -> DockTitle.Orientation.WEST_SIDED;
            case WEST -> DockTitle.Orientation.EAST_SIDED;
        };
    ...
    protected void fireHoldChanged(Dockable dockable, boolean value) {
        for (FlapDockListener listener : flapDockListeners.toArray(new FlapDockListener[0])) {
    ...
    public void setPlaceholders(PlaceholderMap placeholders) {
        ...
        try {
            DockablePlaceholderList<DockableHandle> next = new DockablePlaceholderList<>(placeholders);
            if (getController() != null) {
    ...
    public void setPlaceholders(PlaceholderMap map, final Map<Integer, Dockable> children) {
        ...
        try {
            ...
            handles = new DockablePlaceholderList<>();
            handles.read(map, new PlaceholderListItemAdapter<>() {
     ...
     */
    private FlapDropInfo prepareCombine(Dockable dockable, FlapWindow window, final Point mouseOnScreen,
                                        final boolean mouseOverTitle, Enforcement force) {
    ...
    public void aside(AsideRequest request) {
        ...
        if (location instanceof FlapDockProperty flapLocation) {
    ...
    protected class FlapDropOperation implements StationDropOperation {
        ...
        public void destroy(StationDropOperation next) {
            if (FlapDockStation.this.dropInfo == dropInfo) {
                if (!(next instanceof FlapDropOperation) || next.getTarget() != getTarget()) {
                    setDropInfo(null);
                }
            }
        }
    ...
    private class ButtonListener extends MouseInputAdapter {
        ...
        public void mouseReleased(MouseEvent e) {
            if (dockable.getDockParent() == FlapDockStation.this) {
                ...
                boolean enabled = strategy == null ||
                        (!strategy.isDisabled(dockable) && !strategy.isDisabled(FlapDockStation.this));
