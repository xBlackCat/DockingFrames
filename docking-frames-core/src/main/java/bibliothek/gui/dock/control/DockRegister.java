...
public class DockRegister {
     ...
     */
    private final Set<DockStation> protectedStations = new HashSet<>();
    /**
     * the known stations
     */
    private final List<DockStation> stations = new ArrayList<>();
    /**
     * the known dockables
     */
     ...
     */
    private final List<DockRegisterListener> registerListeners = new ArrayList<>();

    /**
     * an observer of the stations
     */
    private final StationListener stationListener = new StationListener();
     ...
     */
    private final Map<Dockable, Status> changeMap = new HashMap<>();

    /**
     * the order in which the elements of {@link #changeMap} first appeared
     */
    private final LinkedList<Dockable> changeQueue = new LinkedList<>();
     ...
     */
    public void kill() {
        List<DockStation> stations = new ArrayList<>(this.stations);
    ...
    /**
     * Marks <code>station</code> as protected. Any {@link DockStation} can be protected, a protected
     * {@link DockStation}
    ...
    public void remove(DockStation station) {
        if (stations.contains(station)) {
            ...
            DockUtilities.visit(station, new DockUtilities.DockVisitor() {
                private final Set<DockStation> ignored = new HashSet<>();
    ...
    public DockStation[] listDockStations() {
        return stations.toArray(new DockStation[0]);
    }
    ...
    public DockStation[] listRoots() {
        List<DockStation> list = new LinkedList<>();
        for (DockStation station : stations) {
        ...
        return list.toArray(new DockStation[0]);
    }
    ...
    public boolean willBeRegistered(Dockable dockable) {
        ...
        return switch (status) {
            case ADDED, REMOVED_AND_ADDED -> true;
            case ADDED_AND_REMOVED, REMOVED -> false;
    ...
    public Dockable[] listDockables() {
        return dockables.toArray(new Dockable[0]);
    }
    ...
    protected DockRegisterListener[] listDockRegisterListeners() {
        return registerListeners.toArray(new DockRegisterListener[0]);
    }
            ...
            if (!firing) {
                try {
                    ...
                    while (!changeQueue.isEmpty()) {
                        ...
                        switch (status) {
                            case ADDED -> addDockable(next, false);
                            case REMOVED -> removeDockable(next);
                            case ADDED_AND_REMOVED, REMOVED_AND_ADDED -> fireStalledChange(next);
                        }
                    }
                } finally {
                    firing = false;
                        ...
                        if (protectedAncestor == null || !DockUtilities.isAncestor(protectedAncestor, dockable)) {
                            ...
                            if (station != null && isProtected(station)) {
                                ...
                                if (current == null) {
                                    ...
                                    switch (current) {
                                        case REMOVED -> changeMap.put(dockable, Status.REMOVED_AND_ADDED);
                                        case ADDED_AND_REMOVED -> changeMap.put(dockable, Status.ADDED);
                                    }
                                }
                            }
                        }
                        ...
                        if (protectedAncestor == null || !DockUtilities.isAncestor(protectedAncestor, dockable)) {
                            ...
                            if (station != null && isProtected(station)) {
                                ...
                                if (current == null) {
                                    ...
                                    switch (current) {
                                        case ADDED -> changeMap.put(dockable, Status.ADDED_AND_REMOVED);
                                        case REMOVED_AND_ADDED -> changeMap.put(dockable, Status.REMOVED);
                                    }
                                }
                            }
                        }
