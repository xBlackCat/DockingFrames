...
public class DropOperation implements RelocateOperation {
    ...
    private final StationDropOperation operation;
        ...
        try {
            ...
            if (operation.isMove()) {
                DefaultDockRelocatorEvent event =
                        new DefaultDockRelocatorEvent(controller, selection, new Dockable[]{}, station, mouse, true);
                ...
                if (parent != null) {
                    DefaultDockRelocatorEvent event =
                            new DefaultDockRelocatorEvent(controller, selection, new Dockable[]{}, station, mouse,
                                    false);
                    listener.dragging(event);
                    if (event.isCanceled() || event.isForbidden()) {
                        return false;
