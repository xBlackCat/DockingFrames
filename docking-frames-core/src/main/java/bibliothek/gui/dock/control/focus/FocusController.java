...
public interface FocusController {
     ...
     */
    FocusVeto checkFocusedDockable(DockElementRepresentative source);
     ...
     * @param component             the {@link Component} which triggered this call for example because the user
     *                              clicked with the mouse on it.
     *                              This method can assume that the focus will automatically be transferred to
     *                              <code>component</code> by the Swing framework itself.
     *                              Can be <code>null</code>, in which case this method decides on its own which
     *                              {@link Component} to focus. This method may or may
     ...
     * @return whether focus could be transferred, a value of <code>null</code> indicates that
     * {@link #isOnFocusing()} returned
    ...
    FocusVeto setFocusedDockable(DockElementRepresentative source, Component component, boolean force,
                                 boolean ensureFocusSet, boolean ensureDockableFocused);

    /**
     * Sets the {@link Dockable} which should have the focus.
     *
     * @param request information about the {@link Dockable} that should receive the focus, must not be
     *                <code>null</code>
     */
    void focus(FocusRequest request);

    /**
     * After the currently executed {@link FocusRequest} is completed, or if there is currently no
     * {@link FocusRequest} running,
     * <code>run</code> is executed. If this controller is {@link #freezeFocus() frozen}, then <code>run</code> is
     * executed
     * Immediately.
     *
     ...
     */
    void onFocusRequestCompletion(Runnable run);
}
