...
public class DefaultMouseFocusObserver implements MouseFocusObserver {
     ...
     */
    private final VetoableDockRelocatorListener relocatorListener = new VetoableDockRelocatorAdapter() {
     ...
     */
    private final GlobalMouseListener listener = new GlobalMouseListener();
    ...
    public DefaultMouseFocusObserver(DockController controller, ControllerSetupCollection setup) {
        ...
        setup.add(controller1 -> {
            controller1.getRelocator().addVetoableDockRelocatorListener(relocatorListener);
            GlobalMouseDispatcher dispatcher = controller1.getGlobalMouseDispatcher();
     ...
     * @param event                the event that causes this check
     */
    protected void check(final Component component, final boolean ensureFocus, boolean requestFocusInWindow,
        ...
        if (change) {
            if (component instanceof FocusAwareComponent aware) {
                if (requestFocusInWindow) {
                    aware.maybeRequestFocus();
                }
                aware.invokeOnFocusRequest(() -> {
                    DockElementRepresentative element1 = getDockable(component, event);
                    if (element1 != null) {
                        Dockable dock1 = element1.getElement().asDockable();
                        if (dock1 != null) {
                            controller.setFocusedDockable(new DefaultFocusRequest(dock1, component, false,
                                    ensureFocus, element1.shouldTransfersFocus()));
                        }
                    }
                });
            } else {
                ...
                controller.setFocusedDockable(new DefaultFocusRequest(dock, component, false, ensureFocus,
                        element.shouldTransfersFocus()));
            }
        }
    }
