...
public class DefaultDockRelocator extends AbstractDockRelocator {
    ...
    public static final PropertyKey<Boolean> AUTO_DROP_ON_ANY_MOUSE_RELEASED_EVENT =
            new PropertyKey<>("dock.default.relocator.autodrop",
                    new ConstantPropertyFactory<>(true), true);
    ...
    public DefaultDockRelocator(DockController controller, ControllerSetupCollection setup) {
        ...
        setup.add(controller1 -> {
            controller1.addRepresentativeListener(listeners);

            List<Merger> mergers =
                    controller1.getExtensions().load(new ExtensionName<>(MERGE_EXTENSION, Merger.class,
                            EXTENSION_PARAM, DefaultDockRelocator.this));
            for (Merger next : mergers) {
                merger.add(next);
            }

            List<Inserter> inserters =
                    controller1.getExtensions().load(new ExtensionName<>(INSERTER_EXTENSION, Inserter.class,
                            EXTENSION_PARAM, DefaultDockRelocator.this));
    ...
    private StationDropItem createStationDropItem(int mouseX, int mouseY, int titleX, int titleY, Dockable dockable) {
        if (lastItem == null || lastItem.getDockable() != dockable) {
            ...
            lastItem =
                    new StationDropItem(mouseX, mouseY, titleX, titleY, dockable, lastItem.getOriginalSize(),
                            lastItem.getMinimumSize());
        }
        return lastItem;
    }
        ...
        for (DockStation station : list) {
            ...
            if (result != null) {
                ...
                DefaultDockRelocatorEvent event =
                        new DefaultDockRelocatorEvent(getController(), dockable, result.getImplicit(dockable),
                                station, new Point(mouseX, mouseY), move);
                fireSearched(event);
    ...
    protected List<DockStation> listStationsOrdered(int x, int y, Dockable moved) {
        ...
        DockStationDropLayerFactory dropLayerFactory =
                controller.getProperties().get(DockStationDropLayerFactory.DROP_LAYER_FACTORY);
            ...
            if (!isCancelLocation(x, y, moved)) {
                for (DockStation station : controller.getRegister().listDockStations()) {
                    if (disabling == null || !disabling.isDisabled(station)) {
                        if (movedStation == null ||
                                (!DockUtilities.isAncestor(movedStation, station) && movedStation != station)) {
     ...
     *                  if one of the usual conditions is not met. I.e. dragging will start even if
     *                  <code>dockable</code>
    ...
    protected Reaction dragMousePressed(int x, int y, int dx, int dy, int modifiers, Dockable dockable,
        ...
        if (((modifiers & (onmask | offmask)) == onmask)) {
            ...
            DefaultDockRelocatorEvent event = new DefaultDockRelocatorEvent(getController(), dockable, implicit,
                    operation == null ? null : operation.getStation(), new Point(x, y), move);
            event.cancel();
            fireCanceled(event);
    ...
    protected Reaction dragMouseDragged(int x, int y, int modifiers, DockTitle title, Dockable dockable,
                                        boolean always, boolean forceDrag) {
     ...
     *                        if one of the usual conditions is not met. I.e. dragging will start even if
     *                        <code>dockable</code>
     *                        does not have a parent of even if the parent does not allow dragging. This flag should
     *                        be used
     *                        with caution.
     ...
     */
    protected Reaction dragMouseDragged(int x, int y, int modifiers, DockTitle title, Dockable dockable,
                                        boolean always, boolean forceDrag, boolean showMovingImage) {
        listeners.unsetLastActiveListener();
    ...
    private Reaction initiateOperation(Dockable dockable, DockTitle title, Point mouse, boolean showMovingImage) {
        ...
        DefaultDockRelocatorEvent event =
                new DefaultDockRelocatorEvent(getController(), dockable, implicit, null, mouse, false);
        fireGrabbing(event);
        if (event.isIgnored()) {
            return Reaction.CONTINUE;
        }
    ...
    private Reaction selectNextTarget(Dockable dockable, DockTitle title, Point mouse) {
        ...
        DefaultDockRelocatorEvent event = new DefaultDockRelocatorEvent(getController(), dockable, implicit,
                next == null ? null : next.getStation(), mouse, move);
        fireDragged(event);
        if (event.isCanceled()) {
            cancel(dockable);
            return onBreak(event);
    ...
    protected Reaction dragMouseReleased(int x, int y, int modifiers, Dockable dockable) {
        ...
        if (stop && onMove) {
            EventQueue.invokeLater(() -> onMove = false);
        }
        ...
        if (!onMove) {
            ...
            if (stop) {
                DefaultDockRelocatorEvent event =
                        new DefaultDockRelocatorEvent(getController(), dockable, new Dockable[]{}, null, mouse, false);
                event.cancel();
                fireCanceled(event);
            ...
            if (pressPointScreen != null) {
                ...
                if (x != lastPoint.x || y != lastPoint.y) {
                    ...
                    if (next != null) {
                        ...
                        DefaultDockRelocatorEvent event =
                                new DefaultDockRelocatorEvent(getController(), dockable, implicit, next.getStation(),
                                        mouse, move);
                        fireCanceled(event);
                        if (event.isCanceled() || event.isForbidden()) {
                            next = null;
                ...
                if (operation != null) {
                    ...
                    DefaultDockRelocatorEvent event =
                            new DefaultDockRelocatorEvent(getController(), dockable, implicit, operation.getStation()
                                    , mouse, move);
                    event.drop();
                    fireDropping(event);
                    if (event.isCanceled() || event.isForbidden()) {
                ...
                if (operation != null) {
                    ...
                    dropped =
                            new DefaultDockRelocatorEvent(getController(), dockable, implicit, operation.getStation()
                                    , mouse, operation.getOperation().isMove());
                    if (canceled) {
                        dropped.cancel();
                    ...
                    DefaultDockRelocatorEvent event =
                            new DefaultDockRelocatorEvent(getController(), dockable, new Dockable[]{}, null, mouse,
                                    false);
                    event.cancel();
                    fireCanceled(event);
                }
    ...
    private void cancel(Dockable dockable) {
        titleDragCancel();
        onMove = false;
        DefaultDockRelocatorEvent event = new DefaultDockRelocatorEvent(getController(), dockable, new Dockable[]{},
                null, null, false);
        event.cancel();
        fireCanceled(event);
    }
    ...
    private class DefaultRemoteRelocator implements RemoteRelocator, DirectRemoteRelocator {
         ...
         */
        private final boolean forceDrag;
         ...
         *                  if one of the usual conditions is not met. I.e. dragging will start even if
         *                  <code>dockable</code>
         *                  does not have a parent of even if the parent does not allow dragging. This flag should be
         *                  used
         *                  with caution.
         */
    ...
    private class Listener implements DockControllerRepresentativeListener {
        private final Map<DockElementRepresentative, MouseRepresentativeListener> listeners =
                new HashMap<>();

        /**
         * the last {@link MouseRepresentativeListener} that is registered in {@link #listeners}, and that has
         * received a {@link MouseEvent}
         */
    ...
    private class MouseRepresentativeListener extends MouseInputAdapter {
         ...
         */
        private final DockElementRepresentative representative;
    ...
    private class GlobalMouseReleaseListener extends MouseInputAdapter {
        ...
        private final PropertyValue<Boolean> autoCancel = new PropertyValue<>(AUTO_DROP_ON_ANY_MOUSE_RELEASED_EVENT) {
            @Override
