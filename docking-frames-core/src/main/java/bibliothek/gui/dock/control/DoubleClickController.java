...
public class DoubleClickController {
     ...
     */
    private final LocatedListenerList<DoubleClickListener> observers =
            new LocatedListenerList<>();

    /**
     * A map that tells which listener was added to which {@link Dockable}
     */
    private final Map<DockElementRepresentative, GlobalDoubleClickListener> listeners =
            new HashMap<>();
    ...
    public DoubleClickController(ControllerSetupCollection setup) {
        setup.add(controller -> controller.addRepresentativeListener(new DockControllerRepresentativeListener() {
