...
@Todo(compatibility = Compatibility.COMPATIBLE, priority = Priority.MAJOR, target = Version.VERSION_1_1_2,
        description = "In Java 1.7 if a mouse-dragged is followed by a mouse-exit, and the mouse is over another " +
                "GlassedPane, then this GlassedPane no longer receives events that it received in Java 1.6")
public class GlassedPane extends JPanel {
    /**
     * the strategy used by a {@link GlassedPane} to manage its tooltips
     */
    public static final PropertyKey<TooltipStrategy> TOOLTIP_STRATEGY = new PropertyKey<>("tooltip strategy",
     ...
     */
    private final PropertyValue<TooltipStrategy> tooltips = new PropertyValue<>(TOOLTIP_STRATEGY) {
        @Override
    ...
    public class GlassPane extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
         ...
         */
        private final TooltipStrategyCallback callback = new TooltipStrategyCallback() {
