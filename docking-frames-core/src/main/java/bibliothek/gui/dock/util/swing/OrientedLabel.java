    ...
    public Dimension getPreferredSize() {
        ...
        if (isHorizontal()) {
            ...
            return new Dimension(
                    size.width + 5 + iconOffset + iconTextDistance + icon.getIconWidth(), Math.max(size.height,
                    icon.getIconHeight() + 2 * iconOffset));
        } else {
            if (icon == null) {
                return new Dimension(size.height, size.width + 5);
            }

            return new Dimension(Math.max(size.height, icon.getIconWidth() + 2 * iconOffset),
                    size.width + 5 + iconOffset + iconTextDistance + icon.getIconHeight());
        }
    }
