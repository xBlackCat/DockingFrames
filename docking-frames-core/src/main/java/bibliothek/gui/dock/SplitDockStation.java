...
public class SplitDockStation extends SecureContainer implements Dockable, DockStation {
    ...
    public static final PropertyKey<KeyStroke> MAXIMIZE_ACCELERATOR =
            new PropertyKey<>("SplitDockStation maximize accelerator");
    ...
    public static final PropertyKey<SplitLayoutManager> LAYOUT_MANAGER =
            new PropertyKey<>("SplitDockStation layout manager",
     ...
     */
    public static final PropertyKey<SplitDividerStrategy> DIVIDER_STRATEGY =
            new PropertyKey<>("SplitDockStation divider strategy",
     ...
     */
    private final VisibleListener visibleListener = new VisibleListener();
     ...
     */
    private final StationCombinerValue combiner;
     ...
     */
    private final DockableStateListenerManager dockableStateListeners;

    /**
     * an observer ensuring that the {@link DockHierarchyEvent}s are sent properly
     */
    private final DockHierarchyObserver hierarchyObserver;

    /**
     * A list of {@link SplitDockListener} which will be invoked when something noticable happens
     */
    private final List<SplitDockListener> splitListeners = new ArrayList<>();

    /**
     * The handler for events and listeners concerning the visibility of children
     */
    private final DockableShowingManager visibility;

    /**
     * the DockTitles which are bound to this dockable
     */
    private final List<DockTitle> titles = new LinkedList<>();

    /**
     * the list of actions offered for this Dockable
     */
    private final HierarchyDockActionSource globalSource;
     ...
     */
    protected final DockStationListenerManager dockStationListeners = new DockStationListenerManager(this);

    /**
     * Optional text for this station
     */
    private final PropertyValue<String> titleText = new PropertyValue<>(PropertyKey.DOCK_STATION_TITLE) {
        @Override
        protected void valueChanged(String oldValue, String newValue) {
            ...
            for (DockableListener listener : dockableListeners.toArray(new DockableListener[0])) {
     ...
     */
    private final DockIcon titleIcon;

    /**
     * Optional tooltip for this station
     */
    private final PropertyValue<String> titleToolTip = new PropertyValue<>(PropertyKey.DOCK_STATION_TOOLTIP) {
        @Override
        protected void valueChanged(String oldValue, String newValue) {
            for (DockableListener listener : dockableListeners.toArray(new DockableListener[0])) {
     ...
     */
    private final PropertyValue<SplitLayoutManager> layoutManager = new PropertyValue<>(LAYOUT_MANAGER) {
        @Override
     ...
     */
    private final PropertyValue<SplitDividerStrategy> dividerStrategy = new PropertyValue<>(DIVIDER_STRATEGY) {
        @Override
    ...
    private final PropertyValue<PlaceholderStrategy> placeholderStrategyProperty =
     ...
     */
    private final PropertyValue<DisablingStrategy> disablingStrategy = new PropertyValue<>(DisablingStrategy.STRATEGY) {
        @Override
     ...
     */
    private final DisablingStrategyListener disablingStrategyListener = item -> {
     ...
     */
    private final RootPlaceholderStrategy placeholderStrategy = new RootPlaceholderStrategy(this);
     ...
     */
    private final List<StationChildHandle> dockables = new ArrayList<>();
     ...
     */
    private final SplitPlaceholderSet placeholderSet;
     ...
     */
    private final SplitSpanStrategy spanStrategy;
    ...
    public SplitDockStation(boolean createFullScreenAction) {
        ...
        placeholderStrategy.addListener(this::removePlaceholders);
    ...
    public DockTitle[] listBoundTitles() {
        return titles.toArray(new DockTitle[0]);
    }
    ...
    public DefaultDockActionSource getDirectActionOffers(Dockable dockable) {
        if (fullScreenAction == null) {
            ...
            DefaultDockActionSource source =
                    new DefaultDockActionSource(new LocationHint(LocationHint.DIRECT_ACTION, LocationHint.VERY_RIGHT));
    ...
    public void aside(AsideRequest request) {
        ...
        if (location instanceof SplitDockPlaceholderProperty property) {
        ...
        if (location instanceof SplitDockPathProperty property) {
        ...
        if (location instanceof SplitDockProperty property) {
        ...
        if (result) {
            if (request.getPlaceholder() == null) {
                ...
                SplitDockPlaceholderProperty newLocation =
                        new SplitDockPlaceholderProperty(request.getPlaceholder(), path);
    ...
    public SplitDockPathProperty getDockablePathProperty(final Dockable dockable) {
        return root().submit(new SplitTreePathFactory() {
            public SplitDockPathProperty leaf(Dockable check, long id, Path[] placeholders,
            ...
            if (putInfo.getNode() instanceof Leaf) {
                ...
                if (target == null && putInfo.isCombining() && putInfo.getDockable().asDockStation() != null) {
                    ...
                    if (controller != null) {
                        ...
                        if (!merger.canMerge(new SplitDropOperation(access, putInfo, item, move), this,
                                putInfo.getDockable().asDockStation())) {
    ...
    private void drop(DockHierarchyLock.Token token, PutInfo putInfo, StationDropItem item) {
        try {
            ...
            if (putInfo.getNode() == null) {
                ...
                if (putInfo.getCombinerTarget() != null) {
                    if (putInfo.getNode() instanceof Leaf) {
                        if (putInfo.getLeaf() != null) {
                            if (fire) {
                                token = DockHierarchyLock.acquireUnlinking(SplitDockStation.this,
                                        putInfo.getLeaf().getDockable());
                            }
                            try {
                        ...
                        if (dropOver((Leaf) putInfo.getNode(), putInfo.getDockable(), putInfo.getCombinerSource(),
                ...
                if (!finish) {
                    updateBounds();
                    layoutManager.getValue().calculateDivider(SplitDockStation.this, putInfo,
                            root().getLeaf(putInfo.getDockable()), item);
                    dropAside(putInfo.getNode(), putInfo.getPut(), putInfo.getDockable(), putInfo.getLeaf(),
                            putInfo.getDivider(), token);
                }
            }
     ...
     * otherwise
     */
    protected boolean dropOver(Leaf leaf, Dockable dockable, DockableProperty property, CombinerSource source,
        ...
        try {
            ...
            if (property != null) {
                ...
                if (combinedStation != null && dockable.getDockParent() == combinedStation) {
                    if (source.getPlaceholders() == null ||
                            getPlaceholderStrategy().getPlaceholderFor(dockable) == null) {
                        // if a placeholder is present, then the combiner should have set the dockable at the correct
                        // position
     ...
     */
    protected boolean dropAside(SplitNode neighbor, PutInfo.Put put, Dockable dockable, Leaf leaf, double divider,
    ...
    public void dropTree(SplitDockTree<Dockable> tree, boolean checkValidity) {
        ...
        try {
            ...
            SplitDockTree.Key<Dockable> rootKey = tree.getRoot();
            if (rootKey != null) {
                Map<Leaf, Dockable> linksToSet = new HashMap<>();
    ...
    protected void fireFullScreenChanged(Dockable oldDockable, Dockable newDockable) {
        for (SplitDockListener listener : splitListeners.toArray(new SplitDockListener[0])) {
    ...
    /**
     * Adds <code>handle</code> to the list of dockables,also adds the {@link DockableDisplayer} of the new handle to
     * this station.
     *
    ...
    public void removePlaceholder(Path placeholder) {
        Set<Path> placeholders = new HashSet<>();
    ...
    public void removePlaceholders(final Set<Path> placeholders) {
        ...
        final List<SplitNode> nodesToDelete = new ArrayList<>();
        root().visit(new SplitNodeVisitor() {
            public void handleRoot(Root root) {
                handle(root);
            }
    ...
    public void updateBounds() {
        ...
        if (factorW <= 0 || factorH <= 0) {
            manager.updateBounds(root(), 0, 0, 0, 0);
        } else {
    ...
    protected class Access implements SplitDockAccess {
        ...
        private final Set<Dockable> repositioned = new HashSet<>();
        ...
        public void fire() {
            ...
            if (repositionedArm == 0) {
                ...
                if (dockables.size() > 0) {
                    dockStationListeners.fireDockablesRepositioned(dockables.toArray(new Dockable[0]));
                }
