...
public abstract class DisplayerRequest extends ResourceRequest<DockableDisplayer> {
     ...
     * @param defaultFactory the default factory, to be used if no other way was found to create the
     * {@link DockableDisplayer}
     * @param displayerId    a unique identifier that depends on the type of <code>parent</code>, this identifier
     *                       will be forwarded to
     *                       {@link Extension}s allowing them an easy way to filter uninteresting
     *                       {@link DisplayerRequest}s, must not be <code>null</code>
     */
    ...
    /**
     * Sets the {@link DockController} in whose realm this {@link DisplayerRequest} is used. The
     * {@link DockController} can
    ...
    public void setController(DockController controller) {
        if (this.controller != controller) {
            ...
            if (controller != null) {
                extensions =
                        controller.getExtensions().share(new ExtensionName<>(DisplayerFactory.DISPLAYER_EXTENSION,
                                DisplayerFactory.class, DisplayerFactory.DISPLAYER_EXTENSION_ID, displayerId));
                extensions.bind();
            }
        }
    }
