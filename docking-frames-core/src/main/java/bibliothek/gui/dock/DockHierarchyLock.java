...
public class DockHierarchyLock {
     ...
     */
    private final Queue<Runnable> onRelease = new LinkedList<>();
    ...
    private String defaultMessage() {
        return """
                During an operation the framework attempted to acquire the same lock twice. There are two possible explanations:
    ...
    private void ensureLinked(DockStation station, Dockable dockable) {
        if (dockable.getDockParent() != station) {
            throwException(new IllegalStateException(
                    "the parent of '" + dockable + "' is not '" + station + "' but '" + dockable.getDockParent() +
                            "'"));
            return;
        }
        boolean found = false;
    ...
    private void ensureUnlinked(DockStation station, Dockable dockable) {
        if (dockable.getDockParent() != null) {
            throwException(new IllegalStateException(
                    "The parent of '" + dockable + "' is not null but '" + dockable.getDockParent() + "'"));
            return;
        }
    ...
    public static class Token {
        private final DockHierarchyLock lock;
