...
public class StackDockStation extends AbstractDockableStation implements StackDockComponentParent {
    ...
    public static final PropertyKey<Boolean> IMMUTABLE_SELECTION_INDEX =
            new PropertyKey<>("stack dock immutable selection index",
                    new ConstantPropertyFactory<>(false), true);
    ...
    public static final PropertyKey<TabConfigurations> TAB_CONFIGURATIONS =
            new PropertyKey<>("stack dock tab configurations",
                    new ConstantPropertyFactory<>(TabConfigurations.DEFAULT), true);
    ...
    public static final PropertyKey<StackDnDAutoSelectSupport> DND_AUTO_SELECT_SUPPORT =
            new PropertyKey<>("stack dock auto select");
    ...
    public static final PropertyKey<DndAutoSelectStrategy> DND_AUTO_SELECT_STRATEGY =
            new PropertyKey<>("stack dock auto select strategy",
                    new ConstantPropertyFactory<>(DndAutoSelectStrategy.DEFAULT), true);
    ...
    /**
     * Responsible for updating a {@link DockElementRepresentative} that covers the empty areas of
     * {@link #stackComponent}
     */
     ...
     */
    private final PropertyValue<TabContentFilter> tabContentFilter = new PropertyValue<>(TAB_CONTENT_FILTER) {
        @Override
     ...
     */
    private final TabContentFilterListener tabContentFilterListener = new TabContentFilterListener() {
     ...
     */
    private final PropertyValue<StackDnDAutoSelectSupport> autoSelectSupport =
            new PropertyValue<>(DND_AUTO_SELECT_SUPPORT) {
                @Override
    ...
    public boolean isVisible(Dockable dockable) {
        return isStationVisible() &&
                (dockables.dockables().size() == 1 || indexOf(dockable) == stackComponent.getSelectedIndex());
    }
    ...
    public void aside(AsideRequest request) {
        ...
        if (location instanceof StackDockProperty stackLocation) {
    ...
    public void setPlaceholders(PlaceholderMap placeholders) {
        ...
        try {
            DockablePlaceholderList<StationChildHandle> next = new DockablePlaceholderList<>(placeholders);
    ...
    public void setPlaceholders(PlaceholderMap placeholders, final Map<Integer, Dockable> children) {
        ...
        try {
            ...
            dockables.setStrategy(null);
            dockables.unbind();
            DockablePlaceholderList<StationChildHandle> next = new DockablePlaceholderList<>();

            dockables = next;
            next.read(placeholders, new PlaceholderListItemAdapter<>() {
                ...
                public StationChildHandle convert(ConvertedPlaceholderListItem item) {
                    ...
                    StationChildHandle handle =
                            new StationChildHandle(StackDockStation.this, getDisplayers(), dockable, title);
     ...
     * allows clients to make this {@link StackDockStation} small if it is on a {@link SplitDockStation} or another
     * station
     * that pays attention to the minimum size. This even works if the {@link Dockable}s do not have a small minimum
     * size.
     *
     ...
     * @param autoPlaceholder whether the {@link PlaceholderStrategy} can be invoked to search for a matching
     *                        placeholder
     */
    ...
    private boolean validate(Insert insert, Dockable child) {
        return insert != null && accept(child) && child.accept(this) &&
                getController().getAcceptance().accept(this, child);
    }
     ...
     *               <code>handle</code> is not yet stored or the size of {@link #dockables}-1 is <code>handle</code>
     *               already is
     *               stored.
     */
        ...
        if (size == 0 && !singleTabStackDockComponent()) {
            ...
            if (size == 1 && !singleTabStackDockComponent()) {
                ...
                if (list.get(0) == handle) {
                    if (list.size() != 2) {
                        throw new IllegalStateException("handle is stored, size is 1, but number of known dockables " +
                                "is not 2");
                    }
                    handle = list.get(1);
                    index = 1;
    ...
    private void updateContent(int index) {
        if (index >= 0 && (getDockableCount() > 1 || singleTabStackDockComponent())) {
            ...
            TabContent content =
                    new TabContent(dockable.getTitleIcon(), dockable.getTitleText(), dockable.getTitleToolTip());
            if (filter != null) {
                content = filter.filter(content, this, dockable);
            }
            if (content == null) {
    ...
    protected class Background extends SecureContainer {
        private final BackgroundPanel content;
        ...
        protected void paintOverlay(Graphics g) {
            ...
            if (insert != null && dockables.dockables().size() > 1) {
                ...
                if (bounds != null) {
                    ...
                    if (insert.right) {
                        insertionLine(bounds, insert.tab + 1 < stackComponent.getTabCount() ?
                                stackComponent.getBoundsAt(insert.tab + 1) : null, a, b, true);
                    } else {
                        insertionLine(insert.tab > 0 ? stackComponent.getBoundsAt(insert.tab - 1) :
                                null, bounds, a, b, false);
                    }
     ...
     * @param right Whether the gap is left or right of {@link #tab}
     * @author Benjamin Sigg
     */
    public record Insert(int tab, boolean right) {
        /**
        ...
        /**
         * Whether the tab is at the left or the right of {@link #tab ()}.
         *
