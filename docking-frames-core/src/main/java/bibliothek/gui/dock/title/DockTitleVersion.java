...
public class DockTitleVersion implements DockTitleFactory {
     ...
     */
    private final DockTitleFactory[] factories = new DockTitleFactory[3];
    /**
     ...
     */
    private final List<DockTitleRequest> requests = new ArrayList<>();

    /**
     * whether the theme is currently changing, a version does automatically call {@link DockTitleRequest#request()}
    ...
    public DockTitleVersion(DockController controller, String id) {
        ...
        List<DockTitleFactory> list =
                controller.getExtensions().load(new ExtensionName<>(DOCK_TITLE_VERSION_EXTENSION,
                        DockTitleFactory.class, DOCK_TITLE_VERSION_EXTENSION_PARAMETER, this));
        if (!list.isEmpty()) {
            extensionFactories = list.toArray(new DockTitleFactory[0]);
        }
    }
    ...
    public DockTitleFactory getFactory() {
        for (DockTitleFactory factory : factories) {
            if (factory != null) {
                return factory;
            }
        }
