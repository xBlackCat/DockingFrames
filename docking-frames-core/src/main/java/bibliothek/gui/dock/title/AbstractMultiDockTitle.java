 ...
 * Clients should make use of {@link AbstractDockTitle} which is also the super-class for all the {@link DockTitle}s
 * that
...
public abstract class AbstractMultiDockTitle extends ConfiguredBackgroundPanel implements DockTitle {
     ...
     */
    private final List<AbstractDockColor> colors = new ArrayList<>();
    /**
     * the fonts used by this title
     */
    private final List<AbstractDockFont> fonts = new ArrayList<>();
    /**
     ...
     */
    private final PropertyValue<OrientationToRotationStrategy> orientationConverter =
            new PropertyValue<>(DockTitle.ORIENTATION_STRATEGY) {
                protected void valueChanged(OrientationToRotationStrategy oldValue,
                                            OrientationToRotationStrategy newValue) {
                    if (oldValue != null) {
    ...
    /**
     * a listener added to the current {@link OrientationToRotationStrategy} represented by
     * {@link #orientationConverter}
     */
    private final OrientationToRotationStrategyListener orientationListener = (dockable, title) -> {
    ...
    protected void addConditionalFont(String id, Path kind, Condition condition, FontModifier backup) {
        ...
        if (conditionalFonts == null) {
            conditionalFonts = new ArrayList<>();
        }
    ...
    public Rectangle getIconBounds() {
        ...
        if (orientation.isVertical()) {
            ...
            return new Rectangle(insets.left +
                    (width - icon.getIconWidth()) / 2, insets.top, icon.getIconWidth(), icon.getIconHeight());
        } else {
            int height = getHeight() - insets.top - insets.bottom;
            return new Rectangle(insets.left,
                    insets.top + (height - icon.getIconHeight()) / 2, icon.getIconWidth(), icon.getIconHeight());
        }
    }
    ...
    private class TitleBorder implements TitleDockBorder {
        ...
        public void setController(DockController controller) {
            ...
            if (this.controller == null) {
                ...
                this.controller.getThemeManager().add(id, TitleDockBorder.KIND, ThemeManager.BORDER_MODIFIER_TYPE,
                        this);
            }
        }
    }
