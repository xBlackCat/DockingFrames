...
public abstract class AbstractScreenDockWindow extends DisplayerScreenDockWindow {
     ...
     */
    private final DockableListener listener = new DockableAdapter() {
        @Override
    ...
    /**
     * a listener added to this window, will inform other listeners about a changed fullscreen mode when this window
     * is resized
     */
    private final ScreenDockWindowListener windowListener = new ScreenDockWindowListener() {
     ...
     */
    public AbstractScreenDockWindow(ScreenDockStation station, WindowConfiguration configuration, Component window,
     ...
     * @param borderAllowed If <code>true</code> and if {@link WindowConfiguration#isResizeable()}, then a new border
     *                     is installed
     ...
     *                      subclass.
     */
    protected void init(Component window, Container contentParent, WindowConfiguration configuration,
    ...
    public void setWindowBounds(Rectangle bounds, Position position) {
        ...
        if (valid == null) {
            ...
            switch (position) {
                case E -> bounds = new Rectangle(bounds.x, valid.y, valid.width, valid.height);
                case N -> bounds =
                        new Rectangle(valid.x, bounds.y + bounds.height - valid.height, valid.width, valid.height);
                case S -> bounds = new Rectangle(valid.x, bounds.y, valid.width, valid.height);
                case W -> bounds =
                        new Rectangle(bounds.x + bounds.width - valid.width, bounds.y, valid.width, valid.height);
                case NE -> bounds =
                        new Rectangle(bounds.x, bounds.y + bounds.height - valid.height, valid.width, valid.height);
                case NW -> bounds = new Rectangle(
                        bounds.x + bounds.width - valid.width,
                        bounds.y + bounds.height - valid.height, valid.width, valid.height);
                case SE -> bounds = new Rectangle(bounds.x, bounds.y, valid.width, valid.height);
                case SW -> bounds =
                        new Rectangle(bounds.x + bounds.width - valid.width, bounds.y, valid.width, valid.height);
                default -> bounds = valid;
            }
            setWindowBounds(bounds);
        }
    }
    ...
    private class Listener implements MouseListener, MouseMotionListener, ComponentListener {
        ...
        private void updateCursor(MouseEvent e) {
            ...
            boolean valid = e.getComponent() == component && e.getY() <= insets.top ||
