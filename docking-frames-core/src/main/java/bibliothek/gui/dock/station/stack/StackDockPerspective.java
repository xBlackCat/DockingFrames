...
public class StackDockPerspective implements PerspectiveDockable, PerspectiveStation {
    ...
    public void read(PlaceholderMap placeholders, final Map<Integer, PerspectiveDockable> children, int selected) {
        PerspectivePlaceholderList<PerspectiveDockable> dockables = new PerspectivePlaceholderList<>();
    ...
    public void insertPlaceholder(int index, Path placeholder, Level level) {
        switch (level) {
            case BASE -> dockables.list().insertPlaceholder(index, placeholder);
            case DOCKABLE -> dockables.dockables().insertPlaceholder(index, placeholder);
            case PLACEHOLDER -> dockables.purePlaceholders().insertPlaceholder(index, placeholder);
        }
    }
