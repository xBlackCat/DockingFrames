 ...
 */
public class SplitDockStationFactory implements DockFactory<SplitDockStation, SplitDockPerspective,
    ...
    /**
     * Creates a new layout for <code>station</code>. The default implementation just calls
     * {@link #createLayout(SplitDockStationLayout.Entry, int, boolean)}.
     *
     ...
     */
    protected SplitDockStationLayout createLayout(SplitDockStation station, Entry root, int fullscreen,
     ...
     * @return the new layout
     */
    protected SplitDockStationLayout createLayout(SplitDockPerspective station, Entry root, int fullscreen,
                                                  boolean hasFullscreenAction) {
        return createLayout(root, fullscreen, hasFullscreenAction);
    }
    ...
    public SplitDockStationLayout getLayout(final SplitDockStation station, final Map<Dockable, Integer> children) {
        Entry root =
                station.visit(new SplitTreeFactory<>() {
                    private final PlaceholderStrategy strategy = station.getPlaceholderStrategy();
                    ...
                    public Entry horizontal(Entry left, Entry right, double divider, long id, Path[] placeholders,
                                            PlaceholderMap placeholderMap, boolean visible) {
                        if (left == null) {
                            return right;
                        }
                        ...
                        return new SplitDockStationLayout.Node(Orientation.HORIZONTAL, divider, left, right,
                                placeholders, placeholderMap, id);
                    }

                    public Entry vertical(Entry top, Entry bottom, double divider, long id, Path[] placeholders,
                                          PlaceholderMap placeholderMap, boolean visible) {
                        if (top == null) {
                            return bottom;
                        }
                        ...
                        return new SplitDockStationLayout.Node(Orientation.VERTICAL, divider, top, bottom,
                                placeholders, placeholderMap, id);
                    }
                });
        ...
        return createLayout(station, root, Objects.requireNonNullElse(fullscreen, -1), station.hasFullScreenAction());
    }

    public SplitDockStationLayout getPerspectiveLayout(SplitDockPerspective element, Map<PerspectiveDockable,
            Integer> children) {
        if (children == null) {
        ...
        return createLayout(element, root, Objects.requireNonNullElse(fullscreen, -1), element.hasFullscreenAction());
    }
    ...
    private Entry convert(SplitDockPerspective.Entry entry, Map<PerspectiveDockable, Integer> children) {
        ...
        if (entry.asNode() != null) {
            ...
            return new SplitDockStationLayout.Node(node.getOrientation(), node.getDivider(), childA, childB,
                    toArray(node.getPlaceholders()), node.getPlaceholderMap(), node.getNodeId());
        } else if (entry.asLeaf() != null) {
            SplitDockPerspective.Leaf leaf = entry.asLeaf();
            Integer id = children.get(leaf.getDockable());
            return new SplitDockStationLayout.Leaf(
                    id == null ? -1 : id, toArray(leaf.getPlaceholders()), leaf.getPlaceholderMap(), leaf.getNodeId());
        } else {
    ...
    private Path[] toArray(Set<Path> placeholders) {
        ...
        return placeholders.toArray(new Path[0]);
    }
    ...
    public void setLayout(SplitDockStation station, SplitDockStationLayout layout, Map<Integer, Dockable> children,
        ...
        DockableSplitDockTree.Key<Dockable> root = null;
    ...
    public void layoutPerspective(SplitDockPerspective perspective, SplitDockStationLayout layout, Map<Integer,
            PerspectiveDockable> children) {
        if (children != null) {
            PerspectiveSplitDockTree tree = new PerspectiveSplitDockTree();
            PerspectiveSplitDockTree.Key<PerspectiveDockable> root = null;
            if (layout.getRoot() != null) {
                root = handleEntry(layout.getRoot(), tree, children);
            }
            if (root != null) {
     ...
     */
    private <D> SplitDockTree.Key<D> handleEntry(SplitDockStationLayout.Entry entry, SplitDockTree<D> tree,
                                                 Map<Integer, D> children) {
     ...
     * @return the key or <code>null</code>
     */
    private <D> SplitDockTree.Key<D> handleLeaf(SplitDockStationLayout.Leaf leaf, SplitDockTree<D> tree, Map<Integer,
            D> children) {
     ...
     * @return the key or <code>null</code>
     */
    private <D> SplitDockTree.Key<D> handleNode(SplitDockStationLayout.Node node, SplitDockTree<D> tree, Map<Integer,
            D> children) {
        SplitDockTree.Key<D> a = handleEntry(node.getChildA(), tree, children);
        SplitDockTree.Key<D> b = handleEntry(node.getChildB(), tree, children);
        ...
        return switch (node.getOrientation()) {
            case HORIZONTAL ->
                    tree.horizontal(a, b, node.getDivider(), node.getPlaceholders(), node.getPlaceholderMap(),
                            node.getNodeId());
            case VERTICAL -> tree.vertical(a, b, node.getDivider(), node.getPlaceholders(), node.getPlaceholderMap(),
                    node.getNodeId());
        };
     ...
     * @throws IOException if an I/O-error occurs
     */
    private SplitDockStationLayout.Entry readEntry(DataInputStream in, boolean version8, boolean version8a,
                                                   PlaceholderStrategy strategy) throws IOException {
        long id = -1;
        if (version8) {
            id = in.readLong();
        }
        if (version8a) {
            ...
            if (node) {
                ...
                return new SplitDockStationLayout.Node(orientation, divider, childA, childB, placeholders,
                        placeholderMap, id);
            } else {
    ...
    private Path[] readPlaceholders(DataInputStream in, PlaceholderStrategy placeholders) throws IOException {
        ...
        List<Path> result = new ArrayList<>(length);
    ...
    private SplitDockStationLayout.Entry readEntry(XElement element, PlaceholderStrategy strategy) {
        ...
        if (xplaceholders != null) {
            ...
            if (xchildren.length > 0) {
                List<Path> collection = new ArrayList<>(xchildren.length);
                for (XElement xchild : xchildren) {
                    Path placeholder = new Path(xchild.getString());
                    if (strategy == null || strategy.isValidPlaceholder(placeholder)) {
                        collection.add(placeholder);
                    }
                }
                placeholders = collection.toArray(new Path[0]);
            }
        }
