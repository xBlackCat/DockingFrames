...
public class MagnetController {
     ...
     */
    private final PropertyValue<MagnetStrategy> strategy = new PropertyValue<>(ScreenDockStation.MAGNET_STRATEGY) {
        @Override
     ...
     */
    private final PropertyValue<AttractorStrategy> attraction =
            new SilentPropertyValue<>(ScreenDockStation.ATTRACTOR_STRATEGY);
    ...
    /**
     * Gets an array containing all the {@link ScreenDockWindow}s that are currently shown by the
     * {@link #getStation() station}.
     *
    ...
    /**
     * Gets all the {@link ScreenDockWindow}s of the {@link #getStation() station} that are attracted to
     * <code>window</code>.
     *
    ...
    public ScreenDockWindow[] getAttracted(ScreenDockWindow window) {
        List<ScreenDockWindow> result = new ArrayList<>();
        int count = station.getDockableCount();
        for (int i = 0; i < count; i++) {
            ...
            if (next != window) {
                ...
                switch (attraction) {
                    case STRONGLY_ATTRACTED, ATTRACTED -> result.add(next);
                }
            }
        }
        return result.toArray(new ScreenDockWindow[0]);
    }

    /**
     * Calculates the distance between <code>sideA</code> of <code>windowA</code> to <code>sideB</code> of
     * <code>windowB</code>.
     * If either window is the {@link #getCurrent() current} window, then its {@link MagnetRequest#getBounds() base
     * boundaries}
     ...
     */
    public int distance(ScreenDockWindow windowA, MagnetRequest.Side sideA, ScreenDockWindow windowB,
                        MagnetRequest.Side sideB, boolean initialBoundaries) {
        if (sideA != sideB) {
            if ((sideA == Side.NORTH || sideA == Side.SOUTH) != (sideB == Side.NORTH || sideB == Side.SOUTH)) {
                throw new IllegalArgumentException(
                        "sideA and sideB are neither equal nor opposite: " + sideA + ", " + sideB);
            }
        }
    ...
    /**
     * Tells whether the <code>y</code> coordinate and the <code>height</code> of <code>windowA</code> and
     * <code>windowB</code>
    ...
    /**
     * Tells whether the <code>x</code> coordinate and the <code>width</code> of <code>windowA</code> and
     * <code>windowB</code>
    ...
    public int getValue(Rectangle rectangle, Side side) {
        return switch (side) {
            case NORTH -> rectangle.y;
            case SOUTH -> rectangle.y + rectangle.height - 1;
            case WEST -> rectangle.x;
            case EAST -> rectangle.x + rectangle.width - 1;
    ...
    private class Operation implements MagnetizedOperation, MagnetRequest {
         ...
         */
        private final ScreenDockWindow window;

        /**
         * the boundaries any {@link ScreenDockWindow} had before the operation started
         */
        private final Map<ScreenDockWindow, Rectangle> initialBoundaries = new HashMap<>();
        ...
        public void resizingAttraction(ScreenDockWindow neighbor, Side windowSide, Side neighborSide) {
            ...
            if (windowSide != neighborSide) {
                switch (windowSide) {
                    case NORTH, WEST -> delta += 1;
                    case EAST, SOUTH -> delta -= 1;
                }
            }
            ...
            switch (windowSide) {
                ...
                case SOUTH -> resultBoundaries.height += delta;
                case WEST -> {
                    resultBoundaries.x += delta;
                    resultBoundaries.width -= delta;
                }
                case EAST -> resultBoundaries.width += delta;
            }
        }
        ...
        public void movingAttraction(ScreenDockWindow neighbor, Side windowSide, Side neighborSide) {
            ...
            switch (windowSide) {
                case NORTH, SOUTH -> resultBoundaries.y += delta;
                case EAST, WEST -> resultBoundaries.x += delta;
            }
        }
                ...
                switch (windowSide) {
                    case EAST:
                        if (neighborSide != Side.WEST) {
                            throw new IllegalArgumentException(
                                    "windowSide and neighborSide not the same and not opposing: " + windowSide +
                                            " vs. " + neighborSide);
                        }
                        break;
                    case WEST:
