...
public class ButtonContent {
    ...
    /**
     * Only the icon and the title text are painted. Please note that this constant only remains for backwards
     * compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent ICON_AND_TEXT_ONLY = new ButtonContent(FALSE, TRUE, TRUE, FALSE, FALSE, FALSE);
    /**
     * The icon, or if not present the title text, is painted. Please note that this constant only remains for
     * backwards compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent ICON_THEN_TEXT_ONLY =
            new ButtonContent(FALSE, TRUE, NOT_IF_ICON, FALSE, FALSE, FALSE);
    /**
     * The title text, or if not present the icon, is painted. Please note that this constant only remains for
     * backwards compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent TEXT_THEN_ICON_ONLY =
            new ButtonContent(FALSE, NOT_IF_TEXT, TRUE, FALSE, FALSE, FALSE);
    /**
     * Only the icon and the actions are painted. Please note that this constant only remains for backwards
     * compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent ICON_ACTIONS = new ButtonContent(FALSE, TRUE, FALSE, FALSE, TRUE, FALSE);
    /**
     * Only the title text and the actions are painted. Please note that this constant only remains for backwards
     * compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent TEXT_ACTIONS = new ButtonContent(FALSE, FALSE, TRUE, FALSE, TRUE, FALSE);
    /**
     * Icon, title text and actions are painted. Please note that this constant only remains for backwards
     * compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent ICON_AND_TEXT_ACTIONS = new ButtonContent(FALSE, TRUE, TRUE, FALSE, TRUE, FALSE);
    /**
     * The icon, or if not present the title text, and the actions are painted. Please note that this constant only
     * remains for backwards compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent ICON_THEN_TEXT_ACTIONS =
            new ButtonContent(FALSE, TRUE, NOT_IF_ICON, FALSE, TRUE, FALSE);
    /**
     * The title text, or if not present the icon, and the actions are painted. Please note that this constant only
     * remains for backwards compatibility,
     * creating a new {@link ButtonContent} would have the exact same effect.
     */
    public static final ButtonContent TEXT_THEN_ICON_ACTIONS =
            new ButtonContent(FALSE, NOT_IF_TEXT, TRUE, FALSE, TRUE, FALSE);

    private final ButtonContentCondition knob;
    private final ButtonContentCondition icon;
    private final ButtonContentCondition text;
    private final ButtonContentCondition actions;
    private final ButtonContentCondition filterActions;
    private final ButtonContentCondition children;

    private final Map<Dockable, List<ButtonContentListener>> listeners = new HashMap<>();

    /**
     * Creates a new set of properties. All arguments can have a value of <code>null</code>, in which case they default
     * to {@link #THEME}
     *
     * @param knob          whether to paint a "knob" where the user can grab the title and move around. A knob really
     *                        is only
     ...
     *                      open the station and focus one of its children with on click. If the represented
     *                      {@link Dockable} is not a
     ...
     * @param filterActions whether only important {@link DockAction}s, as defined by {@link ButtonContentFilter},
     *                         should be shown
     */
    public ButtonContent(ButtonContentCondition knob, ButtonContentCondition icon, ButtonContentCondition text,
                         ButtonContentCondition children, ButtonContentCondition actions,
    ...
    protected ButtonContentListener[] listeners(Dockable dockable) {
        List<ButtonContentListener> list = listeners.get(dockable);
        if (list != null) {
            return list.toArray(new ButtonContentListener[0]);
        }
