...
public abstract class AbstractTabPane<T extends Tab, M extends TabMenu, I extends LonelyTabPaneComponent> implements TabPane {
     ...
     */
    private final PropertyValue<TabLayoutManager> layoutManager =
            new PropertyValue<>(TabPane.LAYOUT_MANAGER) {
                @Override
     ...
     */
    private final Map<Dockable, T> tabs = new HashMap<>();

    /**
     * all the menus, visible and invisible
     */
    private final List<M> menus = new ArrayList<>();

    /**
     * tells for each {@link Dockable} on which menu it is
     */
    private final Map<Dockable, M> menuPosition = new HashMap<>();

    /**
     * a list of listeners to be informed when the content or the selection changes
     */
    private final List<TabPaneListener> listeners = new ArrayList<>();
    ...
    protected TabPaneListener[] listeners() {
        return listeners.toArray(new TabPaneListener[0]);
    }
    ...
    public TabMenu[] getMenus() {
        return menus.toArray(new TabMenu[0]);
    }
