...
public abstract class PlaceholderList<D, S, P extends PlaceholderListItem<D>> {
     ...
     * view on all items
     */
    private final SubList<Item> allItems = new SubList<Item>(Level.BASE) {
        @Override
     ...
     */
    private final SubList<Set<Path>> allPlaceholders = new SubList<Set<Path>>(Level.BASE) {
        @Override
     ...
     */
    private final SubList<Set<Path>> purePlaceholders = new SubList<Set<Path>>(Level.PLACEHOLDER) {
        @Override
        protected Item wrap(Set<Path> object) {
            return new Item(object);
        }
     ...
     */
    private final SubList<P> dockables = new SubList<P>(Level.DOCKABLE) {
        @Override
     ...
     */
    private final PlaceholderStrategyListener listener = this::removeAll;
    ...
    protected void read(PlaceholderMap map, PlaceholderListItemConverter<D, P> converter, boolean simulate) {
        ...
        for (Key placeholder : placeholders) {
            Set<Path> paths = null;
            if (!simulate) {
                Path[] list = placeholder.getPlaceholders();

                if (list.length > 0) {
                    paths = new HashSet<>(Arrays.asList(list));
                }
            }

            P dockable = null;

            if (map.contains(placeholder, "convert")) {
                ConvertedPlaceholderListItem converted = new ConvertedPlaceholderListItem();

                Object[] keys = map.getArray(placeholder, "convert-keys");

                for (Object convertKey : keys) {
                    String metaKey = (String) convertKey;
                    converted.put(metaKey, map.get(placeholder, "dock." + metaKey));
                }

                if (map.contains(placeholder, "map")) {
                    converted.setPlaceholderMap(map.getMap(placeholder, "map"));
                }
            ...
            if (!simulate) {
                ...
                if (item != null) {
                    ...
                    if (map.contains(placeholder, "item")) {
                        Object[] keys = map.getArray(placeholder, "item-keys");

                        for (Object itemKey : keys) {
                            String key = (String) itemKey;
                            item.put(key, map.get(placeholder, "item." + key));
                        }
                    }
    ...
    public PlaceholderMap toMap(PlaceholderListItemConverter<?, ? super P> converter) {
        ...
        for (Item entry : list()) {
            ...
            placeholderSet = new HashSet<>(placeholderSet);
            ...
            Path[] placeholders = placeholderSet.toArray(new Path[0]);
        ...
        if (strategy != null) {
            ...
            while (iter.hasNext()) {
                ...
                if (placeholders != null) {
                    placeholders.removeIf(path -> !strategy.isValidPlaceholder(path));
                }
    ...
    /**
     * Adds <code>placeholder</code> at the location of <code>dockable</code>. This method will remove
     * <code>placeholder</code> from all
     * other locations.
     *
     ...
     * @throws IllegalArgumentException if either argument is <code>null</code>, or if the location of
     *                                  <code>dockable</code> cannot be found
     */
    ...
    public int size(Level level) {
        return switch (level) {
            case BASE -> list().size();
            case DOCKABLE -> dockables().size();
            case PLACEHOLDER -> purePlaceholders().size();
    ...
    private Entry head(Level level) {
        return switch (level) {
            case BASE -> head;
            case PLACEHOLDER -> headPlaceholder;
            case DOCKABLE -> headDockable;
        };
    }
    ...
    private class Entry {
        ...
        public Entry next(Level level) {
            return switch (level) {
                case BASE -> next;
                case PLACEHOLDER -> item.isPlaceholder() ? nextLevel : null;
                case DOCKABLE -> item.isPlaceholder() ? null : nextLevel;
            };
        }
        ...
        public Entry previous(Level level) {
            return switch (level) {
                case BASE -> previous;
                case PLACEHOLDER -> item.isPlaceholder() ? previousLevel : null;
                case DOCKABLE -> item.isPlaceholder() ? null : previousLevel;
            };
        }
    ...
    public interface Filter<M> extends Iterable<M> {
         ...
         * <ul>
         * <li>If <code>sourceList</code> is <code>this</code>, then calling this method is equivalent of calling
         * {@link #move(int, int)}.</li>
         * <li>If <code>sourceList</code> is an inner object from the same {@link PlaceholderList}, then calling
         * {@link #move(int, int)} of {@link PlaceholderList#list()} with adapted arguments is equivalent of calling
         * this method. </li>
         * <li>Otherwise the item will be removed from the <code>sourceList</code>, and a copy will be inserted into
         * this list.</li>
         * </ul>
         * This method has to assume that <code>sourceList</code> is of the same type as <code>this</code>. This is
         * <code>true</code> for
         * all {@link Filter}s that were created by a {@link PlaceholderList}, but if clients implement the interface
         ...
         *                                  not recognized by this class, or if any of <code>sourceIndex</code> of
         *                                  <code>destination</code> is invalid
         */
        void move(Filter<M> sourceList, int sourceIndex, int destination);
    }
        ...
        public void move(Filter<A> sourceList, int sourceIndex, int destination) {
            ...
            if (!(sourceList instanceof SubList)) {
                throw new IllegalArgumentException(
                        "The type " + sourceList.getClass().getName() + " is not recognized");
            }
