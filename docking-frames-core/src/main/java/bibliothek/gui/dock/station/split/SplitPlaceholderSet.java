...
public class SplitPlaceholderSet {
     ...
     */
    private final SplitDockAccess access;
     ...
     * @param protectedNodes nodes that will not be removed even if they are no longer {@link SplitNode#isOfUse()
     * useful}
     */
    ...
    public void removeDoublePlaceholders(final SplitNode node, final PlaceholderMap map) {
        final Set<Path> placeholdersToRemove = new HashSet<>();
        ...
        root.visit(new SplitNodeVisitor() {
            ...
            private void handle(SplitNode check) {
                if (check != node) {
                    placeholdersToRemove.addAll(Arrays.asList(check.getPlaceholders()));
                }
            }
