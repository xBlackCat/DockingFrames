...
public class TabComponentLayoutManager implements LayoutManager {
    ...
    private final OrientedLabel label;
    private final ButtonPanel actions;
    ...
    public void setOrientation(TabPlacement orientation) {
        ...
        if (this.orientation != orientation) {
            ...
            switch (orientation) {
                case BOTTOM_OF_DOCKABLE -> actions.setOrientation(DockTitle.Orientation.SOUTH_SIDED);
                case LEFT_OF_DOCKABLE -> actions.setOrientation(DockTitle.Orientation.EAST_SIDED);
                case RIGHT_OF_DOCKABLE -> actions.setOrientation(DockTitle.Orientation.WEST_SIDED);
                case TOP_OF_DOCKABLE -> actions.setOrientation(DockTitle.Orientation.NORTH_SIDED);
            }
        }
    }
    ...
    public Dimension preferredLayoutSize(Container parent) {
        ...
        if (orientation.isHorizontal()) {
            result = new Dimension(
                    ...
                    size.height + freeSpaceToOpenSide + freeSpaceToParallelBorder + labelInsets.top +
                            labelInsets.bottom);
            ...
            if (actions.hasActions()) {
                ...
                result.height = Math.max(result.height,
                        size.height + freeSpaceToOpenSide + freeSpaceToParallelBorder + actionInsets.top +
                                actionInsets.bottom);
            } else {
                result.width += actionInsets.right;
                result.height = Math.max(result.height,
                        size.height + freeSpaceToOpenSide + freeSpaceToParallelBorder + actionInsets.bottom);
            }
        } else {
            result = new Dimension(
            ...
            if (actions.hasActions()) {
                ...
                result.width = Math.max(result.width,
                        size.width + freeSpaceToOpenSide + freeSpaceToParallelBorder + actionInsets.left +
                                actionInsets.right);
            } else {
                result.height += actionInsets.bottom;
                result.width = Math.max(result.width,
                        size.width + freeSpaceToOpenSide + freeSpaceToParallelBorder + actionInsets.right);
            }
        }
        return result;
    }
        ...
        switch (orientation) {
            case TOP_OF_DOCKABLE -> {
                label.setBounds(
                        ...
                        labelSize(parent, width - 2 * freeSpaceToSideBorder - actionsSize.width - labelInsets.left -
                                labelInsets.right, freeSpaceToSideBorder + labelInsets.left, showActions),
                        height - freeSpaceToOpenSide - freeSpaceToParallelBorder - labelInsets.top -
                                labelInsets.bottom);
                if (showActions) {
                    int actionsHeight =
                            Math.min(actionsSize.height, height - freeSpaceToOpenSide - freeSpaceToParallelBorder);
            ...
            case RIGHT_OF_DOCKABLE -> {
                label.setBounds(
                        ...
                        labelSize(parent, height - 2 * freeSpaceToSideBorder - actionsSize.height - labelInsets.top -
                                labelInsets.bottom, freeSpaceToSideBorder + labelInsets.top, showActions));
                if (showActions) {
                    int actionsWidth =
                            Math.min(actionsSize.width, width - freeSpaceToOpenSide - freeSpaceToParallelBorder);
                    int delta =
                            width - freeSpaceToOpenSide - freeSpaceToParallelBorder - actionsWidth - actionInsets.left -
                                    actionInsets.right;
    ...
    private int labelSize(Container parent, int suggested, int start, boolean showActions) {
        ...
        if (configuration.isKeepLabelBig() && label.getIcon() != null) {
            if (orientation.isHorizontal()) {
                return Math.min(Math.max(suggested, label.getIconOffset() + label.getIcon().getIconWidth()),
                        parent.getWidth() - start);
            } else {
                return Math.min(Math.max(suggested, label.getIconOffset() + label.getIcon().getIconHeight()),
                        parent.getHeight() - start);
            }
        }
        return suggested;
    }
    ...
    protected boolean shouldShowActions(Container parent) {
        ...
        if (orientation.isHorizontal()) {
            ...
            int labelDelta =
                    freeSpaceToSideBorder + freeSpaceBetweenLabelAndActions + freeSpaceToOpenSide + labelInsets.left +
                            labelInsets.right + actionInsets.left + actionInsets.right;
            ...
            int labelDelta =
                    freeSpaceToSideBorder + freeSpaceBetweenLabelAndActions + freeSpaceToOpenSide + labelInsets.top +
                            labelInsets.bottom + actionInsets.top + actionInsets.bottom;
            switch (getConfiguration().getActionHiding()) {
                case NEVER:
                    return true;
                case NO_SPACE_LEFT:
    ...
    protected boolean shouldShowIcon() {
        ...
        if (orientation.isHorizontal()) {
            return switch (getConfiguration().getIconHiding()) {
                case NEVER -> true;
                case NO_SPACE_LEFT -> label.getWidth() >= label.getIconOffset() + label.getIcon().getIconWidth();
                case TEXT_DISAPPEARING -> label.getWidth() >= label.getPreferredSize().getWidth();
            };
        }
        
        return switch (getConfiguration().getIconHiding()) {
            case NEVER -> true;
            case NO_SPACE_LEFT -> label.getHeight() >= label.getIconOffset() + label.getIcon().getIconHeight();
            case TEXT_DISAPPEARING -> label.getHeight() >= label.getPreferredSize().getHeight();
        };
    }
