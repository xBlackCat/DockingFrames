 ...
 */
public abstract class CombinedStackDockComponent<T extends CombinedTab, M extends CombinedMenu,
        I extends CombinedInfoComponent> extends AbstractTabPane<T, M, I> implements StackDockComponent {
    /**
     * The panel which shows the children
     */
    private final CombinedStackDockContentPane panel;

    /**
     * A list of all {@link Component Components} which are shown on this {@link #componentPanel}
     */
    private final Map<Dockable, Meta> components = new HashMap<>();

    /**
     * The current configuration of the tabs
     */
    private final PropertyValue<TabConfigurations> tabConfiguration =
            new PropertyValue<>(StackDockStation.TAB_CONFIGURATIONS) {
                @Override
     ...
     */
    private final BackgroundAlgorithm background;

    /**
     * The panel which displays one of the children of this pane
     */
    private final BackgroundPanel componentPanel = new ConfiguredBackgroundPanel(null, Transparency.DEFAULT) {
        @Override
     ...
     */
    private final List<StackDockComponentListener> listeners = new ArrayList<>();

    /**
     * Handles visibility of tabs
     */
    private final CombinedHandler<CombinedTab> tabHandler = new CombinedHandler<>() {
     ...
     */
    private final CombinedHandler<CombinedMenu> menuHandler = new CombinedHandler<>() {
     ...
     */
    private final CombinedHandler<AbstractTabPaneComponent> infoHandler = new CombinedHandler<>() {
     ...
     */
    private final TabDisablingStrategyObserver tabDisabling = new TabDisablingStrategyObserver() {
        @Override
    ...
    public CombinedStackDockComponent() {
        ...
        addTabPaneListener(new TabPaneListener() {
            ...
            public void selectionChanged(TabPane pane) {
                for (StackDockComponentListener listener : listeners.toArray(new StackDockComponentListener[0])) {
                    listener.selectionChanged(CombinedStackDockComponent.this);
                }
            }

            public void infoComponentChanged(TabPane pane, LonelyTabPaneComponent oldInfo,
    ...
    public Rectangle getAvailableArea() {
        ...
        return new Rectangle(insets.left, insets.top, Math.max(1,
                panel.getWidth() - insets.left - insets.right), Math.max(1, panel.getHeight()
    ...
    public void setTitleAt(int index, String newTitle) {
        ...
        meta.text = Objects.requireNonNullElse(newTitle, "");
        meta.forward();
    }
