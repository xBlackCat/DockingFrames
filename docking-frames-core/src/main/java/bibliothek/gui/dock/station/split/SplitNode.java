...
public abstract class SplitNode {
    ...
    protected void split(SplitDockPathProperty property, int depth, SplitNode newChild, long newNodeId) {
        ...
        if (node.location() == SplitDockPathProperty.Location.LEFT ||
                node.location() == SplitDockPathProperty.Location.RIGHT) {
            orientation = SplitDockStation.Orientation.HORIZONTAL;
        } else {
            orientation = SplitDockStation.Orientation.VERTICAL;
        }

        boolean reverse = node.location() == SplitDockPathProperty.Location.RIGHT ||
                node.location() == SplitDockPathProperty.Location.BOTTOM;

        SplitDockPathProperty.Node lastNode = property.getLastNode();

        if (lastNode != null) {
            newNodeId = lastNode.id();
        }
        ...
        if (reverse) {
            ...
            split.setDivider(1 - node.size());
        } else {
            split = createNode(newNodeId);
            split.setLeft(newChild);
            split.setRight(this);
            split.setOrientation(orientation);
            split.setDivider(node.size());
        }
    ...
    /**
     * Schedules a call to {@link #ensureIdUnique()} of the {@link Root} node. If this method is not called within
     * the EDT,
     ...
     * This method is recursive, it will call
     * {@link #updateBounds(double, double, double, double, double, double, boolean) updateBounds} on
     ...
     * call {@link #setBounds(double, double, double, double, double, double, boolean) getBounds} on the children of
     * this node.
     *
     * @param x                     the relative x-coordinate
     * @param y                     the relative y-coordinate
     ...
     */
    public void setBounds(double x, double y, double width, double height, double factorW, double factorH,
                          boolean updateComponentBounds) {
        this.x = x;
        this.y = y;
        this.width = width;
     ...
     * from the station. They must be removed explicitly using
     * {@link Leaf#setDockable(Dockable, bibliothek.gui.dock.DockHierarchyLock.Token)}
     *
     * @param key           the key to read
     * @param linksToSet    a map that is to be filled with all new {@link Leaf}s and their {@link Dockable}s which
     *                         are not yet set.
     * @param checkValidity whether to ensure that all new {@link Dockable}s are
     *                      acceptable or not.
     */
    public abstract void evolve(SplitDockTree.Key<Dockable> key, boolean checkValidity, Map<Leaf, Dockable> linksToSet);
     ...
     * @param linksToSet    a map that will be filled up with new {@link Leaf}s whose {@link Dockable}s have not yet
     *                         been set
     * @return the new node
     */
    protected SplitNode create(SplitDockTree.Key<Dockable> key, boolean checkValidity, Map<Leaf, Dockable> linksToSet) {
            ...
            if (dockables == null || dockables.length == 0) {
                ...
                if (dockables.length == 1) {
                        ...
                        if (acceptance != null) {
                            if (!acceptance.accept(split, dockables[0])) {
                                throw new SplitDropTreeException(split,
                                        "DockAcceptance does not allow child " + dockables[0]);
                            }
                        }
                    }
                    ...
                    if (checkValidity) {
                        ...
                        if (acceptance != null) {
                            if (!acceptance.accept(split, dockables[0], dockables[1])) {
                                throw new SplitDropTreeException(split,
                                        "DockAcceptance does not allow to combine " + dockables[0] + " and " +
                                                dockables[1]);
                            }
                        }
                    }
                    ...
                    CombinerSource source =
                            new NodeCombinerSource(dockables[0], dockables[1], key.getTree().getPlaceholderMap(key));
                    ...
                    if (dockables.length == 2) {
                        ...
                        DockStation station = combination.asDockStation();
                        if (station == null) {
                            throw new SplitDropTreeException(access.getOwner(), "Combination of two Dockables does " +
                                    "not create a new station");
                        }
                        ...
                        for (int i = 2; i < dockables.length; i++) {
                            ...
                            if (checkValidity) {
                                if (!dockable.accept(station) || !station.accept(dockable)) {
                                    throw new SplitDropTreeException(access.getOwner(),
                                            "No acceptance of " + dockable + " and " + station);
                                }

                                if (acceptance != null) {
                                    if (!acceptance.accept(station, dockable)) {
                                        throw new SplitDropTreeException(split,
                                                "DockAcceptance does not allow " + dockable + " as child of " +
                                                        station);
                                    }
                                }
                            }
    ...
    private class NodeCombinerSource implements CombinerSource {
        private final Dockable child;
        private final Dockable dropping;
