...
public class DefaultScreenDockWindowBorder implements ScreenDockWindowBorder {
    ...
    public static final ScreenDockWindowBorderFactory FACTORY =
            DefaultScreenDockWindowBorder::new;
    ...
    public Insets getBorderInsets(Component c) {
        if (isResizeable()) {
            ...
            return switch (titleLocation) {
                case BOTTOM -> new Insets(0, 0, 5, 0);
                case TOP -> new Insets(5, 0, 0, 0);
                case LEFT -> new Insets(0, 5, 0, 0);
                case RIGHT -> new Insets(0, 0, 0, 5);
                default -> throw new IllegalStateException("unknown location: " + titleLocation);
            };
        } else {
    ...
    private class PaintParameters {
        ...
        public PaintParameters(int width, int height, Component c) {
            ...
            corner =
                    Math.max(cornerSize, Math.max(Math.max(insets.top, insets.bottom), Math.max(insets.left,
                            insets.right)));
        }
    }
