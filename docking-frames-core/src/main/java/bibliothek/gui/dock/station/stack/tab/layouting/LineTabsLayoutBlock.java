...
public class LineTabsLayoutBlock extends AbstractTabsLayoutBlock {
    ...
    public LineSize[] getSizes() {
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            result[i] =
                    new LineSize(Size.Type.MINIMUM, size, selection, i + 1 == tabs.length, i / (double) tabs.length);
        }
    ...
    protected Tab[] getCurrentTabs() {
        ...
        Arrays.sort(tabs, Comparator.comparingInt(TabPaneComponent::getZOrder));
        return tabs;
    }
    ...
    public void doLayout() {
        ...
        if (sumPreferred <= bounds.width) {
            doLayoutPreferred(conversion, bounds.width, bounds.height, preferreds, tabs, zorder, overlapPrevious,
					overlapNext);
        } else if (sumMinimum <= bounds.width) {
            doLayoutMinimum(conversion, bounds.width, bounds.height, minimums, preferreds, tabs, zorder,
					overlapPrevious, overlapNext);
        } else {
            doLayoutShrinked(conversion, bounds.width, bounds.height, minimums, tabs, zorder, overlapPrevious,
					overlapNext);
        }
    ...
    private void doLayoutPreferred(AxisConversion conversion, int width, int height, Dimension[] preferreds,
								   Tab[] tabs, ZOrder order, int[] overlapPrevious, int[] overlapNext) {
        int x = 0;

        for (int i = 0; i < tabs.length; i++) {
            Dimension size = preferreds[i];

            tabs[i].setBounds(conversion.modelToView(new Rectangle(x, 0, size.width,
                    sameSize ? height : Math.min(height, size.height))));
            x += size.width;
    ...
    private void doLayoutMinimum(AxisConversion conversion, int width, int height, Dimension[] minimums,
								 Dimension[] preferreds, Tab[] tabs, ZOrder order, int[] overlapPrevious,
								 int[] overlapNext) {
        int x = 0;
        ...
        for (int i = 0; i < tabs.length - 1; i++) {
            ...
            tabs[i].setBounds(conversion.modelToView(new Rectangle(x, 0, tabWidth,
                    sameSize ? height : Math.min(height, preferreds[i].height))));
        ...
        if (tabs.length > 0) {
            ...
            tabs[last].setBounds(conversion.modelToView(new Rectangle(x, 0, tabWidth,
                    sameSize ? height : Math.min(height, preferreds[last].height))));
        }
    }

    private void doLayoutShrinked(AxisConversion conversion, int width, int height, Dimension[] minimums, Tab[] tabs,
								  ZOrder order, int[] overlapPrevious, int[] overlapNext) {
        int x = 0;
        ...
        for (int i = 0; i < tabs.length - 1; i++) {
            ...
            tabs[i].setBounds(conversion.modelToView(new Rectangle(x, 0, tabWidth,
                    sameSize ? height : Math.min(height, minimums[i].height))));
            x += tabWidth;
        ...
        int last = tabs.length - 1;
        if (last >= 0) {
            tabs[last].setBounds(conversion.modelToView(new Rectangle(x, 0,
                    width - x, sameSize ? height : Math.min(height, minimums[last].height))));
        }
    }
     ...
     * @author Benjamin Sigg
     */
    public static class LineSize extends TabsSize {
        private final boolean allTabs;
     ...
     */
    protected static class ZOrder {
        private final Tab[] tabs;

        private final List<Integer>[] onTop;
        private final List<Integer>[] onBottom;
        ...
        public ZOrder(Tab[] tabs) {
            ...
            for (int i = 0; i < tabs.length; i++) {
                onTop[i] = new ArrayList<>(5);
                onBottom[i] = new ArrayList<>(5);
            }
        }
    ...
    protected class SizeCollector {
        private final Dockable[] dockables;
        private final Tab[] tabs;

        private final Dimension[] minimum;
        private final Dimension[] preferred;
        private final int[] overlapPrevious;
        private final int[] overlapNext;
