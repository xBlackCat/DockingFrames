...
public class InternalFullscreenStrategy implements ScreenDockFullscreenStrategy {
                        ...
                        try {
                            for (Map.Entry<ScreenDockStation, Dockable[]> entry : dockables.entrySet()) {
                                ...
                                for (Dockable dockable : entry.getValue()) {
                                    ...
                                    window.setWindowBounds(new Rectangle(0, 0, desktop.getWidth(),
                                            desktop.getHeight()));
                                }
     ...
     */
    private final Map<ScreenDockStation, Dockable[]> dockables = new HashMap<>();
     ...
     * a listener added to any station
     */
    private final ScreenDockStationListener screenListener = new ScreenDockStationListener() {
    ...
    public boolean isFullscreen(ScreenDockWindow window) {
        ...
        return bounds.x <= 0 && bounds.y <= 0 && bounds.width + bounds.x >= desktop.getWidth() &&
                bounds.height + bounds.y >= desktop.getHeight();
    }
