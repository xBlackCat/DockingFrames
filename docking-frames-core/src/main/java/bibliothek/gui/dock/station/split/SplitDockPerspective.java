...
public class SplitDockPerspective implements PerspectiveDockable, PerspectiveStation {
     ...
     */
    private final List<PerspectiveDockable> children = new ArrayList<>();
    ...
    protected EntryListener[] listeners() {
        return listeners.toArray(new EntryListener[0]);
    }

    /**
     * Calls {@link EntryListener#removed(SplitDockPerspective.Entry, SplitDockPerspective.Entry)} on all listeners
     * that are currently
    ...
    /**
     * Calls {@link EntryListener#added(SplitDockPerspective.Entry, SplitDockPerspective.Entry)} on all listeners that
     * are currently
     * known to this perspective
     *
    ...
    private Entry convert(PerspectiveSplitDockTree.Key<PerspectiveDockable> key) {
        if (key == null) {
            return null;
        }
        ...
        if (tree.isNode(key)) {
            ...
            return new Node(orientation, tree.getDivider(key), childA, childB, toSet(tree.getPlaceholders(key)),
                    tree.getPlaceholderMap(key), key.getNodeId());
        }
    ...
    private Set<Path> toSet(Path[] placeholders) {
        ...
        if (placeholders != null) {
            result.addAll(Arrays.asList(placeholders));
        }
        return result;
    }
    ...
    public interface EntryListener {
         ...
         */
        void added(Entry parent, Entry child);
         ...
         * @param child  the child, either a {@link Node} or a {@link Leaf}
         */
        void removed(Entry parent, Entry child);
         ...
         */
        void modified(Leaf leaf, PerspectiveDockable oldDockable, PerspectiveDockable newDockable);
    }
    ...
    public static abstract class Entry {
         ...
         */
        private final Set<Path> placeholders;
        /**
    ...
    public static class Node extends Entry {
         ...
         */
        public Node(Orientation orientation, double divider, Entry childA, Entry childB, Set<Path> placeholders,
    ...
    public static class Leaf extends Entry {
        ...
        public void remove() {
            SplitDockPerspective perspective = getPerspective();
            if (perspective == null) {
                throw new IllegalStateException("this method can only be applied to leafs that have a root as " +
                        "anchestor");
            }
