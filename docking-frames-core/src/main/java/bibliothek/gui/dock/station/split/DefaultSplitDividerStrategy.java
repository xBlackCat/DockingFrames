...
public class DefaultSplitDividerStrategy implements SplitDividerStrategy {
    private final Map<SplitDockStation, Handler> handlers = new HashMap<>();
    ...
    public static class Handler extends MouseAdapter implements MouseListener, MouseMotionListener, AWTEventListener,
        ...
        private final PropertyValue<Boolean> restricted = new PropertyValue<>(DockController.RESTRICTED_ENVIRONMENT) {
            @Override
        ...
        /**
         * Will be set to true when mouse is over divider, and set to false when exited. (see AWTListener method below
         * for more details).
         */
        ...
        private void updateEventListener() {
            ...
            if (expected != awtListenerEnabled) {
                ...
                if (expected) {
                    // if this goes wrong, the offending client rightly gets an exception. It's his fault because he
                    // did set the "restricted environment" property wrong.
                    Toolkit.getDefaultToolkit().addAWTEventListener(Handler.this,
            ...
            if (event.getID() == MouseEvent.MOUSE_MOVED || event.getID() == MouseEvent.MOUSE_RELEASED) {
                ...
                if (mev.getSource() != Handler.this.container && withinBounds) {
                    if (mev.getSource() instanceof GlassedPane.GlassPane) {
                        ...
                        // mouse is over another component which is not the registered container and the mouse cursor
                        // had not been reseted yet -> reset mouse cursor
                ...
                SwingUtilities.invokeLater(() -> {
                    if (container != null) {
                        ...
                        if (current == null) {
                            ...
                            // check bounds with one pixel delta -> divider needs to be greater than 2 pixels,
                            // because divider bounds will be shrinked by 1 pixel at each side
                            if (bounds.width > 2 && bounds.height > 2) {
                                if (e.x <= bounds.x || e.x >= bounds.x + bounds.width - 1 || e.y <= bounds.y ||
