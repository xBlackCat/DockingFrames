...
public class ButtonPane extends SecureContainer {
     ...
     */
    private final FlapSpanStrategy span;
    ...
    public void setDropInfo(FlapDropInfo dropInfo) {
        if (dropInfo == null) {
            ...
            if (index < station.getDockableCount() && station.getDockable(index) == dropInfo.getDockable()) {
                dropInfo = null;
                span.tease(-1);
            } else if (index - 1 >= 0 && index - 1 < station.getDockableCount() &&
                    station.getDockable(index - 1) == dropInfo.getDockable()) {
                dropInfo = null;
                span.tease(-1);
            } else {
    ...
    protected void paintOverlay(Graphics g) {
        if (dropInfo != null && dropInfo.getCombineTarget() == null) {
            ...
            if (x1 >= x2 || y1 >= y2) {
                ...
                station.getPaint().drawInsertion(g, new Rectangle(0, 0, getWidth(), getHeight()), new Rectangle(x1, y1,
                        x2 - x1, y2 - y1));
            }
        }
    }
