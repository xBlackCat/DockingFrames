...
public class DefaultSplitLayoutManager implements SplitLayoutManager {
    ...
    protected PutInfo calculateSideSnap(SplitDockStation station, int x, int y, Leaf leaf, Dockable drop) {
        ...
        if (leaf != null && station.getRoot().getChild() instanceof Node node) {
            if (node.getLeft().isVisible() && node.getRight().isVisible()) {
                if (info.getPut() == PutInfo.Put.TOP && node.getOrientation() == Orientation.VERTICAL &&
                        node.getLeft() == leaf) {
                    return null;
                }

                if (info.getPut() == PutInfo.Put.BOTTOM && node.getOrientation() == Orientation.VERTICAL &&
                        node.getRight() == leaf) {
                    return null;
                }

                if (info.getPut() == PutInfo.Put.LEFT && node.getOrientation() == Orientation.HORIZONTAL &&
                        node.getLeft() == leaf) {
                    return null;
                }

                if (info.getPut() == PutInfo.Put.RIGHT && node.getOrientation() == Orientation.HORIZONTAL &&
                        node.getRight() == leaf) {
                    return null;
                }
            }
    ...
    public void calculateDivider(SplitDockStation station, PutInfo putInfo, Leaf origin, StationDropItem item) {
        ...
        if (origin != null) {
            if (origin.getParent() instanceof Node originParent) {
    ...
    public double validateDivider(SplitDockStation station, double divider, Node node) {
        ...
        return validateDivider(station, divider, leftMin, rightMin, node.getOrientation(), node.getWidth(),
                node.getHeight());
    }
     ...
     */
    protected double validateDivider(SplitDockStation station, double divider, Dimension minimumLeft,
                                     Dimension minimumRight, Orientation orientation, double width, double height) {
        double factor;
        double size;
    ...
    public PutInfo validatePutInfo(SplitDockStation station, PutInfo putInfo) {
        if (putInfo != null) {
            ...
            if (putInfo.getNode() != null &&
                    (putInfo.getPut() == PutInfo.Put.CENTER || putInfo.getPut() == PutInfo.Put.TITLE)) {
                if (!putInfo.getDockable().accept(station, ((Leaf) putInfo.getNode()).getDockable()) ||
                        ...
                        !station.getController().getAcceptance().accept(station,
                                ((Leaf) putInfo.getNode()).getDockable(), putInfo.getDockable())) {
                    return null;
                }
            } else {
