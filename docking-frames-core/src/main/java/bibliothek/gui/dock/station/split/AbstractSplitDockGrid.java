...
public abstract class AbstractSplitDockGrid<D> {
     ...
     */
    private final List<Node<D>> nodes = new ArrayList<>();
    /**
    ...
    /**
     * Whether to automatically call {@link #unpack(double, double, double, double)} before adding any new
     * {@link Dockable}s
    ...
    /**
     * Tells whether {@link #unpack(double, double, double, double)} is called automatically before adding new
     * {@link Dockable}s
     * to this grid.
     *
     ...
     * of all {@link DockStation}s are removed and re-added as if
     * {@link #addDockable(double, double, double, double, Object...)}
    ...
    public void unpack(double x, double y, double width, double height) {
        ...
        if (node.dockables != null) {
            List<D> copy = new ArrayList<>();
            for (D dockable : node.dockables) {
                copy.addAll(Arrays.asList(unpack(dockable)));
            }
            node.dockables = copy.toArray(array(copy.size()));
        }
    }

    /**
     * Unpacks <code>dockable</code>. Unpacking means converting <code>dockable</code> in something like a
     * {@link DockStation}
     ...
     */
    @SafeVarargs
    public final void addDockable(double x, double y, double width, double height, D... dockables) {
        if (dockables == null) {
    ...
    private Node<D> nodeAt(double x, double y, double width, double height) {
        for (Node<D> existingNode : nodes) {
            if (existingNode.x == x && existingNode.y == y && existingNode.width == width &&
    ...
    protected void fillTree(SplitDockTree<D> tree) {
        Node<D> root = tree();
        if (root != null) {
            SplitDockTree.Key<D> key = root.put(tree);
            tree.root(key);
        }
    }
    ...
    protected Node<D> tree() {
        List<Node<D>> nodes = new ArrayList<>(this.nodes);
    ...
    protected static class Node<D> implements GridNode<D> {
         ...
         */
        public SplitDockTree.Key<D> put(SplitDockTree<D> tree) {
            if (dockables != null || childA == null || childB == null) {
                return tree.put(dockables, selected, placeholders, placeholderMap, -1);
            } else if (horizontal) {
                return tree.horizontal(childA.put(tree), childB.put(tree), divider, placeholders, placeholderMap,
                        -1);
            } else {
                return tree.vertical(childA.put(tree), childB.put(tree), divider, placeholders, placeholderMap,
                        -1);
            }
        }
