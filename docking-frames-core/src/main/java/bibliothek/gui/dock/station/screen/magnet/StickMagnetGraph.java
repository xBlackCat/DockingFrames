...
public class StickMagnetGraph {
     ...
     */
    private final MagnetController controller;

    /**
     * information about the event
     */
    private final MagnetRequest request;
     ...
     */
    private final List<DefaultNode> nodes = new ArrayList<>();
    ...
    protected DefaultEdge[] getEdgesByDistance() {
        DefaultEdge[] edges = this.edges.toArray(new DefaultEdge[0]);
        Arrays.sort(edges, (o1, o2) -> o2.getTarget().getRootDistance() - o1.getTarget().getRootDistance());
        return edges;
    }
     ...
     * and sets the hard flag on <code>side</code> and on {@link Side#opposite()} on all {@link Node#getConstraints()}
     * s it encounters.
     *
    ...
    /**
     * Ensures that if one side of an {@link Edge} is {@link StickMagnetGraphConstraint#isHard(MagnetRequest.Side)
     * hard}, then
     ...
     * Implementations should also pay attention to {@link StickMagnetGraphConstraint#isHard(MagnetRequest.Side)} and
     * not modify
     * hard sides.<br>
    ...
    protected void validateConstraints() {
        ...
        final List<Node> resizedVertically = new ArrayList<>();
        final List<Node> resizedHorizontally = new ArrayList<>();
     ...
     * nodes. All the affected sides are set to
     * {@link StickMagnetGraphConstraint#setHard(MagnetRequest.Side, boolean) hard} to
     * prevent further modifications. Resizing stops at any
     * {@link StickMagnetGraphConstraint#isHard(MagnetRequest.Side) hard} side.
     *
    ...
    private void rippleSide(Node start, Side direction, int leftInitial, int leftAfter, int rightInitial,
                            int rightAfter) {
    ...
    public interface Visitor {
         ...
         */
        boolean beginVisit(Node node, boolean revisit);
    ...
    public interface Node {
         ...
         */
        Side getNeighbor(ScreenDockWindow window);
    ...
    protected class DefaultEdge implements Edge {
        private final DefaultNode source;
        private final DefaultNode target;
    ...
    protected class DefaultNode implements Node {
        ...
        private final StickMagnetGraphConstraint constraints = new StickMagnetGraphConstraint();
        ...
        public DefaultEdge[] getEdges() {
            if (edges == null) {
                ...
                return edges.toArray(new DefaultEdge[0]);
            }
        }
    }
}
