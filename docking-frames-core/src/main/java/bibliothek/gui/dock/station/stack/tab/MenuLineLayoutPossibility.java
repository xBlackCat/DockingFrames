...
public class MenuLineLayoutPossibility {
    private final MenuLineLayoutPane pane;
    private final Size menuSize;
    private final Size infoSize;
    private final Size tabSize;
    ...
    public void apply() {
        ...
        if (menuSize != null) {
            Set<Dockable> dockables = new HashSet<>(Arrays.asList(pane.getDockables()));
        ...
        pane.setSelectedBounds(conversion.modelToView(new Rectangle(available.x,
                available.y + required, available.width, available.height - required)));
                ...
                switch (item) {
                    case INFO -> {
                        ...
                        Rectangle infoBounds =
                                new Rectangle(x + deltaX, available.y + reqDelta / 2, itemWidth, required - reqDelta);
                        x += availableWidth;
     ...
     * @return the widths, where 0 is tabs, 1 is menus and 2 is info components. The sum of the widths equal to
     * <code>available</code>
     */
    private int[] calculateWidths(TabPlacement orientation, MenuLineLayoutOrder order, int available, Size tabSize,
                                  Size menuSize, Size infoSize) {
