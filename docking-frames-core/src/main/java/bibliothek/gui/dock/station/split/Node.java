...
public class Node extends VisibleSplitNode implements Divideable {
     ...
     */
    private final Rectangle dividerBounds = new Rectangle();
    ...
    private Dimension getSize(Dimension left, Dimension right) {
        if (left != null && right != null) {
            ...
            return Objects.requireNonNullElseGet(right, Dimension::new);
        }
    }
    ...
    @Override
    public void evolve(SplitDockTree.Key<Dockable> key, boolean checkValidity, Map<Leaf, Dockable> linksToSet) {
    ...
    private long getSplitId(SplitDockPathProperty property, int depth) {
        ...
        if (lastNode != node) {
            if (depth > 0) {
                return property.getNode(depth - 1).id();
            } else {
                return node.id();
            }
        }
    ...
    public <N> N submit(SplitTreeFactory<N> factory) {
        if (orientation == SplitDockStation.Orientation.HORIZONTAL) {
            return factory.horizontal(left.submit(factory), right.submit(factory), divider, getId(),
                    getPlaceholders(), getPlaceholderMap(), isVisible());
        } else {
            return factory.vertical(left.submit(factory), right.submit(factory), divider, getId(), getPlaceholders(),
                    getPlaceholderMap(), isVisible());
        }
    }
    ...
    public void toString(int tabs, StringBuilder out) {
        ...
        out.append('\n');
        out.append("\t".repeat(Math.max(0, tabs + 1)));
        if (left == null) {
            out.append("<null>");
        } else {
