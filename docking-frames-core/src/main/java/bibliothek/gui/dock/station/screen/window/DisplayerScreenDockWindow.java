...
public abstract class DisplayerScreenDockWindow implements ScreenDockWindow {
     ...
     */
    private final List<ScreenDockWindowListener> listeners = new ArrayList<>();

    /**
     * a listener to the current {@link DockableDisplayer}
     */
    private final DockableDisplayerListener displayerListener = new DockableDisplayerListener() {
    ...
    protected ScreenDockWindowListener[] listeners() {
        return listeners.toArray(new ScreenDockWindowListener[0]);
    }
    ...
    public void setDockable(Dockable dockable) {
        ...
        if (dockable != null) {
            handle = new StationChildHandle(station, station.getDisplayers(), dockable,
                    showTitle ? station.getTitleVersion() : null) {
                @Override
