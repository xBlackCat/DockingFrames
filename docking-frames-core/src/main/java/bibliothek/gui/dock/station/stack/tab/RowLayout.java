...
public class RowLayout implements TabLayoutManager {
    ...
    public void layout(TabPane pane) {
        ...
        if (required.height < available.height) {
            pane.setSelectedBounds(conversion.modelToView(new Rectangle(0, required.height, available.width,
                    available.height - required.height)));
        } else {
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            tabs[i].setBounds(conversion.modelToView(new Rectangle(
                    x + dx, y + dy - size.height + maxRowHeight, size.width, size.height)));
            x += size.width;
            rowCount++;
        }
    }
