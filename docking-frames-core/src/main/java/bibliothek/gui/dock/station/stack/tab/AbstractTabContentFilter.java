 ...
 * Subclasses may override {@link #added(StackDockStation, Dockable) added},
 * {@link #removed(StackDockStation, Dockable) removed},
 * {@link #deselected(StackDockStation, Dockable) deselected} and {@link #selected(StackDockStation, Dockable)
 * selected} to be informed
...
public abstract class AbstractTabContentFilter implements TabContentFilter {
     ...
     */
    private final List<TabContentFilterListener> listeners = new ArrayList<>();

    /**
     * all stations that are currently installed
     */
    protected final List<StackDockStation> stations = new ArrayList<>();

    /**
     * all the components that are currently installed
     */
    protected final List<StackDockComponent> components = new ArrayList<>();

    /**
     * a listener added to all {@link StackDockStation}s
     */
    private final DockStationListener stationListener = new DockStationAdapter() {
    ...
    protected TabContentFilterListener[] listeners() {
        return listeners.toArray(new TabContentFilterListener[0]);
    }
