 ...
 * represented through a {@link Key<D>}. Client code may use these keys to read
 ...
 * class allows to set this identifier either through {@link Key<D>#setNodeId(long)} or
...
public abstract class SplitDockTree<D> {
     ...
     */
    private final Set<D> dockables = new HashSet<>();
     ...
     */
    public SplitDockTree<D> root(Key<D> key) {
        if (key == null) {
            throw new IllegalArgumentException("Key<D> must not be null");
        }

        if (key.getTree() != this) {
            throw new IllegalArgumentException("Key<D> is not created by this tree");
        }

        if (key.parent != null) {
            throw new IllegalArgumentException("Key<D> has a parent, and can't be the root");
        }
     ...
     * @return the new key
     */
    public Key<D> put(D dockable, long nodeId) {
        D[] array = array(1);
        array[0] = dockable;
     ...
     */
    @SafeVarargs
    public final Key<D> put(D... dockables) {
     ...
     * @return the new key
     */
    public Key<D> put(D[] dockables, D selected) {
     ...
     * @return the new key
     */
    public Key<D> put(D[] dockables, D selected, long nodeId) {
     ...
     * @return the new key
     */
    public Key<D> put(Path[] placeholders, PlaceholderMap placeholderMap) {
     ...
     * @return the new key
     */
    public Key<D> put(Path[] placeholders, PlaceholderMap placeholderMap, long nodeId) {
     ...
     * @return the new key
     */
    public Key<D> put(D[] dockables, D selected, Path[] placeholders, PlaceholderMap placeholderMap, long nodeId) {
        ...
        return new Leaf<>(SplitDockTree.this, dockables, selected, placeholders, placeholderMap, nodeId);
    }
     ...
     */
    public Key<D> horizontal(D left, D right) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> horizontal(D left, D right, double divider) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> horizontal(Key<D> left, Key<D> right) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> horizontal(Key<D> left, Key<D> right, double divider) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> horizontal(Key<D> left, Key<D> right, double divider, long nodeId) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> horizontal(Key<D> left, Key<D> right, double divider, Path[] placeholders,
                             PlaceholderMap placeholderMap, long nodeId) {
        return new Node<>(SplitDockTree.this, left, right, divider, true, placeholders, placeholderMap, nodeId);
    }
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> vertical(D top, D bottom) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> vertical(D top, D bottom, double divider) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> vertical(Key<D> top, Key<D> bottom) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> vertical(Key<D> top, Key<D> bottom, double divider) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> vertical(Key<D> top, Key<D> bottom, double divider, long nodeId) {
     ...
     * @return a key of the combination of the two elements
     */
    public Key<D> vertical(Key<D> top, Key<D> bottom, double divider, Path[] placeholders,
                           PlaceholderMap placeholderMap, long nodeId) {
        return new Node<>(SplitDockTree.this, top, bottom, divider, false, placeholders, placeholderMap, nodeId);
    }
     ...
     */
    public boolean isDockable(Key<D> key) {
     ...
     */
    public boolean isNode(Key<D> key) {
     ...
     */
    public boolean isPlaceholder(Key<D> key) {
     ...
     */
    public Path[] getPlaceholders(Key<D> key) {
     ...
     */
    public PlaceholderMap getPlaceholderMap(Key<D> key) {
     ...
     */
    public D[] getDockables(Key<D> key) {
        if (!isDockable(key)) {
     ...
     */
    public D getSelected(Key<D> key) {
        if (!isDockable(key)) {
            throw new IllegalArgumentException("Not a Dockable");
        }
     ...
     */
    public boolean isHorizontal(Key<D> key) {
        if (!isNode(key)) {
     ...
     */
    public Key<D> getLeft(Key<D> key) {
        if (!isNode(key)) {
            throw new IllegalArgumentException("Not a node");
        }
     ...
     */
    public Key<D> getRight(Key<D> key) {
        if (!isNode(key)) {
            throw new IllegalArgumentException("Not a node");
        }
     ...
     */
    public Key<D> getBottom(Key<D> key) {
        if (!isNode(key)) {
            throw new IllegalArgumentException("Not a node");
        }
        return key.asNode().keyB;
     ...
     */
    public double getDivider(Key<D> key) {
        if (!isNode(key)) {
            throw new IllegalArgumentException("Not a node");
        }
    ...
    public String toString() {
        ...
        builder.append(getClass().getName()).append("[root=\n");
        if (root == null) {
            builder.append("null");
        } else {
     ...
     * @author Benjamin Sigg
     */
    public static abstract class Key<D> {
        private final SplitDockTree<D> tree;
        /**
         * the parent of this node or leaf
         */
        private Key<D> parent;
         ...
         */
        public Key(SplitDockTree<D> tree, Path[] placeholders, PlaceholderMap placeholderMap, long id) {
            this.id = id;
         ...
         */
        private void setParent(Key<D> parent) {
            this.parent = parent;
        }
         ...
         * @return this or <code>null</code>
         */
        protected Node<D> asNode() {
            return null;
        }
    }

    /**
     * A {@link Key<D>} which represents a leaf.
     *
     * @author Benjamin Sigg
     */
    private static class Leaf<D> extends Key<D> {
        /**
         ...
         */
        public Leaf(SplitDockTree<D> tree, D[] dockables, D selected, Path[] placeholders,
                    PlaceholderMap placeholderMap, long id) {
            super(tree, placeholders, placeholderMap, id);
            if (dockables != null) {
        ...
        protected void toString(StringBuilder builder, int depth) {
            ...
            Path[] placeholders = getTree().getPlaceholders(this);
            if (placeholders != null) {
    ...
    /**
     * A {@link Key<D>} which represents a node in the tree.
     *
     * @author Benjamin Sigg
     */
    private static class Node<D> extends Key<D> {
        /**
         * left or top child
         */
        public Key<D> keyA;
        ...
        public Node(
            ...
            if (keyA.getTree() != getTree()) {
                throw new IllegalArgumentException("Key<D> of first argument belongs not to this tree");
            }

            if (keyB.getTree() != getTree()) {
                throw new IllegalArgumentException("Key<D> of second argument belongs not to this tree");
            }
