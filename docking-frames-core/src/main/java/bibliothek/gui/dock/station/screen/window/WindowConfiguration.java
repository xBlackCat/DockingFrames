 ...
 * ({@link ScreenDockWindowFactory#createWindow(bibliothek.gui.dock.ScreenDockStation, WindowConfiguration)}),
...
public class WindowConfiguration {
    ...
    /**
     * only required if {@link #moveOnTitleGrab}, starts a drag and drop operation if the mouse is over an area where
     * dropping is possible
     */
    private boolean allowDragAndDropOnTitle = true;

    /**
     * only required if {@link #moveOnTitleGrab} and {@link #allowDragAndDropOnTitle}, the window jumps back into its
     * original position if dropping is possible
     */
     ...
     *
     * @param allowDragAndDropOnTitle whether drag and drop should be possible even if the window is already moved
     *                                around
     */
     ...
     * set, but not both at the same time. If both are set, then {@link #isTransparent() transparency} is considered
     * to be more
     ...
     * border is actually created, most windows will not create a window if {@link #isResizeable()} returns
     * <code>false</code>.
     *
