...
public class OrderedLayerCollection {
     ...
     */
    private final Set<DockStation> stations = new HashSet<>();

    /**
     * factory applied to all {@link DockStation}s to find their {@link DockStationDropLayer}s
     */
    private final DockStationDropLayerFactory factory;
    ...
    public List<DockStation> sort(int x, int y) {
        Map<DockStation, Node> nodes = new HashMap<>();
        ...
        List<DockStationDropLayer> layers = new ArrayList<>();
        for (Node root : roots) {
        ...
        List<DockStation> result = new ArrayList<>();
    ...
    protected List<DockStationDropLayer> sort(List<DockStationDropLayer> layers) {
        List<DockStationDropLayer> result = new LinkedList<>();
     ...
     * @return a value less/equal/greater to 0 depending on whether <code>a</code> is less/equal/greater than
     * <code>b</code>.
     */
    ...
    private class Node {
        ...
        private final DockStationDropLayer[] layers;
        ...
        public Node(DockStation station) {
            ...
            if (station.getDockableCount() > 0) {
                children = new ArrayList<>(station.getDockableCount());
            }
