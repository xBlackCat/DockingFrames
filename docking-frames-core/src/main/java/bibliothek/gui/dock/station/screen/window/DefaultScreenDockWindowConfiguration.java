...
public class DefaultScreenDockWindowConfiguration implements ScreenDockWindowConfiguration {
     ...
     */
    private final ScreenDockWindowConfiguration[] extensions;
    ...
    public DefaultScreenDockWindowConfiguration(DockController controller) {
        if (controller != null) {
            List<ScreenDockWindowConfiguration> list =
                    controller.getExtensions().load(new ExtensionName<>(CONFIGURATION_EXTENSION,
                            ScreenDockWindowConfiguration.class, CONFIGURATION_EXTENSION_PARAM, this));
            extensions = list.toArray(new ScreenDockWindowConfiguration[0]);
        } else {
