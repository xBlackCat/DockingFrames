 ...
 */
public class ScreenDockStationFactory implements DockFactory<ScreenDockStation, ScreenDockPerspective,
    ...
    public void estimateLocations(ScreenDockStationLayout layout, final LocationEstimationMap children) {
        if (layout instanceof RetroScreenDockStationLayout retro) {

            for (int i = 0, n = retro.size(); i < n; i++) {
                DockLayoutInfo info = children.getChild(retro.id(i));
                if (info != null) {
                    ScreenDockProperty property =
                            new ScreenDockProperty(retro.x(i), retro.y(i), retro.width(i), retro.height(i), null);
                    info.setLocation(property);
                }
            }
        } else {
            DockablePlaceholderList.simulatedRead(layout.getPlaceholders(), new PlaceholderListItemAdapter<>() {
                @Override
                public PlaceholderListItem<Dockable> convert(ConvertedPlaceholderListItem item) {
                    ...
                    for (int i = 0, n = children.getSubChildCount(id); i < n; i++) {
                        ...
                        info.setLocation(new ScreenDockProperty(x, y, width, height, info.getPlaceholder(),
                                fullscreen));
                    }
    ...
    public ScreenDockStationLayout getPerspectiveLayout(ScreenDockPerspective element, Map<PerspectiveDockable,
            Integer> children) {
    ...
    public void layoutPerspective(ScreenDockPerspective perspective, ScreenDockStationLayout layout, Map<Integer,
