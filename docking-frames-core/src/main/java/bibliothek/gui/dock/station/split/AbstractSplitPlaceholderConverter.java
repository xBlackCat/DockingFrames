...
public abstract class AbstractSplitPlaceholderConverter<P, N, D> {
    ...
    public void setPlaceholders(PlaceholderMap map) {
        ...
        BuildNode<D> root = new BuildNode<>();
        for (Key key : keys) {
    ...
    private Key handleBase(PlaceholderMap result, N node, String path, Set<Path> allPlaceholders,
                           Set<Path> localPlaceholders) {
        ...
        if (!allPlaceholders.isEmpty()) {
            Key key = result.newKey(allPlaceholders.toArray(new Path[0]));
            ...
            if (localPlaceholders != null && !localPlaceholders.isEmpty()) {
                Key arrayKey = result.newKey(localPlaceholders.toArray(new Path[0]));
    ...
    private void handleEntry(Key key, PlaceholderMap map, BuildNode<D> root) {
        for (String item : map.getKeys(key)) {
            BuildNode<D> node = root.get(path(item));
            String itemKey = key(item);
            switch (itemKey) {
                case "type" -> node.type = map.getString(key, item);
                case "id" -> node.id = map.getLong(key, item);
                case "map" -> node.map = map.getMap(key, item);
                case "array" -> node.placeholders = key.getPlaceholders();
                case "divider" -> node.divider = map.getDouble(key, item);
                case "orientation" -> node.orientation = map.getBoolean(key, item);
            }
        }
    }
     ...
     * @author Benjamin Sigg
     */
    private static class BuildNode<D> {
        private BuildNode<D>[] children;
        private long id = -1;
        private String type;
        ...
        public BuildNode<D> get(String path) {
            return get(path, 0);
        }

        private BuildNode<D> get(String path, int index) {
        ...
        private BuildNode<D>[] createArray(int length) {
            return (BuildNode<D>[]) new BuildNode[length];
        }

        private BuildNode<D> get(int index) {
            if (children == null) {
                children = createArray(index + 1);
            }
            if (children.length <= index) {
                BuildNode<D>[] temp = createArray(index + 1);
        ...
        public SplitDockTree.Key<D> collapse(SplitDockTree<D> tree) {
            if ("l".equals(type) || "p".equals(type)) {
                ...
                SplitDockTree.Key<D> left = null;
                SplitDockTree.Key<D> right = null;
                ...
                if (children != null && children.length > 0 && children[0] != null) {
                    SplitDockTree.Key<D> child = children[0].collapse(tree);
                    if (child != null) {
                        tree.root(child);
                        return child;
                    }
