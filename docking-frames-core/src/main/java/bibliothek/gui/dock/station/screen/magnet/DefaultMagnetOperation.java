...
public class DefaultMagnetOperation implements MagnetOperation {
    ...
    protected void moved(MagnetController controller, MagnetRequest request) {
        ...
        List<ScreenDockWindow> windows = new ArrayList<>();
            ...
            if (graph.getRoot().getNeighbor(partner) == null) {
                for (Side side : Side.values()) {
                    ...
                    if (distance <= threshold) {
                        boolean neighbors = switch (side) {
                            case NORTH, SOUTH -> controller.intersectVertically(request.getWindow(), partner, false);
                            case EAST, WEST -> controller.intersectHorizontally(request.getWindow(), partner, false);
                        };
                        if (neighbors) {
                            if (distance < nearest) {
                                nearest = distance;
    ...
    private void neighborMoved(MagnetController controller, MagnetRequest request, List<ScreenDockWindow> neighbors,
                               Side side) {
        Side checkA;
        Side checkB;
    ...
    private void neighborResized(MagnetController controller, MagnetRequest request, Side side,
                                 List<ScreenDockWindow>[] neighbors) {
        Side checkA = switch (side) {
            case NORTH, SOUTH -> Side.EAST;
            case EAST, WEST -> Side.NORTH;
        };
