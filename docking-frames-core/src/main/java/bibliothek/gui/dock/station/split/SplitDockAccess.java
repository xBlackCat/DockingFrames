...
public interface SplitDockAccess {
     ...
     */
    boolean drop(Dockable dockable, final SplitDockProperty property, SplitNode root);

    /**
     * Invoked whenever a node changes its shape. Leads to a call to
     * {@link DockStationListener#dockablesRepositioned(bibliothek.gui.DockStation, Dockable[])}
     ...
     */
    void drop(Token token, PutInfo putInfo, StationDropItem item);
}
