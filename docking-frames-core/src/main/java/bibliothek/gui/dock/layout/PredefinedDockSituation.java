...
public class PredefinedDockSituation extends DockSituation {
     ...
     */
    private final Map<String, DockElement> stringToElement = new HashMap<>();
    /**
     * A mapping from a list of elements to their ids
     */
    private final Map<DockElement, String> elementToString = new HashMap<>();
     ...
     */
    private final Map<String, DockFactory<? extends DockElement, ?, ? extends BackupFactoryData<?>>> backups =
            new HashMap<>();
     ...
     * Please note that clients need to register {@link DockElement}s using
     * {@link PredefinedPerspective#put(String, PerspectiveElement)}
     * before the perspective can be used. The {@link DockElement}s registered to <code>this</code> by using
     * {@link #put(String, DockElement)}
     * are ignored by the perspective.
     */
    @Override
    public PredefinedPerspective createPerspective() {
        return new PredefinedPerspective(this) {
            private final PreloadFactory preload;
     ...
     * factory has only to look at the {@link BackupFactoryData#data() data}-property.
    ...
    private <D extends DockElement> void listVisible(final Collection<D> base, DockLayoutComposition composition,
                                                     final Set<D> result) {
     ...
     */
    public Map<String, DockableProperty> listEstimatedLocations(DockLayoutComposition composition,
                                                                boolean missingOnly) {
     ...
     */
    public Map<String, DockableProperty> listEstimatedLocations(DockLayoutComposition composition,
                                                                DockableProperty location, boolean missingOnly) {
        estimateLocations(composition, location);

        Map<String, DockableProperty> map = new HashMap<>();
    ...
    private void listEstimatedLocations(DockLayoutComposition composition, boolean missingOnly, Map<String,
            DockableProperty> map) {
    ...
    public Map<String, DockLayoutComposition> listLayouts(DockLayoutComposition composition, boolean missingOnly) {
        Map<String, DockLayoutComposition> map = new HashMap<>();
        listLayouts(composition, missingOnly, map);
        return map;
    }

    private void listLayouts(DockLayoutComposition composition, boolean missingOnly, Map<String,
            DockLayoutComposition> map) {
        String key = getKey(composition, missingOnly);
        if (key != null) {
    ...
    private DockLayoutInfo fillMissingStream(PredefinedLayout layout) {
        ...
        try {
            ...
            if (factory == null) {
                ...
                if (backup != null) {
                    BackupFactoryData<Object> data =
                            (BackupFactoryData<Object>) backup.read(in, getPlaceholderStrategy());
                    if (data != null && data.data() != null) {
                        info = new DockLayoutInfo(new DockLayout<>(factoryId, data.data()));
                    }
                }
            } else {
    ...
    private DockLayoutInfo fillMissingXML(PredefinedLayout layout) {
        ...
        if (factory == null) {
            ...
            if (backup != null) {
                BackupFactoryData<Object> data =
                        (BackupFactoryData<Object>) backup.read(xdelegate, getPlaceholderStrategy());
                if (data != null) {
                    delegate = data.data();
                }
            }
        ...
        return new DockLayoutInfo(new DockLayout<>(factoryId, delegate));
    }
    ...
    private class PreloadFactory implements DockFactory<DockElement, PerspectiveElement, PredefinedLayout> {
        ...
        public void estimateLocations(PredefinedLayout layout, LocationEstimationMap children) {
            ...
            if (delegate.getKind() == DockLayoutInfo.Data.DOCK_LAYOUT) {
                ...
                DockFactory<DockElement, ?, Object> factory =
                        (DockFactory<DockElement, ?, Object>) getFactory(factoryId);
                if (factory != null) {
        ...
        public PredefinedLayout getLayout(DockElement element, Map<Dockable, Integer> children) {
            if (shouldLayout(element)) {
                ...
                DockLayout<Object> layout = new DockLayout<>(factoryId, data);
        ...
        @SuppressWarnings("unchecked")
        public void setLayout(DockElement element, PredefinedLayout layout, Map<Integer, Dockable> children,
        ...
        public DockElement layout(PredefinedLayout layout, Map<Integer, Dockable> children,
                                  PlaceholderStrategy placeholders) {
            DockLayoutInfo delegate = layout.getDelegate();
        ...
        @SuppressWarnings("unchecked")
        public PerspectiveElement layoutPerspective(PredefinedLayout layout,
                                                    Map<Integer, PerspectiveDockable> children) {
            if (perspective == null) {
                throw new IllegalStateException("the perspective of this factory is not set, meaning this factory " +
                        "cannot be used handling perspective dependent tasks");
            }
            ...
            if (element == null && isLayout) {
                ...
                if (factory != null) {
                    return factory.layoutPerspective(new BackupFactoryData<Object>(layout.getPredefined(),
                            delegate.getDataLayout().getData()), children);
                }
                return null;
            }
        ...
        @SuppressWarnings("unchecked")
        public void layoutPerspective(PerspectiveElement element, PredefinedLayout layout, Map<Integer,
        ...
        @SuppressWarnings("unchecked")
        public PredefinedLayout getPerspectiveLayout(PerspectiveElement element,
            ...
            if (key == null) {
                throw new IllegalStateException(
                        "Expected a key for an element, the element should be known to the perspective, otherwise " +
        ...
        public void write(PredefinedLayout layout, DataOutputStream out) throws IOException {
            ...
            if (info.getKind() == DockLayoutInfo.Data.BYTE) {
                ...
                throw new IllegalArgumentException("Cannot store information as byte[], it is not present as raw " +
                        "byte[] or in an understandable format");
            }
        }
        ...
        public void write(PredefinedLayout layout, XElement element) {
            ...
            if (info.getKind() == DockLayoutInfo.Data.XML) {
                ...
                throw new IllegalArgumentException("Cannot store information as xml, it is neither present as raw xml" +
                        " nor in an understandable format");
            }
        }
        ...
        public PredefinedLayout read(XElement element, PlaceholderStrategy placeholders) {
            ...
            if (delegate == null) {
                ...
                DockLayout<Object> layout = new DockLayout<>(factoryId, delegate);
