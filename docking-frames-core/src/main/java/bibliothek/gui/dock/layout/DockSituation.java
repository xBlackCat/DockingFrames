 ...
 * stream of bytes. The other direction, read a stream and create <code>Dockable</code>s and
 * <code>DockStation</code>s, is also possible.<br>
 *
...
public class DockSituation {
     ...
     */
    private final Map<String, DockFactory<?, ?, ?>> factories = new HashMap<>();
     ...
     */
    private final Map<String, AdjacentDockFactory<?>> adjacent = new HashMap<>();
    ...
    public DockSituation(DockController controller) {
        ...
        List<DockFactory> factories =
                controller.getExtensions().load(new ExtensionName<>(DOCK_FACTORY_EXTENSION, DockFactory.class,
        ...
        @SuppressWarnings("rawtypes")
        List<AdjacentDockFactory> adjacent =
                controller.getExtensions().load(new ExtensionName<>(ADJACENT_DOCK_FACTORY_EXTENSION,
                        AdjacentDockFactory.class, EXTENSION_PARAM, this));
    ...
    public DockLayoutComposition convert(DockElement element) {
        ...
        Map<Dockable, Integer> ids = new HashMap<>();
        List<DockLayoutComposition> children = new ArrayList<>();
        ...
        DockLayout<Object> layout = new DockLayout<>(id, data);
    ...
    public DockElement convert(DockLayoutComposition composition) {
        ...
        DockFactory<DockElement, ?, Object> factory =
                (DockFactory<DockElement, ?, Object>) getFactory(layout.getFactoryID());
        if (factory == null) {
            return null;
        }
        ...
        if (result != null) {
            ...
            if (adjacent != null) {
                for (DockLayout<?> adjacentLayout : adjacent) {
                    AdjacentDockFactory<Object> adjacentFactory =
                            (AdjacentDockFactory<Object>) getAdjacentFactory(adjacentLayout.getFactoryID());
    ...
    private void writeCompositionStream(DockLayoutComposition composition, DataOutputStream out) throws IOException {
        ...
        if (info.getKind() == DockLayoutInfo.Data.BYTE) {
            ...
            throw new IllegalArgumentException("Cannot store layout in stream: it was never translated and its raw " +
                    "format is not a byte stream");
        }
        ...
        if (Version.VERSION_1_0_7.compareTo(version) <= 0) {
            ...
            if (layoutCount > 0) {
                ...
                for (int i = 0; i < layoutCount; i++) {
                    ...
                    AdjacentDockFactory<Object> adjacentFactory =
                            (AdjacentDockFactory<Object>) getAdjacentFactory(adjacentFactoryId);
                    if (adjacentFactory == null) {
                        if (missingAdjacent == null) {
                            ...
                            Object data =
                                    missingAdjacent.read(getAdjacentFactoryID(adjacentFactoryId), din, adjacentCount);
                            if (data != null) {
    ...
    public Map<String, DockStation> read(DataInputStream in) throws IOException {
        ...
        int count = in.readInt();
        Map<String, DockStation> result = new HashMap<>();
        for (int i = 0; i < count; i++) {
    ...
    public void writeCompositionXML(DockLayoutComposition composition, XElement element) {
        ...
        if (info.getKind() == DockLayoutInfo.Data.XML) {
            ...
            // there is nothing to write...
            throw new IllegalArgumentException("Cannot store layout as XML: it was never translated and its raw " +
                    "format is not XML");
        }
    ...
    public DockLayoutComposition readCompositionXML(XElement element) {
        ...
        if (xadjacent != null) {
            ...
            for (XElement xlayout : xadjacent.getElements("layout")) {
                ...
                AdjacentDockFactory<Object> adjacentFactory =
                        (AdjacentDockFactory<Object>) getAdjacentFactory(factoryId);
                if (adjacentFactory != null) {
    ...
    public DockLayoutComposition fillMissing(DockLayoutComposition composition) throws IOException, XException {
        ...
        if (createNew) {
            return new DockLayoutComposition(info, composition.getAdjacent(), newChildren,
                    composition.isIgnoreChildren());
        }
    ...
    @Todo(compatibility = Compatibility.BREAK_MINOR, priority = Priority.MAJOR, target = Todo.Version.VERSION_1_1_3,
            description = "remove the legacy code that filters out identifiers that look like 'secure ...'. Instead " +
