...
/**
 * This default implementation of a {@link LayoutChangeStrategy} heavily depends on the methods of
 * {@link PredefinedDockSituation}. It
 * also offers a set of methods that may be interesting for subclasses which do not use a
 * {@link PredefinedDockSituation}.
 *
...
public class DefaultLayoutChangeStrategy implements LayoutChangeStrategy {
    ...
    public boolean setLayout(DockFrontendInternals frontend, Setting setting, boolean entry) throws IOException,
                                                                                                    XException {
        try {
    ...
    private boolean updateLayout(DockFrontendInternals frontend, Setting setting, boolean entry) throws IOException,
                                                                                                        XException {
     ...
     * @param onSetLayout whether this method is called by
     *                    {@link #setLayout(DockFrontendInternals, Setting, boolean)} or not. If
     *                    <code>true</code> then the situation is used to apply some layout, otherwise it is used to
     *                    store or read a layout
    ...
    protected PredefinedDockSituation createSituation(final DockFrontendInternals frontend, final boolean entry,
                                                      boolean onSetLayout) {
        ...
        for (RootInfo info : frontend.getRoots()) {
            situation.put(DockFrontend.ROOT_KEY_PREFIX + info.name(), info.station());
        }
    ...
    /**
     * Tells whether the element <code>dockable</code> should be added as predefined element to the
     * {@link PredefinedDockSituation}
    ...
    public DockFrontendPerspective createPerspective(DockFrontendInternals frontend, boolean entry,
                                                     final FrontendPerspectiveCache cache) {
        ...
        for (RootInfo info : frontend.getRoots()) {
            PerspectiveElement element = cache.get(info.name(), info.station(), true);
            perspective.put(DockFrontend.ROOT_KEY_PREFIX + info.name(), element);
        }
    ...
    protected void applyLayout(DockFrontendInternals frontend, DockSituation situation, SettingAccess setting,
        ...
        for (RootInfo info : frontend.getRoots()) {
            DockLayoutComposition layout = setting.getRoot(info.name());
            if (layout != null) {
                ...
                if (missingLocations != null) {
                    for (Map.Entry<String, DockableProperty> missing : missingLocations.entrySet()) {
                        ...
                        if (dockInfo != null) {
                            dockInfo.setLocation(info.name(), missing.getValue());
     ...
     * @throws XException  if the layout cannot be converted
     */
    protected void applyInvisibleLayout(DockFrontendInternals frontend, DockSituation situation,
                                        SettingAccess setting) throws IOException, XException {
        DockFrontend dockFrontend = frontend.getFrontend();
        MissingDockableStrategy missingDockable = frontend.getMissingDockableStrategy();
    ...
    protected Map<String, DockLayoutComposition> listLayouts(DockSituation situation, DockLayoutComposition layout) {
        if (situation instanceof PredefinedDockSituation) {
            ...
            Map<String, DockLayoutComposition> result = new HashMap<>();
    ...
    protected Map<String, DockableProperty> listEstimateLocations(DockSituation situation,
        ...
        if (situation instanceof PredefinedDockSituation) {
            Map<String, DockableProperty> map =
                    ((PredefinedDockSituation) situation).listEstimatedLocations(layout, true);
            Map<String, DockableProperty> result = new HashMap<>();
    ...
    public void estimateLocations(DockFrontendInternals frontend, DockSituation situation,
                                  DockLayoutComposition layout) {
        if (situation instanceof PredefinedDockSituation) {
            situation.estimateLocations(layout);
        }
    }
    ...
    protected Collection<Dockable> approveClosing(DockFrontendInternals frontend, DockSituation situation,
        ...
        Set<Dockable> remainingVisible = new HashSet<>();
        for (RootInfo info : frontend.getRoots()) {
            DockLayoutComposition layout = setting.getRoot(info.name());
            if (layout != null) {
     ...
     *
     * @param frontend the caller of this method
     * @param visible  the elements that remain visible as told by
     *                 {@link #estimateVisible(DockFrontendInternals, DockSituation, DockLayoutComposition)}.
    ...
    protected Collection<Dockable> getClosingDockables(DockFrontendInternals frontend, Set<Dockable> visible) {
        List<Dockable> closing = new ArrayList<>();
        for (DockInfo info : frontend.getDockables()) {
     ...
     */
    protected Set<Dockable> estimateVisible(DockFrontendInternals frontend, DockSituation situation,
                                            DockLayoutComposition layout) {
        if (situation instanceof PredefinedDockSituation predefined) {
            Set<Dockable> allDockables = new HashSet<>();
            for (DockInfo info : frontend.getDockables()) {
                Dockable dockable = info.getDockable();
                if (dockable != null) {
            ...
            return predefined.listVisible(allDockables, layout);
        }
     ...
     * @author Benjamin Sigg
     */
    protected static class SettingAccess {
