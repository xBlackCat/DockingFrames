...
public class VetoManager {
     ...
     */
    private final Set<Dockable> expectedToShow = new HashSet<>();

    /**
     * the set of elements which are not yet hidden but are expected to hide soon
     */
    private final Set<Dockable> expectedToHide = new HashSet<>();

    /**
     * the set of elements which are shown
     */
    private final Set<Dockable> dockables = new HashSet<>();

    /**
     * A list of observers to be notified if a {@link Dockable} gets closed or opened
     */
    private final List<VetoableDockFrontendListener> vetoableListeners = new ArrayList<>();
    ...
    protected VetoableDockFrontendListener[] vetoableListeners() {
        return vetoableListeners.toArray(new VetoableDockFrontendListener[0]);
    }
    ...
    protected boolean fireAllHiding(Collection<Dockable> dockables, final boolean cancelable) {
        ...
        VetoableDockFrontendEvent event =
                new VetoableDockFrontendEvent(frontend, cancelable, true, dockables.toArray(new Dockable[0]));
    ...
    protected void fireAllHidden(Collection<Dockable> dockables, final boolean expected) {
        if (!dockables.isEmpty()) {
            VetoableDockFrontendEvent event =
                    new VetoableDockFrontendEvent(frontend, false, expected, dockables.toArray(new Dockable[0]));
