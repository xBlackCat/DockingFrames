...
public interface LayoutChangeStrategy {
     ...
     */
    boolean setLayout(DockFrontendInternals frontend, Setting setting, boolean entry) throws IOException, XException;
     ...
     *                 <code>false</code> if the situation is used as the final setting which will
     *                 be loaded the next time the application starts.
     * @param cache    a cache that takes {@link DockElement}s and returns the matching {@link PerspectiveElement}.
     *                 The cache
     *                 also offers methods to convert ids and {@link PerspectiveElement}s directly
     * @return the new perspective
     */
    DockFrontendPerspective createPerspective(DockFrontendInternals frontend, boolean entry,
    ...
    /**
     * Gets the default {@link PlaceholderStrategy} which should be used to filter placeholders by
     * <code>frontend</code>.
     *
    ...
    /**
     * Called when <code>dockable</code> is added to a {@link DockFrontend}, and the frontend already knows the
     * layout which should be used
     * for <code>dockable</code>. If the result is <code>true</code>, then all children of <code>dockable</code> are
     * removed, and reloaded to apply the layout
     * that is stored in the frontend.<br>
     * Subclasses may return <code>false</code> if {@link #setLayout(DockFrontendInternals, Setting, boolean)} is
     * currently executed, preventing concurrent
