 ...
 *  to know the identifiers of unregistered elements. These elements will be ignored and thrown away when reading the
 *  layout.</li>
 * </ul>
 *
...
public class DefaultFrontendPerspectiveCache implements FrontendPerspectiveCache {
    ...
    private final Map<PerspectiveElement, String> identifiers = new HashMap<>();
    ...
    public DefaultFrontendPerspectiveCache(DockFrontend frontend) {
        ...
        extensions =
                frontend.getController().getExtensions().load(new ExtensionName<>(CACHE_EXTENSION,
                        FrontendPerspectiveCacheExtension.class));
    }
        ...
        if (result == null) {
            ...
            if (!element.getFactoryID().equals(result.getFactoryID())) {
                throw new IllegalArgumentException(
                        "the factory configured for 'element' is '" + element.getFactoryID() + "', but expected was '" +
                                result.getFactoryID() +
                                "'. Clients need to subclass this cache and handle this special case.");
            }
