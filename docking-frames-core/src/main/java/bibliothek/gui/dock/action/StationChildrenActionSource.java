...
public class StationChildrenActionSource extends AbstractDockActionSource {
    ...
    private final List<Tuple<Dockable, DockAction>> actions = new ArrayList<>();
    ...
    public Iterator<DockAction> iterator() {
        updateActionList(false);
        return new Iterator<>() {
            private final Iterator<Tuple<Dockable, DockAction>> iterator = actions.iterator();
    ...
    private void updateActionList(boolean force) {
        ...
        try {
            ...
            List<Dockable> list = new ArrayList<>();
            fill(dockable, list);
            sort(list);
            ...
            Set<Dockable> pendingActions = new HashSet<>();
    ...
    protected class FocusAction extends SimpleSelectableAction.Check implements DockableListener {
        ...
        private final DockStationAdapter adapter = new DockStationAdapter() {
            @Override
        ...
        private final DockHierarchyListener hierarchy = new DockHierarchyListener() {
        ...
        public FocusAction(Dockable dockable) {
            ...
            addSelectableListener((action, dockables) -> checkDockable());
        }
