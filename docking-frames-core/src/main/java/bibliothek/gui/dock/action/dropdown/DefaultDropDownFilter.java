...
public class DefaultDropDownFilter extends AbstractDropDownFilter {
    ...
    protected void updateIcon(DropDownViewItem selection) {
        Set<ActionContentModifier> modifiers = new HashSet<>(icons.keySet());
        modifiers.addAll(Arrays.asList(getView().getIconContexts()));
        modifiers.addAll(Arrays.asList(getAction().getIconContexts(getDockable())));
