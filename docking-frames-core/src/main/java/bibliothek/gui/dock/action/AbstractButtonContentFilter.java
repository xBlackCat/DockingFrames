...
public abstract class AbstractButtonContentFilter implements ButtonContentFilter {
     ...
     */
    private final List<ButtonContentFilterListener> listeners = new ArrayList<>();

    /**
     * all the action that are currently installed
     */
    private final Map<DockAction, Map<Dockable, Integer>> count = new HashMap<>();
     ...
     *
     * @param dockable the dockable for which the result of {@link #showText(Dockable, DockAction)} changed, can be
     *                    <code>null</code>
     * @param action   the action for which the result of {@link #showText(Dockable, DockAction)} changed, can be
     *                    <code>null</code>
     */
    protected void fire(Dockable dockable, DockAction action) {
        for (ButtonContentFilterListener listener : listeners.toArray(new ButtonContentFilterListener[0])) {
