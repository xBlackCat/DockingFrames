...
public class ActionViewConverter {
     ...
     */
    private final Map<Tuple<ActionType<?>, ViewTarget<?>>, Entry<?, ?>> converters =
            new HashMap<>();
    ...
    public ActionViewConverter() {
        ...
        putDefault(ActionType.BUTTON, ViewTarget.MENU, (converter, action, dockable) -> new ButtonMenuHandler(action,
                dockable));

        putDefault(ActionType.CHECK, ViewTarget.MENU,
                (converter, action, dockable) -> new SelectableMenuHandler(action, dockable, new JCheckBoxMenuItem()));

        putDefault(ActionType.MENU, ViewTarget.MENU, (converter, action, dockable) -> new MenuMenuHandler(action,
                dockable));

        putDefault(ActionType.RADIO, ViewTarget.MENU,
                (converter, action, dockable) -> new SelectableMenuHandler(action, dockable,
                        new JRadioButtonMenuItem()));

        putDefault(ActionType.SEPARATOR, ViewTarget.MENU, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.DROP_DOWN, ViewTarget.MENU,
                (converter, action, dockable) -> new DropDownMenuHandler(action, dockable));

        // Converter for title

        putDefault(ActionType.BUTTON, ViewTarget.TITLE, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.CHECK, ViewTarget.TITLE, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.MENU, ViewTarget.TITLE, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.RADIO, ViewTarget.TITLE, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.SEPARATOR, ViewTarget.TITLE, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.DROP_DOWN, ViewTarget.TITLE, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.BUTTON, ViewTarget.DROP_DOWN,
                (converter, action, dockable) -> new ButtonDropDownHandler(action, dockable, new JMenuItem()));

        putDefault(ActionType.CHECK, ViewTarget.DROP_DOWN,
                (converter, action, dockable) -> new SelectableDropDownHandler(action, dockable,
                        new JCheckBoxMenuItem()));

        putDefault(ActionType.RADIO, ViewTarget.DROP_DOWN,
                (converter, action, dockable) -> new SelectableDropDownHandler(action, dockable,
                        new JRadioButtonMenuItem()));

        putDefault(ActionType.SEPARATOR, ViewTarget.DROP_DOWN, (converter, action, dockable) -> {
        ...
        putDefault(ActionType.DROP_DOWN, ViewTarget.DROP_DOWN,
                (converter, action, dockable) -> new SubDropDownHandler(action.createView(ViewTarget.MENU, converter,
                        dockable)));

        putDefault(ActionType.MENU, ViewTarget.DROP_DOWN,
                (converter, action, dockable) -> new SubDropDownHandler(action.createView(ViewTarget.MENU, converter,
                        dockable)));
    }
     ...
     */
    public <A, D extends DockAction> void putClient(ActionType<D> action, ViewTarget<A> target,
     ...
     * @param generator the generator to store, may be <code>null</code>
     */
    public <A, D extends DockAction> void putTheme(ActionType<D> action, ViewTarget<A> target,
                                                   ViewGenerator<D, A> generator) {
        if (action == null) {
            throw new IllegalArgumentException("Action must not be null");
        }
     ...
     * @param generator the generator to store, may be <code>null</code>
     */
    public <A, D extends DockAction> void putDefault(ActionType<D> action, ViewTarget<A> target,
                                                     ViewGenerator<D, A> generator) {
        if (action == null) {
            throw new IllegalArgumentException("Action must not be null");
        }
     ...
     * @throws IllegalArgumentException if an unknown argument is used
     */
    public <A, D extends DockAction> A createView(ActionType<D> type, D action, ViewTarget<A> target,
                                                  Dockable dockable) {
    ...
    private <A, D extends DockAction> Entry<D, A> getEntry(ActionType<D> action, ViewTarget<? super A> target) {
        ...
        if (result == null) {
            ...
            converters.put(new Tuple<>(action, target), result);
        }
