 ...
 */
public abstract class SimpleSelectableAction extends SimpleDropDownItemAction implements SharingSelectableDockAction,
                                                                                         SelectableDockAction {
    /**
     * observers of this action
     */
    private final List<SelectableDockActionListener> listeners = new ArrayList<>();
    /**
     ...
     */
    private final Map<ActionContentModifier, Icon> selectIcons = new HashMap<>();
    ...
    protected void fireSelectedChanged() {
        ...
        for (SelectableDockActionListener listener : listeners.toArray(new SelectableDockActionListener[0])) {
    ...
    public ActionContentModifier[] getIconContexts(Dockable dockable) {
        Set<ActionContentModifier> result = new HashSet<>();
        result.addAll(Arrays.asList(super.getIconContexts(dockable)));
        result.addAll(selectIcons.keySet());
        return result.toArray(new ActionContentModifier[0]);
    }
