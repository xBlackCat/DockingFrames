 ...
 * 	<li>Due to this delay, this listener may seem to receive events too late. However, once all events are received,
 * 	the
...
public class DockableStateListenerManager {
     ...
     */
    private final List<DockableStateListener> listeners = new ArrayList<>();
     ...
     */
    private final DockableStateListener dockableStateListener = new DockableStateListener() {
     ...
     */
    private final DockHierarchyListener dockHierarchyListener = new DockHierarchyListener() {
     ...
     */
    private final DockStationListener dockStationListener = new DockStationAdapter() {
    ...
    private void fireNow() {
        if (current != 0) {
            if (listeners.size() > 0) {
                ...
                current = 0;
                for (DockableStateListener listener : listeners.toArray(new DockableStateListener[0])) {
