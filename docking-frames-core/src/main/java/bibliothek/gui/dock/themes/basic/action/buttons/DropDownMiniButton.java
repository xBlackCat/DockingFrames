    ...
    public void doLayout() {
        ...
        if (getModel().getOrientation().isHorizontal()) {
            ...
            content.setBounds(
                    ...
                    getWidth() - insets.left - 2 * insets.right - 1 - dropWidth,
                    getHeight() - insets.top - insets.bottom);
        } else {
            int dropHeight = dropIcon.getIconHeight();
            content.setBounds(insets.left,
                    insets.top + 1,
                    getWidth() - insets.left - insets.right,
                    getHeight() - insets.top - 2 * insets.bottom - 1 - dropHeight);
        }
    }
...
//						insets.top+(getHeight()-insets.top-insets.bottom-icon.getIconHeight()) / 2 );
            ...
            drop.paintIcon(this, g, (int) (factor * (insets.left + insets.right + iconWidth + dropWidth / 2) -
                            dropWidth / 2),
...
//						(int)(factor * (insets.top + iconHeight/2) - iconHeight/2 ));
            ...
            drop.paintIcon(this, g,
                    insets.left + (getWidth() - insets.left - insets.right - dropIcon.getIconWidth()) / 2,
        ...
        if (border != null) {
            ...
            if (getModel().getOrientation().isHorizontal()) {
                ...
                if (getModel().isMouseOverDropDown()) {
                    border.paintBorder(this, g, 0, 0, (int) (factor *
                            (insets.left + insets.right + iconWidth)), getHeight());
                }
            } else {
                ...
                if (getModel().isMouseOverDropDown()) {
                    border.paintBorder(this, g, 0, 0, getWidth(), (int) (factor *
                            (insets.top + insets.bottom + iconHeight)));
                }
            }
        }
    }
