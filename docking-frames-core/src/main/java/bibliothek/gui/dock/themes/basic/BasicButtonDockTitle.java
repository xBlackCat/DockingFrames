...
public class BasicButtonDockTitle extends AbstractDockTitle {
     ...
     */
    protected final ButtonContentValue behavior;

    /**
     * tells what items to filter
     */
    private final PropertyValue<ButtonContentFilter> connector =
            new PropertyValue<>(FlapDockStation.BUTTON_CONTENT_FILTER) {
     ...
     */
    private final boolean filterActions = false;

    /**
     * the color used for the background when active
     */
    private final TitleColor activeColor = new BasicTitleColor("title.flap.active", null);
    /**
     * the color used for the foreground when active
     */
    private final TitleColor activeTextColor = new BasicTitleColor("title.flap.active.text", null);
    /**
     * the color used for background when inactive
     */
    private final TitleColor inactiveColor = new BasicTitleColor("title.flap.inactive", null);
    /**
     * the color used for foreground when inactive
     */
    private final TitleColor inactiveTextColor = new BasicTitleColor("title.flap.inactive.text", null);
    /**
     * the color used for background when selected
     */
    private final TitleColor selectedColor = new BasicTitleColor("title.flap.selected", null);
    /**
     * the color used for foreground when selected
     */
    private final TitleColor selectedTextColor = new BasicTitleColor("title.flap.selected.text", null);

    /**
     * the color used for the bright side of the knob if active
     */
    private final TitleColor knobActiveHighlightColor = new BasicTitleColor("title.flap.active.knob.highlight", null);
    /**
     * the color used for the dark side of the knob if active
     */
    private final TitleColor knobActiveShadowColor = new BasicTitleColor("title.flap.active.knob.shadow", null);
    /**
     * the color used for the bright side of the knob if inactive
     */
    private final TitleColor knobInactiveHighlightColor =
            new BasicTitleColor("title.flap.inactive.knob.highlight", null);
    /**
     * the color used for the dark side of the knob if inactive
     */
    private final TitleColor knobInactiveShadowColor = new BasicTitleColor("title.flap.inactive.knob.shadow", null);
    /**
     * the color used for the bright side of the knob if selected
     */
    private final TitleColor knobSelectedHighlightColor =
            new BasicTitleColor("title.flap.selected.knob.highlight", null);
    /**
     * the color used for the dark side of the knob if selected
     */
    private final TitleColor knobSelectedShadowColor = new BasicTitleColor("title.flap.selected.knob.shadow", null);

    /**
     * keeps all the {@link DockActionSource}s that have to be shown on this title
     */
    private final MultiDockActionSource allActionsSource = new MultiDockActionSource();
    ...
    private void updateActionSource(boolean force) {
        ...
        if (force || this.showChildren != showChildren || this.showActions != showActions ||
    ...
    public void changed(DockTitleEvent event) {
        if (event instanceof ActivityDockTitleEvent activity) {
    ...
    protected void changeBorder() {
        if (selected && mousePressed) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.selected.pressed", BorderFactory.createBevelBorder(BevelBorder.RAISED));
        } else if (selected) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.selected", BorderFactory.createBevelBorder(BevelBorder.LOWERED));
        } else if (mousePressed) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.pressed", BorderFactory.createBevelBorder(BevelBorder.LOWERED));
        } else {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button", BorderFactory.createBevelBorder(BevelBorder.RAISED));
        }
    }
