...
public class DefaultColorScheme extends AbstractColorScheme {
    ...
    private final Map<Path, ColorBridgeFactory> bridges = new HashMap<>();
    ...
    public void setBridgeFactory(final Path kind, ColorBridgeFactory bridge) {
        ...
        UISchemeEvent<Color, DockColor, ColorBridge> event = new UISchemeEvent<>() {
     ...
     * @return a color that matches id or <code>null</code>, the color may not be stored with the exact identifier
     * <code>id</code>
     */
