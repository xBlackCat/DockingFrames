...
public class MenuMenuHandler extends AbstractMenuHandler<JMenu, MenuDockAction> {
     ...
     */
    private final Listener sourceListener = new Listener();
    ...
    protected void fireActionEvent(ActionEvent event) {
        for (ActionListener listener : listeners.toArray(new ActionListener[0])) {
    ...
    protected MenuViewItem<JComponent> handlerFor(DockAction action) {
        ...
        MenuViewItem<JComponent> result =
                dockable.getController().getActionViewConverter().createView(action, target, dockable);
     ...
     *
     * @param menu the menu wrapped by this object
     * @author Benjamin Sigg
     */
    private record JMenuWrapper(JMenu menu) implements Menu {
     ...
     *
     * @param menu the menu covered by this wrapper
     * @author Benjamin Sigg
     */
    private record JPopupMenuWrapper(JPopupMenu menu) implements Menu {
