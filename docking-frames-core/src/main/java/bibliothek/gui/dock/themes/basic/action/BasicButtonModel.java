...
public class BasicButtonModel {
     ...
     */
    private final Map<ActionContentModifier, Icon> icons = new HashMap<>();

    /**
     * automatically created icons used when this model is not enabled
     */
    private final Map<ActionContentModifier, Icon> disabledIcons = new HashMap<>();
     ...
     */
    private final BasicResourceInitializer initializer;
     ...
     */
    private final List<BasicButtonModelListener> listeners = new ArrayList<>();

    /**
     * a list of borders to use by the associated button
     */
    private final Map<String, BorderModifier> borders = new HashMap<>();
    ...
    protected List<Triple<KeyStroke, String, Action>> listActions() {
        List<Triple<KeyStroke, String, Action>> actions = new ArrayList<>();

        Triple<KeyStroke, String, Action> select = new Triple<>();
        ...
        Triple<KeyStroke, String, Action> trigger = new Triple<>();
    ...
    protected BasicButtonModelListener[] listeners() {
        return listeners.toArray(new BasicButtonModelListener[0]);
    }
     ...
     *                            <code>background</code> is not <code>null</code>, must represents
     *                            {@link #getOwner()} as {@link Component}.
     */
    ...
    public ActionContentModifier[] getIconContexts() {
        return icons.keySet().toArray(new ActionContentModifier[0]);
    }
    ...
    public Icon getPaintIcon(boolean enabled) {
        ...
        List<ActionContentModifier> modifiers = new LinkedList<>();
        ...
        while (!modifiers.isEmpty()) {
            ...
            modifiers.addAll(Arrays.asList(modifier.getBackup()));
        }
