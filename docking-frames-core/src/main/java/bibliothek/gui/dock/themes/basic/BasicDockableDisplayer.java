...
public class BasicDockableDisplayer extends ConfiguredBackgroundPanel implements DockableDisplayer {
     ...
     */
    private final List<DockableDisplayerListener> listeners = new ArrayList<>();
     ...
     */
    private final SingleTabDeciderListener singleTabListener = (source, dockable) -> {
     ...
     */
    private final PropertyValue<SingleTabDecider> decider = new PropertyValue<>(SingleTabDecider.SINGLE_TAB_DECIDER) {
        @Override
     ...
     */
    private final BasicDockableDisplayerDecoratorListener decoratorListener = decorator -> fireMoveableElementChanged();
     ...
     *
     *
     *        {@link #init(DockStation, Dockable, DockTitle, bibliothek.gui.dock.station.DockableDisplayer.Location)
     *        init}
     *                   will be called.
     */
...
//    	content.setOpaque( false );
    ...
    /**
     * Creates the {@link DockComponentRootHandler} for this displayer. The root handler is required to inform the
     * client
    ...
    protected BasicDockableDisplayerDecorator createStackedDecorator(final PropertyKey<DockActionDistributor> distributor) {
        return new MinimalDecorator() {
            private final DockActionDistributorSource source =
                    new DockActionDistributorSource(Target.TITLE, distributor);
    ...
    protected DockableDisplayerListener[] listeners() {
        return listeners.toArray(new DockableDisplayerListener[0]);
    }
    ...
    protected DockTitle.Orientation orientation(Location location) {
        return switch (location) {
            case TOP -> DockTitle.Orientation.NORTH_SIDED;
            case BOTTOM -> DockTitle.Orientation.SOUTH_SIDED;
            case LEFT -> DockTitle.Orientation.WEST_SIDED;
            case RIGHT -> DockTitle.Orientation.EAST_SIDED;
        };
    ...
    public DisplayerCombinerTarget prepareCombination(CombinerSource source, Enforcement force) {
        if (decorator instanceof TabDecorator) {
            TabDisplayerCombinerTarget target = new TabDisplayerCombinerTarget(this,
                    ((TabDecorator) decorator).getStackComponent(), source, force);
            if (target.isValid()) {
                return target;
            }
        }
