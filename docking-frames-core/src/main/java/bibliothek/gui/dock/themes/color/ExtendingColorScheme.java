...
public class ExtendingColorScheme extends AbstractColorScheme {
    ...
    private final ColorScheme[] extensions;

    private final UISchemeListener<Color, DockColor, ColorBridge> delegateListener = event -> {
        UISchemeEvent<Color, DockColor, ColorBridge> forward = new UISchemeEvent<>() {
    ...
    public ExtendingColorScheme(ColorScheme scheme, DockController controller) {
        ExtensionName<ColorScheme> name = new ExtensionName<>(
        ...
        this.scheme = scheme;
        this.extensions = extensions.toArray(new ColorScheme[0]);
    }
