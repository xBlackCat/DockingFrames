...
public class TabDecorator implements BasicDockableDisplayerDecorator, StackDockComponentParent {
     ...
     */
    private final StackDockComponentListener componentListener = new StackDockComponentListener() {
    ...
    private final PropertyValue<StackDockComponentFactory> factory =
            new PropertyValue<>(StackDockStation.COMPONENT_FACTORY) {
                @Override
                protected void valueChanged(StackDockComponentFactory oldValue, StackDockComponentFactory newValue) {
                    ...
                    if (newValue != null) {
                        ...
                        if (dockable != null) {
                            component.addTab(dockable.getTitleText(), dockable.getTitleIcon(), representation,
                                    dockable);
    ...
    private final PropertyValue<TabPlacement> tabPlacement =
            new PropertyValue<>(StackDockStation.TAB_PLACEMENT) {
    ...
    private final PropertyValue<TabContentFilter> filter = new PropertyValue<>(StackDockStation.TAB_CONTENT_FILTER) {
        @Override
    ...
    private final TabContentFilterListener filterListener = new TabContentFilterListener() {
    ...
    private final StackDockComponentRepresentative representative = new StackDockComponentRepresentative();
    private DockActionDistributorSource actions;
    private final List<BasicDockableDisplayerDecoratorListener> listeners = new ArrayList<>(2);
    ...
    protected void fireMoveableElementChanged() {
        for (BasicDockableDisplayerDecoratorListener listener :
                listeners.toArray(new BasicDockableDisplayerDecoratorListener[0])) {
    ...
    public Insets getDockableInsets() {
        ...
        return new Insets(location.y, location.x,
                size.height - used.height - location.y, size.width - used.width - location.x);
    }
}
