...
public class BasicDropDownButtonHandler extends AbstractBasicHandler<DropDownAction, BasicDropDownButtonModel> implements BasicDropDownButtonTrigger, BasicTitleViewItem<JComponent> {
     ...
     */
    private final List<DockAction> actions = new ArrayList<>();
    /**
     * the views for the items of {@link #actions}. Not all actions have a view.
     */
    private final Map<DockAction, DropDownItemHandle> items = new HashMap<>();
     ...
     */
    private final ButtonView buttonView = new ButtonView();
    /**
     ...
     */
    private final PropertyValue<ButtonContentFilter> buttonContentFilter =
            new PropertyValue<>(DockAction.BUTTON_CONTENT_FILTER) {
                @Override
     ...
     */
    private final ButtonContentFilterListener buttonContentFilterListener = (filter, dockable, action) -> {
    ...
    private void add(int index, DockAction action) {
        ...
        DropDownViewItem item =
                action.createView(ViewTarget.DROP_DOWN, dockable.getController().getActionViewConverter(), dockable);
        if (item != null) {
    ...
    protected class ButtonView implements DropDownView {
        ...
        /**
         * Updates the text that is shown by the button, respects the
         * {@link BasicDropDownButtonHandler#buttonContentFilter}.
         */
    ...
    private class Listener implements StandardDockActionListener, DropDownActionListener, DockActionSourceListener {
        ...
        public void actionIconChanged(StandardDockAction action, ActionContentModifier modifier,
