...
public abstract class AbstractColorScheme implements ColorScheme {
     ...
     */
    private final List<UISchemeListener<Color, DockColor, ColorBridge>> listeners = new ArrayList<>();

    /**
     * all the managers using this scheme
     */
    private final List<ColorManager> managers = new ArrayList<>();

    /**
     * calls {@link #updateUI()} if the look and feel changed
     */
    private final LookAndFeelColorsListener updateUiListener = new LookAndFeelColorsListener() {
    ...
    protected UISchemeListener<Color, DockColor, ColorBridge>[] listeners() {
        return listeners.toArray(new UISchemeListener[0]);
    }
    ...
    protected ColorManager[] managers() {
        return managers.toArray(new ColorManager[0]);
    }
