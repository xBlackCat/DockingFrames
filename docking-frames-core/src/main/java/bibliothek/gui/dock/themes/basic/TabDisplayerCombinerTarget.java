...
public class TabDisplayerCombinerTarget implements DisplayerCombinerTarget {
     ...
     */
    private final DockableDisplayer displayer;

    /**
     * the stack component used to paint a single dockable
     */
    private final StackDockComponent stack;
     ...
     */
    public TabDisplayerCombinerTarget(DockableDisplayer displayer, StackDockComponent stack, CombinerSource source,
                                      Enforcement force) {
    ...
    public void paint(Graphics g, Component component, StationPaint paint, Rectangle stationBounds,
        ...
        if (stack.getDockTabPlacement().isHorizontal()) {
            if (index == 0) {
                ...
                paint.drawInsertionLine(g, displayer.getStation(),
                        tab.x + tab.width, tab.y, tab.x + tab.width, tab.y + tab.height);
            }
        } else {
            if (index == 0) {
                paint.drawInsertionLine(g, displayer.getStation(), tab.x, tab.y, tab.x + tab.width, tab.y);
            } else {
                paint.drawInsertionLine(g, displayer.getStation(), tab.x,
                        tab.y + tab.height, tab.x + tab.width, tab.y + tab.height);
            }
        }
    }
