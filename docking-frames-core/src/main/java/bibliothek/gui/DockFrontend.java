...
public class DockFrontend {
    ...
    public static final PropertyKey<KeyStroke> HIDE_ACCELERATOR =
            new PropertyKey<>("frontend hide accelerator");
     ...
     */
    private final Map<String, DockInfo> dockables = new HashMap<>();
    /**
     ...
     */
    private final Map<String, RootInfo> roots = new HashMap<>();

    /**
     * A set of factories needed to store Dockables
     */
    private final Set<DockFactory<?, ?, ?>> dockFactories =
            new HashSet<>();

    /**
     * A set of factories needed to read Dockables that are missing in the cache
     */
    private final Set<DockFactory<? extends Dockable, ?, ?>> backupDockFactories =
            new HashSet<>();

    /**
     * A set of factories needed to store additional information about Dockables
     */
    private final Set<AdjacentDockFactory<?>> adjacentDockFactories =
            new HashSet<>();

    /**
     * A set of factories needed to store {@link DockableProperty properties}
     */
    private final Set<DockablePropertyFactory> propertyFactories = new HashSet<>();
     ...
     * A list of observers
     */
    private final List<DockFrontendListener> listeners = new ArrayList<>();
    ...
    public DockFrontend(DockController controller, WindowProvider owner) {
        ...
        extensions =
                controller.getExtensions().load(new ExtensionName<>(FRONTEND_EXTENSION, DockFrontendExtension.class));
    ...
    public void setMissingDockableStrategy(MissingDockableStrategy missingDockable) {
        this.missingDockable = Objects.requireNonNullElse(missingDockable, MissingDockableStrategy.DISCARD_ALL);
    }
     ...
     *
     * @return the new transformer, created by the current {@link #setLayoutChangeStrategy(LayoutChangeStrategy)
    ...
    public Map<String, Dockable> getNamedDockables() {
        Map<String, Dockable> result = new HashMap<>();
    ...
    public String[] getRootNames() {
        return roots.keySet().toArray(new String[0]);
    }
    ...
    public void removeRoot(DockStation station) {
        ...
        if (info != null) {
            if (defaultStation == info.station()) {
                defaultStation = null;
            }

            roots.remove(info.name());
     ...
     * {@link #isHiddenRootStation(DockElement)} is <code>true</code> or if {@link #isShown(Dockable)} is <code>false
     * </code>.<br>
     *
     ...
     *              as much information as possible should be read. Should be the same value as was used for
     *              {@link #save(String, boolean)}
     * @throws IllegalArgumentException if no setting <code>name</code> could be found
     */
    ...
    public void setSetting(Setting setting, boolean entry) {
        ...
        } catch (IOException | XException e) {
    ...
    private void fillMissing() {
        ...
        if (lastAppliedEntrySetting != null && lastAppliedEntrySetting != lastAppliedFullSetting &&
                !settings.containsValue(lastAppliedEntrySetting)) {
    ...
    private void fillMissing(String root, DockLayoutComposition composition, DockFactory<?, ?, ?> factory,
                             String factoryId) {
        ...
        if (info.getKind() == DockLayoutInfo.Data.DOCK_LAYOUT) {
            if (info.getDataLayout().getFactoryID().equals(factoryId)) {
                DockableProperty location = info.getLocation();
                if (location != null) {
                    DockFactory<DockElement, ?, Object> normalizedFactory =
                            (DockFactory<DockElement, ?, Object>) factory;
                    if (missingDockable.shouldCreate(normalizedFactory, info.getDataLayout().getData())) {
                        DockElement element =
                                normalizedFactory.layout(info.getDataLayout().getData(),
                                        layoutChangeStrategy.getPlaceholderStrategy(new Internals()));
                        if (element != null) {
                            Dockable dockable = element.asDockable();
                            if (dockable != null) {
                                RootInfo rootInfo = roots.get(root);
                                if (!rootInfo.station().drop(dockable, location)) {
                                    rootInfo.station().drop(dockable);
                                }
                            }
                        }
                    }
    ...
    public List<Dockable> listDockables() {
        List<Dockable> result = new ArrayList<>(dockables.size());
        for (DockInfo info : dockables.values()) {
            if (info.getDockable() != null) {
    ...
    public List<FrontendEntry> listFrontendEntries() {
        return new ArrayList<>(dockables.values());
    }
    ...
    protected void clean(DockSituationIgnore ignore) {
        for (RootInfo root : roots.values()) {
            if (!ignore.ignoreElement(root.station())) {
                clean(root.station(), ignore);
            }
        }
    }
    ...
    public int deleteAll() {
        ...
        String[] array = settings.toArray(new String[0]);
    ...
    protected DockFrontendListener[] listeners() {
        return listeners.toArray(new DockFrontendListener[0]);
    }
    ...
    private class Internals implements DockFrontendInternals {
        ...
        public AdjacentDockFactory<?>[] getAdjacentDockFactories() {
            return adjacentDockFactories.toArray(new AdjacentDockFactory[0]);
        }

        public DockFactory<?, ?, ?>[] getBackupDockFactories() {
            return backupDockFactories.toArray(new DockFactory[0]);
        }

        public DockFactory<?, ?, ?>[] getDockFactories() {
            return dockFactories.toArray(new DockFactory[0]);
        }

        public DockInfo[] getDockables() {
            return dockables.values().toArray(new DockInfo[0]);
        }
        ...
        public DockablePropertyFactory[] getPropertyFactories() {
            return propertyFactories.toArray(new DockablePropertyFactory[0]);
        }

        public RootInfo[] getRoots() {
            return roots.values().toArray(new RootInfo[0]);
        }
    ...
    public class DockInfo implements FrontendEntry {
        ...
        public DockInfo(Dockable dockable, String key) {
            ...
            source =
                    new DefaultDockActionSource(new LocationHint(LocationHint.ACTION_GUARD, LocationHint.RIGHT_OF_ALL));
        ...
        public void updateLocation() {
            ...
            root = info.name();
            location = DockUtilities.getPropertyChain(info.station(), dockable);
        }
     ...
     * @author Benjamin Sigg
     */
    public record RootInfo(DockStation station, String name) {}
    ...
    public class Hider extends SimpleButtonAction implements ActionGuard {
        private final DockActionIcon icon;

        private final DockActionText text;
        private final DockActionText tooltip;
        ...
        public Hider() {
            ...
            PropertyValue<KeyStroke> stroke = new PropertyValue<>(HIDE_ACCELERATOR) {
                @Override
