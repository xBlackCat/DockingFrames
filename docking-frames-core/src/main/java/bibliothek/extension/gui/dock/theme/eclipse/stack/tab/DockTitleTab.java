...
public class DockTitleTab implements TabComponent, EclipseTabStateInfo {
    ...
    public static final TabPainter createFactory(final TabPainter fallback) {
        return new TabPainter() {
            public TabComponent createTabComponent(EclipseTabPane pane, Dockable dockable) {
                ...
                DockTitleVersion version = controller == null ? null :
                        controller.getDockTitleManager().getVersion(EclipseTheme.TAB_DOCK_TITLE);
                if (version == null) {
     ...
     */
    private final DockTitleRequest title;
    /**
    ...
    private final MouseInputListener mouseListener = new MouseInputListener() {
        public void mouseClicked(MouseEvent e) {
            for (MouseListener m : mouseListeners.toArray(new MouseListener[0])) {
                m.mouseClicked(e);
            }
        }
        ...
        public void mouseDragged(MouseEvent e) {
            for (MouseMotionListener m : mouseMotionListeners.toArray(new MouseMotionListener[0])) {
                m.mouseDragged(e);
            }
        }
    ...
    private void updateOrientation() {
        ...
        if (placement != null && title != null) {
            switch (placement) {
                case TOP_OF_DOCKABLE -> title.setOrientation(Orientation.NORTH_SIDED);
                case BOTTOM_OF_DOCKABLE -> title.setOrientation(Orientation.SOUTH_SIDED);
                case LEFT_OF_DOCKABLE -> title.setOrientation(Orientation.WEST_SIDED);
                case RIGHT_OF_DOCKABLE -> title.setOrientation(Orientation.EAST_SIDED);
            }
        }
    }
    ...
    protected void fire() {
        ...
        if (answer != null) {
            EclipseDockTitleEvent eclipseEvent =
                    new EclipseDockTitleEvent(station, dockable, selected, focused, paintIconWhenInactive);
            answer.changed(eclipseEvent);

            DockActionSource actions =
                    new EclipseDockActionSource(theme, dockable.getGlobalActionOffers(), getEclipseTabStateInfo(),
                            true);
