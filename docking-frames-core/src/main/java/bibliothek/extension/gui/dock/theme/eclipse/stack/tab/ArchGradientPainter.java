...
public class ArchGradientPainter extends BaseTabComponent {
    ...
    public Insets getOverlap(TabComponent other) {
        if (other instanceof ArchGradientPainter painter) {
    ...
    public void paintBackground(Graphics g) {
        ...
        if (isSelected()) {
            switch (getOrientation()) {
                case TOP_OF_DOCKABLE, LEFT_OF_DOCKABLE -> paintSelected(g2d, color1, color2);
                case BOTTOM_OF_DOCKABLE, RIGHT_OF_DOCKABLE -> paintSelected(g2d, color2, color1);
            }
        } else {
            if (getTransparency() != Transparency.TRANSPARENT) {
                GradientPaint gradient =
                        color1.equals(color2) ? null : new GradientPaint(x, y, color1, x, y + h, color2);
                Paint old = g2d.getPaint();
                g2d.setPaint(Objects.requireNonNullElse(gradient, color1));
    ...
    private void paintSelected(Graphics g, Color top, Color bottom) {
        ...
        Paint old = g2d.getPaint();
        g2d.setPaint(Objects.requireNonNullElse(gradient, top));
        ...
        switch (getOrientation()) {
            case TOP_OF_DOCKABLE -> g.drawLine(0, h - 1, w - 1, h - 1);
            case BOTTOM_OF_DOCKABLE -> g.drawLine(0, 0, w - 1, 0);
            case LEFT_OF_DOCKABLE -> g.drawLine(w - 1, 0, w - 1, h - 1);
            case RIGHT_OF_DOCKABLE -> g.drawLine(0, 0, 0, h - 1);
        }
        ...
        switch (getOrientation()) {
            case TOP_OF_DOCKABLE, LEFT_OF_DOCKABLE -> g.drawRect(x + w - 1, y + h - 1, 0, 0);
            case RIGHT_OF_DOCKABLE -> g.drawRect(x, y + h - 1, 0, 0);
            case BOTTOM_OF_DOCKABLE -> g.drawRect(x + w - 1, y, 0, 0);
        }
