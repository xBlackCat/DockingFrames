...
public class RoundDropDownButton extends JComponent implements RoundButtonConnectable {
     ...
     */
    private final BasicDropDownButtonModel model;

    /**
     * a handler reacting if this button is pressed
     */
    private final BasicDropDownButtonHandler handler;
     ...
     */
    private final RoundActionColor[] colors;
    ...
    public RoundDropDownButton(BasicDropDownButtonHandler handler, Dockable dockable, DockAction action) {
        ...
        colors = new RoundActionColor[]{
                ...
                new RoundActionColor("action.dropdown.pressed.selected.enabled.focus", dockable, action,
                        Color.DARK_GRAY),
                ...
                new RoundActionColor("action.dropdown.line.pressed.selected.enabled", dockable, action,
                        Color.DARK_GRAY),
        ...
        animation.addTask(this::repaint);
    ...
    public Dimension getPreferredSize() {
        ...
        if (model.getOrientation().isHorizontal()) {
            return new Dimension((int) (0.5 * contentSize.height + contentSize.width + 1 +
                    1.5 * dropIcon.getIconWidth()), (int) (1.5 * contentSize.height));
        } else {
            return new Dimension((int) (1.5 * contentSize.width), (int) (0.5 * contentSize.width + contentSize.height +
                    1 + 1.5 * dropIcon.getIconHeight()));
        }
    }
    ...
    public void doLayout() {
        if (model.getOrientation().isHorizontal()) {
            content.setBounds(
                    getHeight() / 3,
                    getHeight() / 6, (int) (getWidth() - 1 - 1.5 * dropIcon.getIconWidth()), (int) (getHeight() / 1.5));
        } else {
            content.setBounds(
                    getWidth() / 3,
                    getWidth() / 6, (int) (getWidth() / 1.5), (int) (getHeight() - 1 - 1.5 * dropIcon.getIconHeight()));
        }
    }
