...
public class FlatButtonTitle extends BasicButtonDockTitle {
     ...
     */
    private final MouseOverListener mouseover;
    ...
    protected void changeBorder() {
        ...
        if (selected && mousePressed) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.flat.selected.pressed", BorderFactory.createEtchedBorder(EtchedBorder.LOWERED));
        } else if (selected && mouseover) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.flat.selected.hover", BorderFactory.createBevelBorder(BevelBorder.LOWERED));
        } else if (selected) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.flat.selected", BorderFactory.createBevelBorder(BevelBorder.LOWERED));
        } else if (mousePressed) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.flat.pressed", BorderFactory.createBevelBorder(BevelBorder.LOWERED));
        } else if (mouseover) {
            setBorder(ThemeManager.BORDER_MODIFIER +
                    ".title.button.flat.hover", BorderFactory.createEtchedBorder(EtchedBorder.LOWERED));
        } else {
