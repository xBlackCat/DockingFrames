...
public class SmoothDefaultButtonTitle extends BasicButtonDockTitle {
     ...
     */
    private final SmoothChanger changer = new SmoothChanger(3) {
        @Override
    ...
    protected void updateForegroundColor() {
        if (changer != null && changer.isRunning() && current != null) {
            setForeground(triColor(foreground(getActiveTextColor()), foreground(getSelectedTextColor()),
    ...
    protected void updateBackgroundColor() {
        if (changer != null && changer.isRunning() && current != null) {
            setBackground(triColor(background(getActiveColor()), background(getSelectedColor()),
                    background(getInactiveColor())));
        } else {
            if (isActive()) {
