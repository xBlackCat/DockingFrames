...
public abstract class AbstractBubbleDockTitle extends AbstractDockTitle {
     ...
     */
    private final List<BubbleTitleColor> colors = new ArrayList<>();
     ...
     * @param dockable        the {@link Dockable} for which this title is shown
     * @param origin          the creator of this title
     * @param showMiniButtons whether this title should show the {@link bibliothek.gui.dock.action.DockAction actions
     *                        } or not
     */
    ...
    protected Insets getInnerInsets() {
        ...
        return switch (getOrientation()) {
            case EAST_SIDED -> new Insets(edge, edge / 2, edge, edge);
            case FREE_HORIZONTAL -> new Insets(edge, edge, edge, edge);
            case FREE_VERTICAL -> new Insets(edge, edge, edge, edge);
            case NORTH_SIDED -> new Insets(edge, edge, edge / 2, edge);
            case SOUTH_SIDED -> new Insets(edge / 2, edge, edge, edge);
            case WEST_SIDED -> new Insets(edge, edge, edge, edge / 2);
        };
    }
    ...
    private void drawRoundRect(Graphics2D g2, int x, int y, int w, int h) {
        switch (getOrientation()) {
            case FREE_HORIZONTAL, FREE_VERTICAL -> g2.fillRoundRect(x, y, w, h, arc, arc);
            case EAST_SIDED -> g2.fillRoundRect(x - arc, y, w + arc, h, arc, arc);
            case NORTH_SIDED -> g2.fillRoundRect(x, y, w, h + arc, arc, arc);
            case SOUTH_SIDED -> g2.fillRoundRect(x, y - arc, w, h + arc, arc, arc);
            case WEST_SIDED -> g2.fillRoundRect(x, y, w + arc, h, arc, arc);
        }
    }
