...
public class InspectionTree implements TreeModel {
    ...
    private final InspectionGraph graph;
    ...
    protected TreeModelListener[] listeners() {
        return listeners.toArray(new TreeModelListener[0]);
    }
    ...
    public void update(boolean cleanse) {
        graph.updateAll();
        if (cleanse) {
            Set<InspectionNode> set = new HashSet<>();
            root.collect(set);
            graph.retainAll(set);
        }
    }
    ...
    private class Node implements InspectionNodeListener {
        ...
        private final InspectionNode content;
