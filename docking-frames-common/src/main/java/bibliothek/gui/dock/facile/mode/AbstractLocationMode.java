...
public abstract class AbstractLocationMode<A extends ModeArea> implements Iterable<A>, LocationMode {
     ...
     */
    private final Map<String, A> areas = new HashMap<>();

    /**
     * The order in which the areas were added
     */
    private final List<A> areaOrder = new LinkedList<>();
     ...
     */
    private final List<LocationModeListener> listeners = new ArrayList<>();
     ...
     */
    private final AreaListener modeAreaListener = new AreaListener();

    /**
     * listener to register new dockables and remove old ones
     */
    private final ModeManagerListener<Location, LocationMode> managerListener = new ManagerListener();

    /**
     * temporary information associated with the currently registered Dockables
     */
    private final Map<Dockable, DockableHandle> handles = new HashMap<>();
    ...
    public void setActionProvider(LocationModeActionProvider actionProvider) {
        ...
        if (!handles.isEmpty()) {
            throw new IllegalStateException("can only set actionProvider if no Dockables are currently showing " +
                    "actions of the old provider");
        }
    ...
    protected LocationModeListener[] listeners() {
        return listeners.toArray(new LocationModeListener[0]);
    }
    ...
    public Iterator<A> iterator() {
        List<A> copy = new ArrayList<>(areas.values());
