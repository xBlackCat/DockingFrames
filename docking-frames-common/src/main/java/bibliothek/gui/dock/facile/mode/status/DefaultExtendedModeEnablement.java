...
public class DefaultExtendedModeEnablement extends AbstractExtendedModeEnablement {
     ...
     */
    public static final ExtendedModeEnablementFactory FACTORY = DefaultExtendedModeEnablement::new;

    /**
     * added to any {@link CDockable}
     */
    private final CDockablePropertyListener listener = new CDockableAdapter() {
        public void minimizableChanged(CDockable dockable) {
            fire(dockable.intern(), ExtendedMode.MINIMIZED,
                    isAvailable(dockable.intern(), ExtendedMode.MINIMIZED).isAvailable());
        }

        public void maximizableChanged(CDockable dockable) {
            fire(dockable.intern(), ExtendedMode.MAXIMIZED,
                    isAvailable(dockable.intern(), ExtendedMode.MAXIMIZED).isAvailable());
        }

        public void externalizableChanged(CDockable dockable) {
            fire(dockable.intern(), ExtendedMode.EXTERNALIZED, isAvailable(dockable.intern(),
                    ExtendedMode.EXTERNALIZED).isAvailable());
        }

        public void normalizeableChanged(CDockable dockable) {
            fire(dockable.intern(), ExtendedMode.NORMALIZED,
                    isAvailable(dockable.intern(), ExtendedMode.NORMALIZED).isAvailable());
        }
    };
     ...
     */
    private final ExtendedModeEnablementListener extensionListener =
            (dockable, mode, available) -> fire(dockable, mode, isAvailable(dockable, mode).isAvailable());

    /**
     * Additional rules loaded from the {@link ExtensionManager}
     */
    private final ExtendedModeEnablement[] extensions;
    ...
    public DefaultExtendedModeEnablement(LocationModeManager<?> manager) {
        super(manager);
        init();
        List<ExtendedModeEnablementFactory> factories =
                manager.getController().getExtensions().load(new ExtensionName<>(EXTENSION,
                        ExtendedModeEnablementFactory.class));
