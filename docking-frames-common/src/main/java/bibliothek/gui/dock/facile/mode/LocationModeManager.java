...
public class LocationModeManager<M extends LocationMode> extends ModeManager<Location, M> {
     ...
     */
    private final Map<Path, List<LocationModeListener>> listeners = new HashMap<>();
     ...
     */
    private final RelocatorListener relocatorListener = new RelocatorListener();
     ...
     */
    private final List<CGroupMovement> currentAction = new ArrayList<>();

    /**
     * the list of {@link Dockable}s for which {@link #refresh(Dockable, boolean)} has to be called
     */
    private final LinkedHashSet<Dockable> pendingRefreshs = new LinkedHashSet<>();

    /**
     * the current {@link ExtendedModeEnablementFactory}
     */
    private final PropertyValue<ExtendedModeEnablementFactory> extendedModeFactory =
            new PropertyValue<>(MODE_ENABLEMENT) {
                @Override
                protected void valueChanged(ExtendedModeEnablementFactory oldValue,
                                            ExtendedModeEnablementFactory newValue) {
                    updateEnablement();
                }
            };
     ...
     */
    private final PropertyValue<DoubleClickLocationStrategy> doubleClickStrategy =
            new PropertyValue<>(DOUBLE_CLICK_STRATEGY) {
                @Override
                protected void valueChanged(DoubleClickLocationStrategy oldValue,
                                            DoubleClickLocationStrategy newValue) {
                    // ignore
                }
            };
     ...
     */
    private final ExtendedModeEnablementListener enablementListener =
            (dockable, mode, available) -> refresh(dockable, true);
    ...
    private final DoubleClickListener doubleClickListener = new DoubleClickListener() {
        ...
        public boolean process(Dockable dockable, MouseEvent event) {
            ...
            if (dockable != null) {
                ...
                ExtendedMode next = getDoubleClickStrategy().handleDoubleClick(dockable,
                        current == null ? null : current.getExtendedMode(), enablement);
        ...
        runTransaction(set -> {
            try {
                ...
                action.apply(new CGroupBehaviorCallback() {
                    ...
                    public Location getLocation(Dockable dockable1) {
                        M mode = getCurrentMode(dockable1);
                        if (mode == null) {
                            return null;
                        }
                        return mode.current(dockable1);
                    }
                });
            } finally {
    ...
    public Map<ExtendedMode, DockStation> getRepresentations(String id) {
        ...
        Map<ExtendedMode, DockStation> result = new HashMap<>();
    ...
    public void addListener(Path identifier, LocationModeListener listener) {
        ...
        List<LocationModeListener> list = listeners.computeIfAbsent(identifier, k -> new ArrayList<>());
        list.add(listener);
     ...
     * and sets <code>dockable</code> as neighbor. This method does not change the actual location of
     * <code>dockable</code>,
    ...
    private class LocationModeListenerAdapter implements ModeManagerListener<Location, LocationMode> {
        ...
        public void dockableRemoved(ModeManager<? extends Location, ? extends LocationMode> manager,
                                    Dockable dockable) {
            // ignore
        }

        public void modeChanged(ModeManager<? extends Location, ? extends LocationMode> manager, Dockable dockable,
                                LocationMode oldMode, LocationMode newMode) {
            // ignore
        }
    }

    /**
     * If the {@link DockRegister} is currently stalled, then a call to
