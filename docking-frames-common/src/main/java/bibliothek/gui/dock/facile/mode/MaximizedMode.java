...
public class MaximizedMode<M extends MaximizedModeArea> extends AbstractLocationMode<M> {
     ...
     */
    private final List<KeyHook> hooks = new LinkedList<>();

    /**
     * {@link KeyStroke} used to go into, or go out from the maximized state.
     */
    private final PropertyValue<KeyStroke> keyStrokeMaximizeChange = new PropertyValue<>(CControl.KEY_MAXIMIZE_CHANGE) {
        @Override
     ...
     * @param area     the future parent of <code>dockable</code>, can be <code>null</code>
     * @param dockable the element that should be made maximized
     * @param history  the expected location of <code>dockable</code> after this method has finished, can be
     *                 <code>null</code>.
    ...
    public boolean maximize(MaximizedModeArea area, Dockable dockable, Location history, AffectedSet set) {
        ...
        if (id == null && current == null) {
            throw new IllegalStateException("an unidentified dockable without location has been found, all dockables " +
                    "except true root-station must have a location, true root-stations can never be used in this " +
                    "method.");
        }
            ...
            if (maximized != null) {
                for (Dockable check : maximized) {
                    if (DockUtilities.isAncestor(check, dockable)) {
                        ...
                        manager.runTransaction(set1 -> {
                            area.setMaximized(element, false, null, set1);
     ...
     * @param history  the history of the last place where <code>dockable</code> was maximized, might be
     *                 <code>null</code>
    ...
    protected void applyStarting(LocationModeEvent event) {
        ...
        for (Dockable value : maximizedNow) {
            if (DockUtilities.isAncestor(value, dockable)) {
                maximized = getMaximizingElement(value, dockable);
                break;
            }
        }
    ...
    public void writeSetting(ModeSetting<Location> setting) {
        if (setting instanceof MaximizedModeSetting modeSetting) {
