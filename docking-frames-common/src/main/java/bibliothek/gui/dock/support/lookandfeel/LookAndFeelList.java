...
public class LookAndFeelList {
     ...
     */
    private static volatile LookAndFeelList list;
     ...
     */
    private final List<LookAndFeelListener> listeners = new ArrayList<>();

    /**
     * The roots of the {@link Component}-trees that need to be updated when the <code>LookAndFeel</code> changes
     */
    private final List<ComponentCollector> componentCollectors = new ArrayList<>();
    ...
    protected LookAndFeelList() {
        ...
        for (LookAndFeelInfo lookAndFeelInfo : preset) {
            add(new Info(lookAndFeelInfo.getClassName(), lookAndFeelInfo.getName()));
        }
    ...
    protected LookAndFeelListener[] listeners() {
        return listeners.toArray(new LookAndFeelListener[0]);
    }
     ...
     * @author Benjamin Sigg
     */
    private static class Wrapper extends Info {
        /**
         * the delegate
         */
