...
public abstract class ModeManager<H, M extends Mode<H>> {
     ...
     */
    private final Map<Path, ModeSettingFactory<H>> factories = new HashMap<>();

    /**
     * lists for all known {@link Dockable}s their {@link DockableHandle}
     */
    private final Map<Dockable, DockableHandle> dockables = new HashMap<>();

    /**
     * list all {@link DockableHandle}s ever created and not dismissed by this manager
     */
    private final Map<String, DockableHandle> entries = new HashMap<>();

    /**
     * all the listeners that are registered at this manager
     */
    private final List<ModeManagerListener<? super H, ? super M>> listeners =
            new ArrayList<>();
     ...
     */
    private final ActionGuard stationGuard = new ActionGuard() {
        public boolean react(Dockable dockable) {
    ...
    protected ModeManagerListener<? super H, ? super M>[] listeners() {
        return listeners.toArray(new ModeManagerListener[0]);
    }
    ...
    public void add(String key, Dockable dockable) {
        ...
        if (dockables.containsKey(dockable)) {
            throw new IllegalArgumentException("The dockable is already known to this manager (but it does have a " +
                    "different name)");
        }
    ...
    public void runTransaction(final AffectingRunnable run, boolean continuous) {
        ...
        try {
            ...
            runTransaction(() -> run.run(affected), continuous);
        } finally {
            closeAffected();
        }
    }
    ...
    public <B> ModeSettings<H, B> createModeSettings(ModeSettingsConverter<H, B> converter) {
        return new ModeSettings<>(converter);
    }
    ...
    public Runnable readSettings(ModeSettings<H, ?> settings, UndoableModeSettings pending) {
        ...
        for (int i = 0, n = settings.size(); i < n; i++) {
            ...
            if (entry != null) {
                ...
                entry.history.clear();
                entry.history.addAll(Arrays.asList(settings.getHistory(i)));
    ...
    private class DockableHandle {
         ...
         */
        public final MultiDockActionSource source;
        /**
    ...
    private class ChangeSet implements AffectedSet {
         ...
         */
        private final Set<Dockable> set = new HashSet<>();
