...
public class ModeSettings<A, B> {
     ...
     */
    private final List<DockableEntry> dockables = new ArrayList<>();

    /**
     * the list of mode information to store
     */
    private final Map<Path, ModeSetting<A>> modes = new HashMap<>();
     ...
     */
    private final Map<Path, ModeSettingFactory<A>> factories = new HashMap<>();
    ...
    public void add(String id, Path current, Map<Path, A> properties, Collection<Path> history) {
        ...
        entry.history = history.toArray(new Path[0]);
