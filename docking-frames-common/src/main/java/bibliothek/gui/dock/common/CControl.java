...
import bibliothek.gui.dock.common.grouping.*;
...
import bibliothek.gui.dock.common.intern.station.*;
import bibliothek.gui.dock.common.intern.ui.*;
...
public class CControl {
    ...
    public static final PropertyKey<KeyStroke> KEY_MAXIMIZE_CHANGE =
            new PropertyKey<>("ccontrol.maximize_change");
    ...
    public static final PropertyKey<KeyStroke> KEY_GOTO_MAXIMIZED =
            new PropertyKey<>("ccontrol.goto_maximized");
    ...
    public static final PropertyKey<KeyStroke> KEY_GOTO_NORMALIZED =
            new PropertyKey<>("ccontrol.goto_normalized");
    ...
    public static final PropertyKey<KeyStroke> KEY_GOTO_MINIMIZED =
            new PropertyKey<>("ccontrol.goto_minimized");
    ...
    public static final PropertyKey<KeyStroke> KEY_GOTO_EXTERNALIZED =
            new PropertyKey<>("ccontrol.goto_externalized");

    /**
     * {@link KeyStroke} used to close a {@link CDockable}.
     */
    public static final PropertyKey<KeyStroke> KEY_CLOSE =
            new PropertyKey<>("ccontrol.close");

    /**
     * {@link KeyStroke} that can be hit during a drag and drop operation, that will cancel the operation (default is
      * ESCAPE).
     */
    public static final PropertyKey<KeyStroke> KEY_CANCEL_OPERATION =
            new PropertyKey<>("ccontrol.cancel_dnd");
    ...
    public static final PropertyKey<ConflictResolver<RequestDimension>> RESIZE_LOCK_CONFLICT_RESOLVER =
            new PropertyKey<>(
                    ...
                    new ConstantPropertyFactory<>(new DefaultConflictResolver<>()), true);
    ...
    public static final PropertyKey<CloseActionFactory> CLOSE_ACTION_FACTORY =
            new PropertyKey<>("ccontrol.closeActionFactory",
                    new ConstantPropertyFactory<>(CloseActionFactory.DEFAULT), true);
    ...
    public static final PropertyKey<CGroupingBehavior> GROUPING_BEHAVIOR =
            new PropertyKey<>("ccontrol.groupingBehavior",
                    new ConstantPropertyFactory<>(new DefaultCGroupingBehavior()), true);
     ...
     */
    private final Map<CDockable, CDockableAccess> accesses = new HashMap<>();
     ...
     */
    private final CControlAccess access = new Access();
     ...
     */
    private final ApplicationResourceManager resources = new ApplicationResourceManager();

    /**
     * a list of listeners which are to be informed when this control is no longer in use
     */
    private final List<DestroyHook> hooks = new ArrayList<>();
     ...
     */
    private final List<CControlListener> listeners = new ArrayList<>();

    /**
     * the list of resize-listeners
     */
    private final List<ResizeRequestListener> resizeListeners = new ArrayList<>();

    /**
     * the collection of global listeners
     */
    private final CListenerCollection listenerCollection = new CListenerCollection();

    /**
     * the preferences used by this instance of {@link CControl}
     */
    private final PreferenceStorage preferences = new PreferenceStorage();
     ...
     *
     * @param window                a provider for the main window of this application. Needed
     *                              to create dialogs for externalized {@link CDockable}s. Must not be
     *                              <code>null</code>, but
     *                              its search method may return <code>null</code>
     * @param restrictedEnvironment whether this application runs in a
     *                              restricted environment and is not allowed to listen for global events.
    ...
    protected void init(WindowProvider window, CControlFactory factory) {
        ...
        frontend.getController().getRegister().addDockRegisterListener(new DockRegisterAdapter() {
            ...
            public void dockableUnregistered(DockController controller, Dockable dockable) {
                if (dockable instanceof CommonDockable) {
                    ...
                    if (cdock instanceof MultipleCDockable multiple) {
        ...
        frontend.getController().getFocusController().addVetoListener(new ControlVetoFocusListener(this,
         listenerCollection.getVetoFocusListener()));
        frontend.getController().getFocusController().setStrategy(new DefaultFocusStrategy(frontend.getController()) {
            public Component getFocusComponent(FocusStrategyRequest request) {
                ...
                if (mouseClicked != null) {
                    if ((mouseClicked.isFocusable() && !excluded(mouseClicked, request)) ||
                            focusable(mouseClicked, request)) {
                        return mouseClicked;
                    }
                }
        ...
        controller.getExtensions().load(new ExtensionName<>(CCONTROL_EXTENSION, Object.class, EXTENSION_PARAM, this));
    }
    ...
    private void initFocusListeners(DockController controller) {
        controller.addDockableFocusListener(event -> {
    ...
    protected void initFactories() {
        ...
        frontend.registerFactory(new StackDockStationFactory() {
            @Override
            public StackDockPerspective layoutPerspective(StackDockStationLayout layout, Map<Integer,
             PerspectiveDockable> children) {
    ...
    protected void initIcons() {
        DefaultIconScheme scheme = new DefaultIconScheme(getController(),
                new DefaultIconScheme.IconResource("data/bibliothek/gui/dock/core/icons.ini", null,
                 DockController.class.getClassLoader()),
                new DefaultIconScheme.IconResource("data/bibliothek/gui/dock/common/icons/icons.ini", null,
CControl.class.getClassLoader()));
    ...
    protected void initTexts(Locale locale) {
        ResourceBundle bundleCore =
                ResourceBundle.getBundle("data.bibliothek.gui.dock.core.locale.text", locale,
                 DockController.class.getClassLoader());
        ResourceBundle bundleCommon =
                ResourceBundle.getBundle("data.bibliothek.gui.dock.common.locale.common", locale,
                 CControl.class.getClassLoader());
    ...
    private CControlListener[] listeners() {
        return listeners.toArray(new CControlListener[0]);
    }
                ...
                public boolean ignoreElement(PerspectiveElement element) {
                    if (element instanceof CommonElementPerspective) {
                        CDockablePerspective perspective =
                                ((CommonElementPerspective) element).getElement().asDockable();
                        if (perspective != null) {
    ...
    private CContentArea createContentArea(String uniqueId, boolean isDefaultContentArea) {
        ...
        if (!isDefaultContentArea && uniqueId.equals(CONTENT_AREA_STATIONS_ID)) {
            throw new IllegalArgumentException("the unique identifier '" + uniqueId +
                    "' is reserved for the default CContentArea and may not be used by the client");
        }
     ...
     * 	<tr><td>{@link BubbleTheme#ACTION_DISTRIBUTOR}</td><td>Default instance of a {@link DockActionDistributor}
     * 	.</td></tr>
     *  <tr><td>{@link FlatTheme#ACTION_DISTRIBUTOR}</td><td>Default instance of a {@link DockActionDistributor}.</td
     *  ></tr>
     *  <tr><td>{@link DefaultDockRelocator#AUTO_DROP_ON_ANY_MOUSE_RELEASED_EVENT}</td><td>Stop drag-and-drop
     *  operations on any mouse-released event (a workaround necessary for some Linux and Mac systems).</td></tr>
     *  <tr><td>{@link DockTheme#BACKGROUND_PAINT} </td><td>The default value of the {@link BackgroundPaint}.</td></tr>
     * 	<tr><td>{@link BasicTheme#BASIC_COLOR_SCHEME}</td><td>The {@link ColorScheme} to use if the {@link BasicTheme}
     * 	is installed.</td></tr>
     *  <tr><td>{@link DockTheme#BORDER_MODIFIER} </td><td>The default value of the {@link BorderModifier}.</td></tr>
     *  <tr><td>{@link ScreenDockStation#BOUNDARY_RESTRICTION}</td><td>How far the user can push a window with a
     *  {@link Dockable} out of the screen(s).</td></tr>
     *  <tr><td>{@link BubbleTheme#BUBBLE_COLOR_SCHEME} </td><td>The {@link ColorScheme} to use if the
     *  {@link BubbleTheme} is installed.</td></tr>
     *  <tr><td>{@link FlapDockStation#BUTTON_CONTENT} </td><td>Tells what content should be on the buttons that
     *  represent minimized {@link Dockable}s.</td></tr>
     * 	<tr><td>{@link FlapDockStation#BUTTON_CONTENT_FILTER} </td><td>Tells which {@link DockAction}s should be shown
     * 	on a button representing a minimized {@link Dockable}.</td></tr>
     *  <tr><td>*&nbsp;{@link CControl#CCONTROL} </td><td>The {@link CControl} in whose realm the property is read,
     *  is a read-only property.</td></tr>
     *  <tr><td>{@link DockTheme#COMBINER} </td><td>Default value of the {@link Combiner}.</td></tr>
     *  <tr><td>{@link StackDockStation#COMPONENT_FACTORY} </td><td>The factory creating the "tabbed panes" of the
     *  {@link StackDockStation}.</td></tr>
     *  <tr><td>{@link DockTheme#DISPLAYER_FACTORY} </td><td>Default value of the {@link DisplayerFactory}.</td></tr>
     *  <tr><td>{@link DockStationDropLayerFactory#DROP_LAYER_FACTORY}</td><td>Factory that defines which parts of
     *  the screen are targets for a drag and drop operation/</td></tr>
     ...
     *  <tr><td>{@link DockTheme#DOCKABLE_MOVING_IMAGE_FACTORY} </td><td>Default value of the
     *  {@link DockableMovingImageFactory}.</td></tr>
     ...
     *  <tr><td>{@link LocationModeManager#DOUBLE_CLICK_STRATEGY} </td><td>Tells what happens if the user double
     *  clicks on a {@link DockTitle} or a {@link Dockable}.</td></tr>
     *  <tr><td>{@link EclipseTheme#ECLIPSE_COLOR_SCHEME} </td><td>The {@link ColorScheme} to use if the
     *  {@link EclipseTheme} is installed.</td></tr>
     *  <tr><td>*&nbsp;{@link ScreenDockStation#EXPAND_ON_DOUBLE_CLICK} </td><td>Whether a double click on a child of
     *  a {@link ScreenDockStation} should maximize the child.</td></tr>
     *  <tr><td>{@link FlatTheme#FLAT_COLOR_SCHEME} </td><td>The {@link ColorScheme} to use if the {@link FlatTheme}
     *  is installed.</td></tr>
     *  <tr><td>{@link ScreenDockStation#FULL_SCREEN_STRATEGY} </td><td>Defines when a floating {@link Dockable} is
     *  considered to be in fullscreen mode.</td></tr>
     *  <tr><td>*&nbsp;{@link DockFrontend#HIDE_ACCELERATOR} </td><td>The {@link KeyStroke} that will call
     *  {@link DockFrontend#hide(Dockable)}</td></tr>
     *  <tr><td>{@link DockableSelector#INIT_SELECTION} </td><td>The {@link KeyStroke} that opens a window where the
     *  user can select a new {@link Dockable}.</td></tr>
     *  <tr><td>{@link StackDockStation#IMMUTABLE_SELECTION_INDEX} </td><td>Prevents the {@link StackDockStation}
     *  from switching the selected index on a drop operation (but does not prevent the {@link FocusManager} from
     *  switching the focus!).</td></tr>
     *  <tr><td>{@link CControl#KEY_CLOSE} </td><td>The {@link KeyStroke} that closes a {@link CDockable}.</td></tr>
     *  <tr><td>{@link CControl#KEY_GOTO_EXTERNALIZED} </td><td>The {@link KeyStroke} that externalizes a
     *  {@link CDockable}.</td></tr>
     *  <tr><td>{@link CControl#KEY_GOTO_MAXIMIZED} </td><td>The {@link KeyStroke} that maximizes a {@link CDockable}
     *  .</td></tr>
     *  <tr><td>{@link CControl#KEY_GOTO_MINIMIZED} </td><td>The {@link KeyStroke} that minimizes a {@link CDockable}
     *  .</td></tr>
     *  <tr><td>{@link CControl#KEY_GOTO_NORMALIZED} </td><td>The {@link KeyStroke} that normalizes a
     *  {@link CDockable}.</td></tr>
     *  <tr><td>{@link CControl#KEY_MAXIMIZE_CHANGE} </td><td>The {@link KeyStroke} that either maximizes or
     *  normalizes a {@link CDockable}.</td></tr>
     *  <tr><td>{@link CControl#KEY_CANCEL_OPERATION} </td><td> The {@link KeyStroke} that will cancel the current
     *  drag and drop operation. </td></tr>
     *  <tr><td>{@link FlapDockStation#LAYOUT_MANAGER} </td><td>Tells the {@link FlapDockStation} the size and the
     *  hold property of its children.</td></tr>
     *  <tr><td>{@link SplitDockStation#LAYOUT_MANAGER} </td><td>Logic of all {@link SplitDockStation}s, used when
     *  dropping a {@link Dockable} or resizing the station.</td></tr>
     *  <tr><td>{@link TabPane#LAYOUT_MANAGER} </td><td>Defines the size and location of tabs of a stack.</td></tr>
     *  <tr><td>{@link TabPane#USE_SMALL_MINIMUM_SIZE} </td><td>Use really small minimum sizes for calculating the
     *  minimum size of a tab-pane, instead of trying to make the content look good</td></tr>
     *  <tr><td>*&nbsp;{@link SplitDockStation#MAXIMIZE_ACCELERATOR} </td><td>The {@link KeyStroke} that maximizes a
     *  child of a {@link SplitDockStation}.</td></tr>
     *  <tr><td>{@link CombinedMenuContent#MENU_CONTENT} </td><td>The menu that shows overflowing {@link Dockable}s
     *  on a stack.</td></tr>
     *  <tr><td>{@link IconManager#MINIMUM_ICON_SIZE}</td><td>The expected minimal size of all icons. </td></tr>
     *  <tr><td>{@link FlapDockStation#MINIMUM_SIZE} </td><td>The minimum size of the {@link Component} that
     *  represents the {@link FlapDockStation}.</td></tr>
     *  <tr><td>*&nbsp;{@link LocationModeManager#MODE_ENABLEMENT} </td><td>Tells which {@link CDockable} is allowed
*  to have which {@link ExtendedMode}.</td></tr>
     *  <tr><td>{@link DockRelocatorMode#NO_COMBINATION_MASK} </td><td>What keys the user has to press during a drag
     *  and drop operation to prevent the framework from combining {@link Dockable}s.</td></tr>
     *  <tr><td>{@link DockTitle#ORIENTATION_STRATEGY} </td><td>Tells how to rotate text on a {@link DockTitle}.</td
*  ></tr>
     *  <tr><td>{@link EclipseTheme#PAINT_ICONS_WHEN_DESELECTED} </td><td>Whether to paint icons on unselected tabs
     *  if using the {@link EclipseTheme}.</td></tr>
     *  <tr><td>{@link PlaceholderStrategy#PLACEHOLDER_STRATEGY} </td><td>A strategy that creates placeholders for
        *  {@link Dockable}s, see {@link CPlaceholderStrategy}.</td></tr>
     *  <tr><td>{@link CControl#RESIZE_LOCK_CONFLICT_RESOLVER} </td><td>Tells what happens if two {@link CDockable}s
     *  have a locked size and the user is resizing the parent of these two elements.</td></tr>
     *  <tr><td>{@link DockController#RESTRICTED_ENVIRONMENT} </td><td>Tells whether the application runs as
     *  applet/with webstart or as free or authenticated application.</td></tr>
     *  <tr><td>{@link DockRelocatorMode#SCREEN_MASK} </td><td>The keys the user has to press during a drag and drop
     *  operation to ensure that the {@link Dockable} is added to a {@link ScreenDockStation}.</td></tr>
     *  <tr><td>{@link SingleTabDecider#SINGLE_TAB_DECIDER} </td><td>Tells which {@link Dockable}s should be
     *  presented with a single tab - even if there is no reason to show a tab.</td></tr>
     *  <tr><td>{@link DockTheme#STATION_PAINT} </td><td>The default value of {@link StationPaint}.</td></tr>
     *  <tr><td>{@link AWTComponentCaptureStrategy#STRATEGY} </td><td>How to make an image of an AWT component
     *  .</td></tr>
     *  <tr><td>{@link DisablingStrategy#STRATEGY}</td><td>Which element to disable.</td></tr>
     *  <tr><td>{@link StackDockStation#TAB_CONTENT_FILTER} </td><td>A filter deciding what content to show on a tab
     *  of a {@link StackDockStation}.</td></tr>
     *  <tr><td>{@link EclipseTheme#TAB_PAINTER} </td><td>The look of tabs if using the {@link EclipseTheme}.</td></tr>
     *  <tr><td>{@link StackDockStation#TAB_PLACEMENT} </td><td>The location of the tabs on a
     *  {@link StackDockStation}.</td></tr>
     *  <tr><td>{@link EclipseTheme#THEME_CONNECTOR} </td><td>Detailed instructions how to present a {@link Dockable}
     *  if using the {@link EclipseTheme}.</td></tr>
     *  <tr><td>{@link FlapDockStation#WINDOW_FACTORY} </td><td>A factory creating {@link FlapWindow}s for the
     *  {@link FlapDockStation}.</td></tr>
     *  <tr><td>{@link ScreenDockStation#WINDOW_FACTORY} </td><td>A factory creating {@link ScreenDockWindow}s for
        *  the {@link ScreenDockStation}.</td></tr>
     * </table>
     *
    ...
    public <S extends SingleCDockable> S addDockable(S dockable) {
        ...
        if (preset != null) {
            if (preset == dockable) {
                ...
                throw new IllegalArgumentException("unique id '" + dockable.getUniqueId() +
                        "' already in use for another SingleCDockable");
            }
        }
     ...
     * for a factory which was added with a specific identifier
     * ({@link #addSingleDockableFactory(String, SingleCDockableFactory)}),
     * if nothing is found then the factories with a filter are searched
     * ({@link #addSingleDockableFactory(Filter, SingleCDockableFactory)}).
     *
     ...
     * @param dockable the new element to show
     * @return <code>dockable</code>
     * @throws IllegalArgumentException if either the {@link MultipleCDockable#getFactory() factory} of
     * <code>dockable</code> is <code>null</code>,
     *                                  or is not registered (see
      *                                  {@link #addMultipleDockableFactory(String, MultipleCDockableFactory)}).
     */
    public <M extends MultipleCDockable> M addDockable(M dockable) {
        ...
        if (factoryId == null) {
            throw new IllegalStateException(
                    "the factory for a MultipleCDockable is not registered: " + dockable.getFactory());
        }
    ...
    private boolean shouldCreate(MultipleCDockableFactory<?, ?> factory, CommonMultipleDockableLayout layout) {
        ...
        MultipleCDockableFactory<?, MultipleCDockableLayout> normalizedFactory =
                (MultipleCDockableFactory<?, MultipleCDockableLayout>) factory;
    ...
    private void addMultipleDockableFactory(final String id, final MultipleCDockableFactory<?, ?> factory,
     boolean check) {
        if (check) {
            checkValidUniqueId(id);
        }
        ...
        if (access.getFactoryId(factory) != null) {
            throw new IllegalArgumentException("this factory-object is already in use and cannot be added a second " +
             "time");
        }
    ...
    public void removeMultipleDockableFactory(String id) {
        ...
        if (factory != null) {
            ...
            List<MultipleCDockable> toRemove = new ArrayList<>();
            for (MultipleCDockable dockable : register.getMultipleDockables()) {
    ...
    public void setMissingStrategy(MissingCDockableStrategy missingStrategy) {
        this.missingStrategy = Objects.requireNonNullElse(missingStrategy, MissingCDockableStrategy.PURGE);
    }
    ...
    public void handleResizeRequests() {
        ResizeRequestListener[] listeners = resizeListeners.toArray(new ResizeRequestListener[0]);
     ...
     * {@link Dockable}s after loading a persistent layout. Basically if this property is set, then all
     * {@link Dockable}s
     ...
     * <ul>
     * 	<li>If the user changes the layout, he/she most likely would like to see the effects. A maximized
      * 	{@link Dockable} would
     *  hide the effects.</li>
     *  <li>For the user re-maximizing an element requires no more than one click with the mouse. It's a cheap
     *  operation.</li>
     *  <li>It is an additional layer of security preventing {@link Dockable}s from being in the wrong position if
     *  the client
    ...
    /**
     * If a {@link CDockable} is minimized, the focus can be automatically transferred to another {@link CDockable}.
     * This
     ...
     * @param name                the name of the layout.
     * @param includeWorkingAreas whether the content of the {@link CStation}s that are marked as
     *                            {@link CStation#isWorkingArea() working area} should be updated as well. This value
     *                            should be the same
    ...
    private class Access implements CControlAccess {
        ...
        public void hide(CDockable dockable) {
            ...
            try {
                ...
                Map<Dockable, ExtendedMode> nonBasic = new HashMap<>();
            ...
            try {
                ...
                if (area != null && area.asDockable() != null) {
                    if (!area.asDockable().isVisible()) {
                        throw new IllegalStateException("A dockable that wants to be on a working-area can't be made " +
            "visible unless the working-area is visible.");
                    }
                }
