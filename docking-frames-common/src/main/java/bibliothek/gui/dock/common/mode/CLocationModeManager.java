...
import bibliothek.gui.dock.support.mode.*;
...
public class CLocationModeManager extends LocationModeManager<CLocationMode> {
    ...
    /**
     * the key used for the {@link IconManager} to read the {@link javax.swing.Icon} for the "unmaximize
     * externalized"-action
     */
    ...
    private final CNormalMode normalMode;
    private final CMaximizedMode maximizedMode;
    private final CMinimizedMode minimizedMode;
    private final CExternalizedMode externalizedMode;
    ...
    public CLocationModeManager(CControlAccess control) {
        ...
        addModeManagerListener(new ModeManagerListener<Location, LocationMode>() {
            public void dockableAdded(ModeManager<? extends Location, ? extends LocationMode> manager,
                                      Dockable dockable) {
                // ignore
            }
            ...
            public void modeChanged(ModeManager<? extends Location, ? extends LocationMode> manager,
                                    Dockable dockable, LocationMode oldMode, LocationMode newMode) {
                CDockableAccess access =
                        CLocationModeManager.this.control.access(((CommonDockable) dockable).getDockable());
                if (access != null) {
            ...
            public void modeRemoved(ModeManager<? extends Location, ? extends LocationMode> manager,
                                    LocationMode mode) {
                // ignore
            }
        });
    ...
    public <B> ModeSettings<Location, B> createModeSettings(ModeSettingsConverter<Location, B> converter) {
        return new CLocationModeSettings<>(converter);
    }

    @Override
    public void readSettings(ModeSettings<Location, ?> settings) {
        UndoableModeSettings undoable = key -> control.getRegister().isMultiId(key);
    ...
    public Runnable readSettings(ModeSettings<Location, ?> settings, UndoableModeSettings pending) {
        ...
        if (settings instanceof CLocationModeSettings<?> locationSettings) {
    ...
    public void setLocation(Dockable dockable, ExtendedMode mode, CLocation location) {
        ...
        if (location != null) {
            ...
            if (property == null) {
                throw new IllegalArgumentException("the location does not carry enough information to find the " +
                        "location of dockable");
            }

            ExtendedMode locationMode = location.findMode();
            if (locationMode == null) {
                throw new IllegalArgumentException("the location does not carry enough information to find the mode " +
                        "of dockable");
            }

            if (!mode.getModeIdentifier().equals(locationMode.getModeIdentifier())) {
                throw new IllegalArgumentException("location and mode do not belong together, they do not have the " +
                        "same identifier");
            }

            setProperties(getMode(mode.getModeIdentifier()), dockable, new Location(mode.getModeIdentifier(), root,
                    property, true));
        } else {
    ...
    public boolean ensureBasicModes() {
        ...
        final Single<Boolean> result = new Single<>(false);
        ...
        for (int i = history.size() - 1; i >= 0 && next == null; i--) {
            ...
            if (path != null && root != null) {
                ...
                if (mode != null) {
                    CStation<?> station = control.getOwner().getStation(root);
                    if (station != null) {
                        if (mode.respectWorkingAreas(station.getStation()) &&
                                mode.isRepresenting(station.getStation())) {
    ...
    protected CStation<?> getAreaOf(Dockable dockable) {
        Map<DockStation, CStation<?>> stations = new HashMap<>();
