...
public class CControlPerspective {
     ...
     * @param root                the element to write into, not <code>null</code>
     * @param perspective         the perspective to write, not <code>null</code>
     * @param includeWorkingAreas whether the output contains information about children of
     * {@link CStation#isWorkingArea() working areas}
     *                            (<code>includeWorkingAreas = true</code>) or not (<code>includeWorkingAreas =
     *                            false</code>)
     */
    public void writeXML(XElement root, CPerspective perspective, boolean includeWorkingAreas) {
        ...
        Map<String, DockLayoutComposition> stations = new HashMap<>();
        ...
        for (String key : perspective.getDockableKeys()) {
            ...
            if (location != null) {
                ...
                if (dockable.getParent() == null) {
                    conversion.getSituation().writeCompositionXML(conversion.convert(dockable.intern()),
                            xdockable.addElement("content"));
                }
     ...
     *
     * @param root                the element which contains information about a perspective
     * @param includeWorkingAreas whether the perspective contains information about children of
     * {@link CStation#isWorkingArea() working areas}
     *                            (<code>includeWorkingAreas = true</code>) or not (<code>includeWorkingAreas =
     *                            false</code>). This parameter should have the same value as was used
    ...
    public CPerspective readXML(XElement root, boolean includeWorkingAreas) throws XException {
        ...
        for (Map.Entry<String, MultipleCDockableFactory<?, ?>> item : control.getRegister().getFactories().entrySet()) {
            conversion.getSituation().add(new CommonMultipleDockableFactory(item.getKey(), item.getValue(), control,
                    perspective));
        }
                ...
                if (dockable == null) {
                    ...
                    if (xcontent != null) {
                        PerspectiveElement element =
                                conversion.convert(conversion.getSituation().readCompositionXML(xcontent));
        ...
        ModeSettingsConverter<Location, Location> converter =
                new LocationSettingConverter(control.getOwner().getController());
            ...
            if (composition != null) {
                ...
                if (element instanceof CommonElementPerspective) {
                    ...
                    if (dockable != null) {
                        ...
                        if (mode != null) {
                            dockable.getLocationHistory().add(mode, new Location(mode.getModeIdentifier(), root,
                                    location, false));
                        }
     ...
     * and write layouts. This method adds {@link CommonSingleDockableFactory}, {@link CommonMultipleDockableFactory}
     * and
    ...
    private DockFrontendPerspective wrap(CPerspective perspective, boolean includeWorkingAreas,
                                         PerspectiveElementFactory factory) {
        ...
        CommonSingleDockableFactory singleDockableFactory =
                new CommonSingleDockableFactory(control.getOwner(), perspective);

        inner.getSituation().add(singleDockableFactory);
        inner.getSituation().addBackup(new RegisteringDockFactory<>(control.getOwner().intern(),
    ...
    private class PerspectiveElementFactory implements FrontendPerspectiveCache {
        private final CPerspective perspective;
        private Perspective basePerspective;
        private final Map<String, SingleCDockablePerspective> dockables = new HashMap<>();
        ...
        public PerspectiveElementFactory(CPerspective perspective) {
            ...
            while (elements.hasNext()) {
                ...
                if (element instanceof SingleCDockablePerspective dockable) {
        ...
        public PerspectiveElement get(String id, DockElement element, boolean isRootStation) {
            if (isRootStation) {
                ...
                if (dockable.asStation() != null) {
                    ...
                    if (station == null) {
                        throw new IllegalArgumentException("Found a non-root CStation that is not registered: " +
                                dockable.asStation().getUniqueId());
                    }
            ...
            throw new IllegalArgumentException(
                    "The intern DockFrontend of the CControl has elements registered that are not SingleCDockables: " +
                            id + "=" + element);
        }
        ...
        private DockLayoutComposition getPredefinedStation(String id, DockLayoutComposition current, DockSituation situation) {
            ...
            if (currentId != null) {
                if (id.length() == DockFrontend.ROOT_KEY_PREFIX.length() + id.length() &&
                        currentId.startsWith(DockFrontend.ROOT_KEY_PREFIX) && currentId.endsWith(id)) {
                    return current;
                }
