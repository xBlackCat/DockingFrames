...
public class LocationModeManagerPerspective {
     ...
     */
    private final Map<ExtendedMode, LocationModePerspective> modes = new HashMap<>();

    /**
     * all the factories that creates {@link ModeSetting}s
     */
    private final Map<Path, ModeSettingFactory<Location>> settingFactories = new HashMap<>();
    ...
    public ExtendedMode[] getModes() {
        return modes.keySet().toArray(new ExtendedMode[0]);
    }
    ...
    public ModeSettings<Location, Location> writeModes(CControlAccess control) {
        ModeSettings<Location, Location> modes =
                new CLocationModeSettings<>(new LocationSettingConverter(control.getOwner().getController()));
