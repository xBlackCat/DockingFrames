 ...
 */
public class CommonSingleDockableFactory implements DockFactory<CommonDockable, CommonElementPerspective,
     ...
     */
    private final List<Entry> factories = new ArrayList<>();

    /**
     * factories using one id only
     */
    private final Map<String, Entry> singleIdFactories = new HashMap<>();
    ...
    public void add(String id, SingleCDockableFactory factory) {
        ...
        Entry entry = new Entry(new PresetFilter<>(id), factory);
        factories.add(entry);
    ...
    public CommonSingleDockableLayout getLayout(CommonDockable element, Map<Dockable, Integer> children) {
        ...
        if (dockable instanceof SingleCDockable single) {
            ...
            throw new IllegalArgumentException(
                    "A CommonSingleDockableFactory works only with Dockables of type SingleCDockable, but this is not" +
                            " a single dockable: " +
                            element);
        }
    }
    ...
    public CommonDockable layout(CommonSingleDockableLayout layout, PlaceholderStrategy placeholders) {
        ...
        if (!factoryId.equals(getID())) {
            throw new IllegalArgumentException("Wrong type of dockable for unique id '" + layout.getId() +
                    "': The backup factory created a dockable which expects a factory with type-id '" + factoryId +
    ...
    public CommonElementPerspective layoutPerspective(CommonSingleDockableLayout layout, Map<Integer,
    ...
    public void layoutPerspective(CommonElementPerspective perspective, CommonSingleDockableLayout layout,
                                  Map<Integer, PerspectiveDockable> children) {
        // can't do anything
    }

    public CommonSingleDockableLayout getPerspectiveLayout(CommonElementPerspective element, Map<PerspectiveDockable,
            Integer> children) {
     ...
     *
     * @param filter  the filter to apply before {@link #factory} can be used
     * @param factory the factory represented by this entry
     */
    private record Entry(Filter<String> filter, SingleCDockableFactory factory) {
        /**
         * Creates a new entry.
         *
