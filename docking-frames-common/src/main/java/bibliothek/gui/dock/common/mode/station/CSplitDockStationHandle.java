...
public class CSplitDockStationHandle {
     ...
     */
    private final CStation<CSplitDockStation> station;
     ...
     * maximized-mode
     */
    private final Maximal maximal = new Maximal();
     ...
     */
    private final CLocationModeManager manager;

    /**
     * the listeners added to this {@link ModeArea}
     */
    private final List<ModeAreaListenerWrapper> listeners = new ArrayList<>();

    private final SplitDockListener fullScreenListener = (station, oldFullScreen, newFullScreen) -> {
        Set<Dockable> affected = new HashSet<>();
        ...
        ModeAreaListenerWrapper[] array = listeners.toArray(new ModeAreaListenerWrapper[0]);
    ...
    private final VetoableDockRelocatorListener relocatorListener = new VetoableDockRelocatorAdapter() {
        ...
        public void dropped(DockRelocatorEvent event) {
            if (!event.isMove()) {
                ...
                if (next == maximal) {
                    manager.runTransaction(set -> maximizedMode.unmaximize((DockStation) getStation(), set));
                }
            }
        }
    };
     ...
     * @author Benjamin Sigg
     */
    protected record ModeAreaListenerWrapper(ModeArea area, ModeAreaListener listener) {
                ...
                if (manager.getMode(dockable) != normalExtendedMode()) {
                    ...
                    if (normal != null) {
                        CGroupMovement movement =
                                maximizedMode.getManager().getGroupBehavior().prepare(manager, dockable,
                                        normal.getExtendedMode());
                        if (movement != null) {
