...
public class RootStationAdjacentFactory implements AdjacentDockFactory<Path> {
    ...
    public void setLayout(DockElement element, Path layout, Map<Integer, Dockable> children,
                          PlaceholderStrategy placeholders) {
        if (!(element instanceof CommonDockStation<?, ?>)) {
            throw new IllegalArgumentException(
                    "expected the 'element' to be a '" + CommonDockStation.class.getSimpleName() +
                            "', but instead it was a '" + element.getClass().getName() + "'.\n" +
                            "The reason for this exception may be be:\n - a DockElement that was registered with the " +
                            "wrong unique identifier\n - the type of a DockElement was changed\n - a " +
                            "SingleCDockableFactory creating the wrong type of items.\n" +
        ...
        if (typeId != null && !typeId.equals(layout)) {
            throw new IllegalArgumentException(
                    "expected the 'element' to have type id '" + layout + "', but instead it was '" + typeId + "'.\n" +
                            "The reason for this exception may be be:\n - a DockElement that was registered with the " +
                            "wrong unique identifier\n - the type of a DockElement was changed\n - a " +
                            "SingleCDockableFactory creating the wrong type of items.\n" +
