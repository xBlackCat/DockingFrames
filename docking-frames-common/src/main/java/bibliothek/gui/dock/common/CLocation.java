 ...
 * this reason clients are strongly encouraged not to store {@link CLocation}s in any kind of collection for later
 * use.</li>
 * <li>There is no code available for storing {@link CLocation} persistently. This is deliberate, as {@link CLocation}s
 * are only good for a momentary snapshot of the location. The framework itself provides facilities to persistently
 * store
 * the location of a {@link Dockable} for a long time. You may have a look at {@link CControl#writeXML(java.io.File)}
 * and
...
public abstract class CLocation {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof CLocation that) {
