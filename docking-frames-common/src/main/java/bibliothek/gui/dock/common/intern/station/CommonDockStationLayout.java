...
public class CommonDockStationLayout {
     ...
     */
    private final DockLayoutInfo layout = new DockLayoutInfo();
    ...
    public void updateLayout(DockFactory<?, ?, Object> factory, PlaceholderStrategy placeholders) {
        try {
            Object data = switch (layout.getKind()) {
                case BYTE ->
                        factory.read(new DataInputStream(new ByteArrayInputStream(layout.getDataByte())), placeholders);
                case XML -> factory.read(layout.getDataXML(), placeholders);
                default -> null;
            };

            if (data != null) {
                layout.setData(new DockLayout<>(factory.getID(), data));
            }
        } catch (IOException e) {
