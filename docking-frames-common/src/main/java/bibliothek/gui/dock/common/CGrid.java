...
public class CGrid {
     ...
     */
    private final SplitDockGrid grid = new SplitDockGrid();
    ...
    /**
     * Adds some placeholders for {@link SingleCDockable}s to this {@link CGrid}. This method does not make any
     * checks concerning
     * the validity of the placeholders, the placeholders will however be checked once the {@link CGrid} is deployed
     * .<br>
     * This method will assume that the {@link CPlaceholderStrategy} is installed and use the method
     * {@link CPlaceholderStrategy#getSingleDockablePlaceholder(String)}<br>
    ...
    public void addSingle(double x, double y, double width, double height, String... identifiers) {
        if (control == null) {
            throw new IllegalStateException("This method is only available if the CGrid was constructed with a " +
                    "CControl");
        }
        ...
        for (int i = 0; i < placeholders.length; i++) {
            placeholders[i] =
                    CPlaceholderStrategy.getSingleDockablePlaceholder(control.getRegister().toSingleId(identifiers[i]));
        }
    }

    /**
     * Adds some placeholders for {@link MultipleCDockable}s to this {@link CGrid}. This method does not make any
     * checks concerning
     * the validity of the placeholders, the placeholders will however be checked once the {@link CGrid} is deployed
     * .<br>
     * This method will assume that the {@link CPlaceholderStrategy} is installed and use the method
     * {@link CPlaceholderStrategy#getMultipleDockablePlaceholder(String)}
     ...
     * @param width       the width of the dockables
     * @param height      the height of the dockables
     * @param identifiers the identifiers that are used when calling
     * {@link CControl#addDockable(String, MultipleCDockable)}
     * @throws IllegalStateException if this {@link CGrid} does not have access to the a {@link CControl}
     */
    public void addMulti(double x, double y, double width, double height, String... identifiers) {
        ...
        for (int i = 0; i < placeholders.length; i++) {
            placeholders[i] =
                    CPlaceholderStrategy.getMultipleDockablePlaceholder(control.getRegister().toMultiId(identifiers[i]));
        }
    }
     ...
     * @throws IllegalArgumentException if <code>dockable</code> is not registered at location
