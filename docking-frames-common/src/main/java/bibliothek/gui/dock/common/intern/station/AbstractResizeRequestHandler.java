        ...
        if (dockable instanceof StackDockStation station) {
            ...
            for (int i = 0, n = station.getDockableCount(); i < n; i++) {
                ...
                if (check != null) {
                    ...
                    if (check.isWidthSet()) {
                        max.setWidth(Math.max(max.getWidth(),
                                check.getWidth() + sizeStation.width - sizeDockable.width));
                    }
                    if (check.isHeightSet()) {
                        max.setHeight(Math.max(max.getHeight(),
                                check.getHeight() + sizeStation.height - sizeDockable.height));
                    }
                }
            }
