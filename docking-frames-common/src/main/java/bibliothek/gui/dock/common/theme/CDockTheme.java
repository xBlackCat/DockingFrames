...
public class CDockTheme<D extends DockTheme> implements DockTheme {
     ...
     */
    private final Map<Path, ColorBridgeFactory> colorBridgeFactories =
            new HashMap<>();

    /**
     * the factories for fonts used in this theme
     */
    private final Map<Path, FontBridgeFactory> fontBridgeFactories =
            new HashMap<>();
    ...
    protected void initDefaultFontBridges(final CControl control) {
        putFontBridgeFactory(TitleFont.KIND_TITLE_FONT, manager -> {
            TitleFontTransmitter transmitter = new TitleFontTransmitter(manager);
            transmitter.setControl(control);
            return transmitter;
        });
        putFontBridgeFactory(TitleFont.KIND_FLAP_BUTTON_FONT, manager -> {
            ButtonFontTransmitter transmitter = new ButtonFontTransmitter(manager);
            transmitter.setControl(control);
            return transmitter;
        });
        putFontBridgeFactory(TabFont.KIND_TAB_FONT, manager -> {
    ...
    protected void install(DockController controller) {
        ...
        try {
            ...
            ExtensionName<CColorBridgeExtension> name = new ExtensionName<>(
                    CColorBridgeExtension.EXTENSION_NAME, CColorBridgeExtension.class,
                    CColorBridgeExtension.PARAMETER_NAME, this);
            ...
            for (Map.Entry<Path, ColorBridgeFactory> entry : colorBridgeFactories.entrySet()) {
                ...
                List<CColorBridgeExtension> filtered = new ArrayList<>();
    ...
    private static class Controller {
         ...
         */
        public final Map<Path, ColorBridge> colors = new HashMap<>();
        /**
         * the list of {@link FontBridge}s which are used in that controller
         */
        public final Map<Path, FontBridge> fonts = new HashMap<>();
    }
}