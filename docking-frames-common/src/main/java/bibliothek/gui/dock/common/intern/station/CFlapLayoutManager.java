...
public class CFlapLayoutManager extends AbstractFlapLayoutManager implements FlapLayoutManager {
     ...
     */
    private final CDockablePropertyListener propertyListener = new CDockableAdapter() {
        @Override
     ...
     */
    private final Map<Dockable, Boolean> holds = new HashMap<>();

    /**
     * temporary storage of sizes for non CommonDockables
     */
    private final Map<Dockable, Integer> sizes = new HashMap<>();
    ...
    private int getMaxSize(Dockable dockable, boolean horizontal) {
        ...
        if (dockable instanceof StackDockStation station) {
            int size = -1;
            for (int i = 0, n = station.getDockableCount(); i < n; i++) {
