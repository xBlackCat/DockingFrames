 ...
 * {@link MultipleCDockableFactory#match(bibliothek.gui.dock.common.MultipleCDockable, bibliothek.gui.dock.common.MultipleCDockableLayout)}
...
public class CLayoutChangeStrategy extends DefaultLayoutChangeStrategy {
     ...
     */
    private static final String REPLACEMENT_FACTORY_ID =
            PredefinedDockSituation.convertFactoryID(ReplacementDockFactory.REPLACEMENT_FACTORY_ID);
    ...
    @Override
    protected PredefinedDockSituation createSituation(DockFrontendInternals frontend, boolean entry,
                                                      boolean onSetLayout) {
     ...
     */
    protected DockLayoutComposition replaceMultipleDockables(DockFrontendInternals frontend, CSettingAccess setting,
        ...
        List<DockLayoutComposition> newChildren = new ArrayList<>(oldChildren.size());
    ...
    /**
     * A {@link bibliothek.gui.dock.frontend.DefaultLayoutChangeStrategy.SettingAccess} that modifies the roots by
     * calling
    ...
    protected class CSettingAccess extends SettingAccess {
        private final DockFrontendInternals frontend;
        private final Map<String, DockLayoutComposition> modifiedRoots = new HashMap<>();

        /**
         * all the factories that might be used to create a match
         */
        private final Map<String, MultipleCDockableFactory<?, ?>> factories;

        /**
         * the dockables which have not yet been paired off ordered by their factories
         */
        private final Map<String, List<MultipleCDockable>> remainingDockables;
            ...
            for (MultipleCDockable dockable : control.getRegister().getMultipleDockables()) {
                for (Map.Entry<String, MultipleCDockableFactory<?, ?>> entry : factories.entrySet()) {
                    if (entry.getValue() == dockable.getFactory()) {
                        ...
                        List<MultipleCDockable> list = remainingDockables.computeIfAbsent(key, k -> new LinkedList<>());
                        list.add(dockable);
                        break;
                    }
                }
        ...
        public MultipleCDockable findMatch(DockLayout<?> layout) {
            ...
            if (data instanceof CommonMultipleDockableLayout) {
                ...
                MultipleCDockableFactory<MultipleCDockable, MultipleCDockableLayout> factory =
                        (MultipleCDockableFactory<MultipleCDockable, MultipleCDockableLayout>) factories.get(factoryId);
                if (factory != null) {
