...
public interface CGroupBehavior {
     ...
     */
    CGroupBehavior TOPMOST = new TopMostGroupBehavior();

    /**
     * A behavior that moves around entire stacks of {@link Dockable}s.
     */
    CGroupBehavior STACKED = new StackGroupBehavior();

    /**
     * Calculates how the mode of <code>dockable</code> has to be changed such that it matches <code>target</code>.
     * Please note that some modules use directly
     * {@link #getGroupElement(LocationModeManager, Dockable, ExtendedMode)} and do never call
     * this method.
     *
     ...
     */
    Dockable getReplaceElement(LocationModeManager<? extends LocationMode> manager, Dockable old, Dockable dockable,
                               ExtendedMode mode);
     ...
     */
    boolean shouldForwardActions(LocationModeManager<? extends LocationMode> manager, DockStation station,
