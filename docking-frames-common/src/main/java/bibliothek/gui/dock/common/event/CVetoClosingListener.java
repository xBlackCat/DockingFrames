 ...
 * 	<li>There is no guarantee that {@link #closing(CVetoClosingEvent)} is called before
 *    {@link #closed(CVetoClosingEvent)}</li>
 * 	<li>The event-object passed to {@link #closing(CVetoClosingEvent)} and {@link #closed(CVetoClosingEvent)} may not
 * 	be equal, even if they describe the same event</li>
 * 	<li>Events may be split: if {@link #closing(CVetoClosingEvent)} was called once and handled three dockables, then
 *    {@link #closed(CVetoClosingEvent)} may be called three times to handle one dockable</li>
 * 	<li> {@link MultipleCDockable}s that are not children of a {@link CWorkingArea} are closed and re-opened if the
 * 	layout changes ({@link CControl#load(String)}). This may be
 *  prevented with a correct implementation of
 *  {@link MultipleCDockableFactory#match(MultipleCDockable, bibliothek.gui.dock.common.MultipleCDockableLayout)
 *  MultipleCDockableFactory.match}. </li>
 * </ul>
 *
...
public interface CVetoClosingListener {
     ...
     */
    void closed(CVetoClosingEvent event);
}
