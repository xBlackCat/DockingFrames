...
public abstract class AbstractCDockable implements CDockable {
     ...
     */
    private final ColorMap colors = new ColorMap(this);

    /**
     * the fonts associated with this dockable
     */
    private final FontMap fonts = new FontMap(this);

    /**
     * the actions that are shown by other modules
     */
    private final Map<String, CAction> actions = new HashMap<>();
     ...
     */
    protected final CListenerCollection listenerCollection = new CListenerCollection();
     ...
     */
    private final CloseActionSource close = new CloseActionSource(this);

    /**
     * The default locations for the available {@link ExtendedMode}s.
     */
    private final Map<ExtendedMode, CLocation> defaultLocations = new HashMap<>(4);
    ...
    private ControlVetoClosingListener getVetoClosingListenerConverter() {
        if (vetoClosingListenerConverter == null) {
            vetoClosingListenerConverter =
                    new ControlVetoClosingListener(control.getOwner(), listenerCollection.getVetoClosingListener()) {
                        @Override
    ...
    public void setVisible(boolean visible) {
        if (control == null) {
            throw new IllegalStateException("This CDockable does not know its CControl. Call CControl.addDockable(.." +
                    ".) to connect this CDockable befor calling setVisible(...).");
        }
    ...
    /**
     * Tries to focus this dockable, and then ensures that the {@link Component} <code>focus</code> actually gets the
     * focus.<br>
     * The behavior of this method is not defined for the case where <code>focus</code> is not a child of
     * <code>this</code>.<br>
     * There is no guarantee of success, this methods fails silently if the focus cannot be gained.
     *
    ...
    public boolean setLocationsAsideFocused() {
        boolean result = setLocationsAside(item ->
                item != AbstractCDockable.this && item.getWorkingArea() == workingArea && item.isVisible());
        if (!result) {
            result = setLocationsAside(item -> item != AbstractCDockable.this && item.getWorkingArea() == workingArea);
        }
        return result;
    }
    ...
    /**
     * Sets the {@link Component} which should receive the focus when this <code>CDockable</code> is focused. Please
     * note
     * that the focus will be transferred to this component every time the dockable lost the focus and gained the
     * focus again. The
     * default behavior of re-focusing the last focus owner should be sufficient for most applications.
     *
     * @param component the component to focus, can be <code>null</code>, should be a child of this
     *                  <code>CDockable</code>
     */
