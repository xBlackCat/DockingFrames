 ...
 */
public class CommonDockStationFactory implements DockFactory<CommonDockStation<?, ?>, CommonDockStationPerspective,
     ...
     * @param singleDockableFactory the factory used to create new {@link CommonDockStation}s that are also
     * {@link SingleCDockable}s, not <code>null</code>
     */
    public CommonDockStationFactory(CControl control, FrontendPerspectiveCache cache,
    ...
    protected CommonDockStation<?, ?> createStation(String id) {
        ...
        if (!factoryId.equals(getID())) {
            throw new IllegalArgumentException("Wrong type of dockable for unique id '" + id +
                    "': The backup factory created a dockable which expects a factory of type '" + factoryId +
        ...
        if (station == null) {
            System.err.println(
                    "unique identifier '" + id + "' was supposed to be a CStation, but factory created a dockable");
            return null;
        }
    ...
    protected void registerStation(CStation<?> station, boolean root) {
        ...
        if (dockable != null) {
            if (dockable instanceof SingleCDockable single) {
    ...
    public CommonDockStationLayout getLayout(CommonDockStation<?, ?> element, Map<Dockable, Integer> children) {
        ...
        DockFactory<DockElement, ?, ?> factory =
                (DockFactory<DockElement, ?, ?>) control.intern().getDockFactory(factoryId);
        if (factory == null) {
            return null;
        }
    ...
    public CommonDockStation<?, ?> layout(CommonDockStationLayout layout, Map<Integer, Dockable> children,
    ...
    @SuppressWarnings("unchecked")
    public void setLayout(CommonDockStation<?, ?> element, CommonDockStationLayout layout,
                          Map<Integer, Dockable> children, PlaceholderStrategy placeholders) {
        String factoryId = element.getConverterID();
        DockFactory<DockElement, ?, Object> factory =
                (DockFactory<DockElement, ?, Object>) control.intern().getDockFactory(factoryId);
        if (factory == null) {
            return;
        }
    ...
    public CommonDockStationPerspective layoutPerspective(CommonDockStationLayout layout, Map<Integer,
    ...
    @SuppressWarnings("unchecked")
    public void layoutPerspective(CommonDockStationPerspective element, CommonDockStationLayout layout, Map<Integer,
            PerspectiveDockable> children) {
        CStationPerspective station = element.getElement().asStation();
        station.setRoot(layout.isRoot());

        DockFactory<?, PerspectiveElement, Object> factory =
                (DockFactory<?, PerspectiveElement, Object>) control.intern().getDockFactory(layout.getFactoryId());
        if (factory == null) {
            return;
        }
    ...
    @SuppressWarnings("unchecked")
    public CommonDockStationLayout getPerspectiveLayout(CommonDockStationPerspective element, Map<PerspectiveDockable
            , Integer> children) {
        String factoryId = element.getConverterID();
        DockFactory<?, PerspectiveElement, Object> dockFactory =
                (DockFactory<?, PerspectiveElement, Object>) control.intern().getDockFactory(factoryId);
