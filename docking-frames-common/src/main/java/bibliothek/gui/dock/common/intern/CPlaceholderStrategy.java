...
public class CPlaceholderStrategy implements PlaceholderStrategy {
     ...
     */
    private final List<PlaceholderStrategyListener> listeners = new ArrayList<>();

    /**
     * a map containing placeholders for various {@link MultipleCDockable}s
     */
    private final Map<MultipleCDockable, Path> multiplePlaceholders = new HashMap<>();

    private final CControlListener controlListener = new CControlListener() {
    ...
    private final CDockFrontendListener frontendListener = new CDockFrontendListener() {
        public void loading(CDockFrontend frontend, CSetting setting) {
            pendingChecks = new HashSet<>();
        }
    ...
    protected void fireInvalidated(Path placeholder) {
        ...
        placeholders = Set.of(placeholder);
        for (PlaceholderStrategyListener listener : listeners.toArray(new PlaceholderStrategyListener[0])) {
    ...
    public Path getPlaceholderFor(Dockable dockable) {
        ...
        if (cdockable instanceof MultipleCDockable) {
            ...
            if (access != null) {
                Path result =
                        getMultipleDockablePlaceholder(controlAccess.getRegister().multiToNormalId(access.getUniqueId()));
