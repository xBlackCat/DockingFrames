 ...
 *  <li>Any root-{@link CStation} used by a {@link CControl} needs to be registered using
 *  {@link #addStation(CStationPerspective)}</li>
 * </ul>
 *
 * @author Benjamin Sigg
 */
@Todo(compatibility = Compatibility.COMPATIBLE, priority = Priority.MAJOR, description = "remove the warning about " +
        "modes without perspective")
public class CPerspective {
    /**
     * All the stations of this perspective
     */
    private final Map<String, CStationPerspective> stations = new HashMap<>();

    /**
     * All the dockables known to this perspective, only updated on a call to {@link #storeLocations()}
     */
    private final Map<String, CDockablePerspective> dockables = new HashMap<>();
    ...
    public String[] getDockableKeys() {
        return dockables.keySet().toArray(new String[0]);
    }
     ...
     */
    public void shrink() {
        List<ShrinkablePerspectiveStation> elements = new ArrayList<>();
    ...
    private static class ElementFrame {
        public final PerspectiveElement[] items;
        public int offset;
    ...
    private class ElementIterator implements Iterator<PerspectiveElement> {
        private final LinkedList<ElementFrame> stack = new LinkedList<>();

        public ElementIterator() {
            List<PerspectiveElement> items = new ArrayList<>();
            ...
            stack.addFirst(new ElementFrame(items.toArray(new PerspectiveElement[0])));
        }
