 ...
 * {@link #hierarchyChanged(Dockable)} was called last. Basically the placeholder always follows the last
 * {@link Dockable}
...
public class PlaceholderGrouping implements DockableGrouping {
    ...
    /**
     * Placeholder that marks the last position where this {@link PlaceholderGrouping} did store something. The
    ...
    /**
     * Called by {@link #getValidatedLocation(Dockable, CLocationMode, Location)}, this method tries to find a
     * location on the
     ...
     * @return either a replacement for <code>validatedHistory</code>, or <code>validatedHistory</code>. Should not
    ...
    protected Location findLocationFor(Dockable dockable, CLocationMode mode, Location validatedHistory) {
        ...
        if (root != null && root.getPlaceholderMapping().hasPlaceholder(placeholder)) {
            ...
            if (location != null) {
                return new Location(mode.getUniqueIdentifier(), validatedHistory.getRoot(), location,
                        validatedHistory.isApplicationDefined());
            }
        }
     ...
     * @return the location of {@link #getPlaceholder() the placeholder}, or <code>null</code> if the placeholder was
     * not found
     */
