...
public class DocKey {
    ...
    public static DocKey[] of(DocRoot root, CssDocKey... keys) {
        List<DocKey> result = new ArrayList<>();
        for (CssDocKey key : keys) {
            result.addAll(Arrays.asList(of(root, key)));
        }
        return result.toArray(new DocKey[0]);
    }
    ...
    public static DocKey[] of(DocRoot root, CssDocKey key) {
        if (key.reference() == Object.class) {
            ...
            if (replacement != null) {
                results.addAll(Arrays.asList(of(root, replacement)));
            }
            if (replacements != null) {
                for (CssDocKey sub : replacements.value()) {
                    results.addAll(Arrays.asList(of(root, sub)));
                }
            }

            return results.toArray(new DocKey[0]);
        }
    }
