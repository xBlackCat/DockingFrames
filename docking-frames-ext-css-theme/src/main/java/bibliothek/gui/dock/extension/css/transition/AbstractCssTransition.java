...
public abstract class AbstractCssTransition<T> extends AbstractCssPropertyContainer implements CssTransition<T> {
     ...
     */
    private final Map<CssPropertyKey, AnimatedProperty<?>> properties = new HashMap<>();

    private CssType<T> type;

    private final CssRuleContentListener listener = new CssRuleContentListener() {
        @Override
    ...
    public void init(CssRuleContent current, CssTransitionCallback callback) {
        ...
        for (CssPropertyKey key : keys) {
            if (propertyFilter == null || propertyFilter.includes(key)) {
                AnimatedProperty<T> property = new AnimatedProperty<>(key, createProperty(type, key), type);
    ...
    private class AnimatedProperty<S> implements TransitionalCssPropertyCallback<S>, CssSchedulable {
        private final TransitionalCssProperty<S> property;

        private final CssPropertyKey key;
        private final CssType<S> type;

        private final Map<CssPropertyKey, CssProperty<?>> dependencies = new HashMap<>();
