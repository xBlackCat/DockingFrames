...
public class DefaultCssSelector implements CssSelector {
    ...
    private DefaultCssSelector(Step[] steps) {
        ...
        for (Step step : steps) {
            ...
            if (next != null) {
                switch (next) {
                    case ATTRIBUTE -> countAttribute++;
                    case ELEMENT -> countElement++;
                    case ID -> countId++;
                    case STYLE -> countStyle++;
                }
            }
        }
    ...
    private interface Step {
         ...
         */
        int[] matches(CssPath path, int offset);
    ...
    private record Element(String name) implements Step {
    ...
    private record Child(String name) implements Step {
    ...
    private record PseudoClass(String name) implements Step {
    ...
    private record Attribute(String key, String value) implements Step {
        @Override
        public int[] matches(CssPath path, int offset) {
            if (value == null) {
    ...
    private record ItemClass(String name) implements Step {
    ...
    private record Identifier(String name) implements Step {
    ...
    public static class Builder {
        ...
        public DefaultCssSelector build() {
            return new DefaultCssSelector(steps.toArray(new Step[0]));
        }
    }
}
