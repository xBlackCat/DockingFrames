...
public class CssScheme {
    ...
    private final Map<CssItem, Match> items = new HashMap<>();
    private final Map<Class<?>, CssType<?>> types = new HashMap<>();
    ...
    private final CssRuleListener selectorChangedListener = source -> {
    ...
    public void rematch() {
        if (!rematchPending) {
            EventQueue.invokeLater(this::match);
        }
    }
    ...
    private void ensureRulesSorted() {
        if (!rulesAreSorted) {
            synchronized (RULES_LOCK) {
                rules.sort(Comparator.comparing(a -> a.getSelector().getSpecificity()));
            }
            rulesAreSorted = true;
        }
    }
    ...
    private class Match implements CssItemListener, CssPathListener {
        private final TransitionalCssRuleChain chain;
    ...
    private record Destroy(MatchedCssRule rule) implements Runnable {
        @Override
            public void run() {
                rule.destroy();
            }
