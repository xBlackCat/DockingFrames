...
/**
 * A {@link CssTransition} describes the transition between one {@link CssRule} to another {@link CssRule}. The
 * transition
 ...
 * property, then one of the transitions is silently ignored. If a customized {@link TransitionalCssRuleChain} is in
 * use,
...
public interface CssTransition<T> extends CssPropertyContainer {
     ...
     */
    void setPropertyFilter(Filter<CssPropertyKey> propertyFilter);
     ...
     */
    boolean isInput(CssPropertyKey property);
     ...
     */
    void init(CssRuleContent source, CssTransitionCallback callback);

    /**
     * Called asynchronously if {@link CssTransitionCallback#step()} is called, or if one of the underlying
     * {@link CssRuleContent}s
     ...
     * to the properties of <code>destination</code>, and then shutdown by calling
     * {@link CssTransitionCallback#destroyed()}.
     *
     * @param destination the target rule which should be adapted by this transition
     */
    void transition(CssRuleContent destination);
}
