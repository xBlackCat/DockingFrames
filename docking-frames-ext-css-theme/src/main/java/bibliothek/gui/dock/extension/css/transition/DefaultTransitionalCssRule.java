...
public class DefaultTransitionalCssRule extends AbstractTransitionalCssRule {
    private final CssRuleContent root;
    private final WrappedCssRuleContent source;
    private final List<Transition> transitions = new ArrayList<>();

    private final WrappedCssRuleContent target;
    ...
    public <T> T getProperty(CssType<T> type, CssPropertyKey property) {
        ...
        if (!transitional.type.equals(type)) {
            throw new IllegalArgumentException(
                    "type conflict, expected " + type + ", but found " + transitional.type + " for property " +
                            property);
        }
        return (T) transitional.value;
    }

    private record TransitionalProperty<T>(CssType<T> type, T value) {}

    private class TargetDependencies extends AbstractCssPropertyContainer {
        private final Map<String, CssProperty<?>> dependencies = new HashMap<>();
        private final PropertyForwarder targetForwarder;
    ...
    private class Transition extends AbstractCssPropertyContainer implements CssTransitionCallback, CssSchedulable {
        private final CssPropertyKey transitionalKey;
        private CssTransition<?> transition;
        private final Map<CssPropertyKey, TransitionalProperty<?>> overridenProperties = new HashMap<>();
        private final Map<String, CssProperty<?>> sourceDependencies = new HashMap<>();
        private final TargetDependencies targetDependencies;

        private final PropertyForwarder sourceForwarder;
        ...
        @Override
        public String[] getPropertyKeys() {
            return sourceDependencies.keySet().toArray(new String[0]);
        }
