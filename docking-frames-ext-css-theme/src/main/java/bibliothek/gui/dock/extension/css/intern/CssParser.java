...
public class CssParser {
    ...
    private CssSelector[] toSelectors(int line, String selector) throws IOException {
        ...
        return result.toArray(new CssSelector[0]);
    }
    ...
    private static class SelectorParser {
        private final DefaultCssSelector.Builder builder = DefaultCssSelector.selector();
        ...
        private void endCurrent(int line) {
            ...
            if (next.length() > 0) {
                ...
                if (!nextIsPseudoClass && !nextIsClass && !nextIsSibling && !nextIsChild && !nextIsSibling &&
                        !nextIsIdentifier) {
                    if ("*".equals(next)) {
                        builder.any();
                    } else {
