...
import bibliothek.gui.dock.extension.css.path.*;
...
public class CssTree {
     ...
     */
    private final Map<Class<?>, CssRelationNodeFactory<?>> relationFactories = new HashMap<>();

    /**
     * factories for creating {@link CssNode}s
     */
    private final Map<Class<?>, CssNodeFactory<?>> nodeFactories = new HashMap<>();

    /**
     * cached paths for the known existing {@link DockElement}s
     */
    private final Map<DockElement, CssPath> pathCache = new HashMap<>();

    /**
     * cached nodes for the known existing {@link DockElement}s
     */
    private final Map<DockElement, CssNode> selfCache = new HashMap<>();
    ...
    private void initRelationFactories() {
        putRelationFactory(StackDockStation.class, StackDockStationNode::new);
        putRelationFactory(ScreenDockStation.class, ScreenDockStationNode::new);
        putRelationFactory(FlapDockStation.class, FlapDockStationNode::new);
        putRelationFactory(SplitDockStation.class, SplitDockStationNode::new);
    }

    @Todo(compatibility = Compatibility.COMPATIBLE, priority = Priority.MAJOR,
            target = Version.VERSION_1_1_2, description = "have better self nodes, e.g. a dockable could have a " +
            "property for its title text")
    private void initNodeFactories() {
        putFactory(StackDockStation.class, object -> new NamedCssNode("stack"));
        putFactory(FlapDockStation.class, object -> new NamedCssNode("flap"));
        putFactory(ScreenDockStation.class, object -> new NamedCssNode("screen"));
        putFactory(SplitDockStation.class, object -> new NamedCssNode("split"));
        putFactory(Dockable.class, element -> new NamedCssNode("dockable"));
        putFactory(DockElement.class, element -> new NamedCssNode("element"));
    }
    ...
    @CssDocPath(
            id = "getPathFor",
            description = @CssDocText(text = "Generic path for a Dockable or a DockStation, the nodes are created by " +
                    "different CssNodeFactorys."),
            unordered = {
                    @CssDocPathNode(name = @CssDocKey(key = "split", description = @CssDocText(text = "Denotes a " +
                            "SplitDockStation when using the default CssNodeFactories"))),
                    @CssDocPathNode(name = @CssDocKey(key = "flap", description = @CssDocText(text = "Denotes a " +
                            "FlapDockStation when using the default CssNodeFactories"))),
                    @CssDocPathNode(name = @CssDocKey(key = "stack", description = @CssDocText(text = "Denotes a " +
                            "StackDockStation when using the default CssNodeFactories"))),
                    @CssDocPathNode(name = @CssDocKey(key = "screen", description = @CssDocText(text = "Denotes a " +
                            "ScreenDockStation when using the default CssNodeFactories"))),
                    @CssDocPathNode(name = @CssDocKey(key = "dockable", description = @CssDocText(text = "Denotes a " +
                            "Dockable when using the default CssNodeFactories"))),
                    @CssDocPathNode(name = @CssDocKey(key = "element", description = @CssDocText(text = "Denotes a " +
                            "generic DockElement when using the default CssNodeFactories"))),
    ...
    public CssNode getRelationNode(DockElement element) {
        ...
        while (clazz != null) {
            CssRelationNodeFactory<DockStation> factory =
                    (CssRelationNodeFactory<DockStation>) relationFactories.get(clazz);
            if (factory == null) {
