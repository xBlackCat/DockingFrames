...
public class ToolbarItemDockable extends AbstractDockable implements ExpandableToolbarItem {
     ...
     */
    private final List<MouseInputListener> mouseListeners = new ArrayList<>();
    ...
    public void setItem(ToolbarItem item, ExpandedState state) {
        ...
        if (newEnabled != enabled) {
            for (ExpandableToolbarItemListener listener :
                    expandableListeners.toArray(new ExpandableToolbarItemListener[0])) {
    ...
    private void forceState(ExpandedState state) {
        ...
        content.revalidate();
        if (oldState != state) {
            for (ExpandableToolbarItemListener listener : expandableListeners.toArray(new ExpandableToolbarItemListener[0])) {
