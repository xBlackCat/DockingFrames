...
public class WizardColumnModel {
     ...
     * @param cellSizes       the size of each cell, this array must have the same dimensions as
     *                        <code>columnsAndCells</code>
     * @param columnSizes     the size of each column, this array must have the same dimensions as
     *                        <code>columnsAndCells</code>
     */
    public void setPersistentColumns(Dockable[][] columnsAndCells, int[][] cellSizes, int[] columnSizes) {
        ...
        for (int i = 0; i < columnsAndCells.length; i++) {
            loop:
            for (int j = 0; j < columnsAndCells[i].length; j++) {
    ...
    public void setDivider(Divideable divideable, double divider) {
        ...
        if (divideable instanceof Node node) {
            Column column;
     ...
     * children of <code>node</code> and forwards the call to
     * {@link #updateBounds(SplitNode, double, double, double, double, WizardNodeMap)}
     ...
     * @param map    information about the current layout
     */
    protected void updateBoundsRecursive(SplitNode node, double x, double y, double width, double height,
                                         WizardNodeMap map) {
        if (node != null && node.isVisible()) {
            if (node instanceof Root) {
                updateBounds(((Root) node).getChild(), x, y, width, height, map);
            } else if (node instanceof Node n) {
                if (n.getLeft() != null && n.getLeft().isVisible() && n.getRight() != null &&
                        n.getRight().isVisible()) {
                    if (n.getOrientation() == Orientation.HORIZONTAL) {
                        ...
                        updateBounds(n.getRight(),
                                x + dividerLocation + dividerWidth / 2, y,
                                width - dividerLocation - dividerWidth / 2, height, map);
                    } else {
                        ...
                        updateBounds(n.getRight(), x,
                                y + dividerLocation + dividerHeight / 2, width,
                                height - dividerLocation - dividerHeight / 2, map);
                    }
                } else {
