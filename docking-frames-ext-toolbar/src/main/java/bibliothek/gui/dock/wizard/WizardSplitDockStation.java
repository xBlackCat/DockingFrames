...
/**
 * A {@link WizardSplitDockStation} has some additional restrictions and other behavior than an ordinary
 ...
 *  <li>This station should be wrapped into a {@link JScrollPane}, it even implements {@link Scrollable} to fully
 *  support the {@link JScrollPane}.</li>
 * </ul>
 *
...
public class WizardSplitDockStation extends SplitDockStation implements Scrollable {
    ...
    private final WizardLayoutManager layoutManager;
    private final WizardSpanStrategy wizardSpanStrategy;
    private Side side;
    ...
    @Override
    protected boolean dropAside(SplitNode neighbor, Put put, Dockable dockable, Leaf leaf, double divider,
                                Token token) {
                ...
                root().visit(new SplitNodeVisitor() {
                    ...
                    public void handleNode(Node node) {
                        switch (node.getOrientation()) {
                            case HORIZONTAL -> node.setOrientation(Orientation.VERTICAL);
                            case VERTICAL -> node.setOrientation(Orientation.HORIZONTAL);
                        }
                    }
     ...
     *
     * @param maxColumnCount the maximum amount of columns, <code>-1</code> or any number that is greater than
     *                       <code>0</code>
     */
        ...
        while (true) {
            ...
            if (parent instanceof Node n) {
                ...
                if (superParent instanceof Root) {
                    ...
                    return;
                } else if (superParent instanceof Node s) {
                    if (n.getLeft() == node) {
                        if (s.getLeft() == n) {
                            s.setLeft(node);
     ...
     * <ul>
     * 	<li>DnD operations must ensure that the {@link Dockable}s remain in columns, see
     *    {@link #ensureDropLocation(PutInfo)}</li>
     * </ul>
     *
     * @author Benjamin Sigg
     */
    public class WizardLayoutManager extends DefaultSplitLayoutManager {
        private final WizardColumnModel model;
