...
public class WizardSplitDockStationFactory extends SplitDockStationFactory {
    ...
    public void setLayout(SplitDockStation station, SplitDockStationLayout splitLayout,
        ...
        for (int i = 0; i < columns.length; i++) {
            columnsAndCells[i] = new Dockable[columns[i].cellKeys().length];
            cellSizes[i] = new int[columnsAndCells[i].length];
            columnSizes[i] = columns[i].size();
            for (int j = 0; j < cellSizes[i].length; j++) {
                columnsAndCells[i][j] = children.get(columns[i].cellKeys()[j]);
                cellSizes[i][j] = columns[i].cellSizes()[j];
            }
        }
    ...
    public void write(SplitDockStationLayout layout, DataOutputStream out) throws IOException {
        ...
        for (Column column : columns) {
            ...
            int[] keys = column.cellKeys();
            int[] sizes = column.cellSizes();
    ...
    public void write(SplitDockStationLayout layout, XElement element) {
        ...
        for (Column column : columns) {
            ...
            xcolumn.addInt("size", column.size());
            int[] keys = column.cellKeys();
            int[] sizes = column.cellSizes();
            for (int i = 0; i < keys.length; i++) {
