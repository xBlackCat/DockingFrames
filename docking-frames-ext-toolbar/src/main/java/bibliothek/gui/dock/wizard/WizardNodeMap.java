...
public abstract class WizardNodeMap {
    ...
    private void buildColumns() {
        ...
        Column[] array = columns.values().toArray(new Column[0]);
    ...
    public Column[] getSortedColumns() {
        ...
        Column[] array = columns.toArray(new Column[0]);
        Arrays.sort(array, Comparator.comparingInt(Column::getIndex));
        return array;
    }
    ...
    public Leaf[] getLastLeafOfColumns() {
        ...
        return result.toArray(new Leaf[0]);
    }
    ...
    public PersistentColumn[] getPersistentColumns() {
        List<PersistentColumn> result = new ArrayList<>(getColumns().size());
        for (Column column : getColumns().values()) {
        ...
        if (persistentColumns == null) {
            persistentColumns = result.toArray(new PersistentColumn[0]);
        } else {
            persistentColumns = adapt(persistentColumns, result.toArray(new PersistentColumn[0]));
        }
    ...
    private PersistentColumn[] adapt(PersistentColumn[] oldColumns, PersistentColumn[] newColumns) {
        for (PersistentColumn column : newColumns) {
            ...
            Set<PersistentColumn> sources = new HashSet<>();
            contentLoop:
            for (Map.Entry<Dockable, PersistentCell> entry : column.getCells().entrySet()) {
    ...
    public class Column {
        ...
        private final Map<SplitNode, Cell> cells = new HashMap<>();
        private final List<Cell> leafCells = new ArrayList<>();
        private int index;
        ...
        private Column(SplitNode root) {
            ...
            Cell[] array = leafCells.toArray(new Cell[0]);
        ...
        public Cell[] getSortedCells() {
            Cell[] array = cells.values().toArray(new Cell[0]);
            Arrays.sort(array, Comparator.comparingInt(Cell::getIndex));
            return array;
        }
        ...
        private Map<Dockable, PersistentCell> getLeafs() {
            final Map<Dockable, PersistentCell> leafs = new HashMap<>();
            root.visit(new SplitNodeVisitor() {
                @Override
                public void handleRoot(Root root) {
                    // ignore
                ...
                if (node instanceof Node) {
                    ...
                    if (left == null) {
                        ...
                        if (((Node) node).getOrientation() == Orientation.HORIZONTAL) {
                            preferredSize =
                                    new Dimension(left.width + right.width, Math.max(left.height, right.height));
                        } else {
                            preferredSize =
                                    new Dimension(Math.max(left.width, right.width), left.height + right.height);
                        }
                    }
                }
                if (node instanceof Root) {
