...
import bibliothek.gui.dock.station.toolbar.group.*;
...
public class ToolbarGroupDockStation extends AbstractToolbarDockStation {
    ...
    public static final PropertyKey<ColumnScrollBarFactory> SCROLLBAR_FACTORY =
            new PropertyKey<>("dock.scrollbarFactory", new ConstantPropertyFactory<>(SlimScrollbar.FACTORY), true);
    ...
    public static final PropertyKey<ToolbarGroupHeaderFactory> HEADER_FACTORY =
            new PropertyKey<>("dock.toolbarGroupHeaderFactory");

    /**
     * Key for a strategy for painting borders between the {@link Dockable}s
     */
    public static final PropertyKey<ToolbarGroupDividerStrategyFactory> DIVIDER_STRATEGY_FACTORY =
            new PropertyKey<>("dock.toolbarGroupDividerStrategy",
                    new ConstantPropertyFactory<>(DefaultToolbarGroupDividierStrategy.FACTORY), true);
     ...
     */
    private final PropertyValue<ToolbarGroupDividerStrategyFactory> dividerStrategyFactory =
            new PropertyValue<>(DIVIDER_STRATEGY_FACTORY) {
                @Override
                protected void valueChanged(ToolbarGroupDividerStrategyFactory oldValue,
                                            ToolbarGroupDividerStrategyFactory newValue) {
                    if (newValue == null) {
     ...
     */
    private final PropertyValue<ColumnScrollBarFactory> scrollbarFactory = new PropertyValue<>(SCROLLBAR_FACTORY) {
        @Override
     ...
     */
    private final Map<Integer, ColumnScrollBar> scrollbars = new HashMap<>();

    /**
     * this listener is added to all {@link ColumnScrollBar}s and ensures an update of positions if the bar changes
     * its value
     */
     ...
     */
    private final PropertyValue<ToolbarGroupHeaderFactory> headerFactory = new PropertyValue<>(HEADER_FACTORY) {
        @Override
    ...
    public ToolbarStrategy getToolbarStrategy() {
        ...
        final SilentPropertyValue<ToolbarStrategy> value =
                new SilentPropertyValue<>(ToolbarStrategy.STRATEGY, controller);
     ...
     *
     * @param allowSideSnap <code>true</code> if the station can snap Dockables which are
     *                      near.
    ...
    public StationDropOperation prepareDrop(StationDropItem item) {
        ...
        if (this.accept(dockable) && dockable.accept(this)) {
            ...
            if (dockable.getDockParent() == this) {
                ...
                effect = effect && !(line == -1 && lineCount(currentColumn) == 1 &&
                        (currentColumn == column || currentColumn == column - 1));
            }
     ...
     * @param column   the column where insert
     * @param line     the line where insert
     ...
     *
     * @param dockable the dockable to insert
     * @param column   the column index to create
     * @return true if the dockable has been inserted, false otherwise
     */
     ...
     * @param all      whether there should be an event for all the columns after
     ...
     *
     * @param dockable some item from a column that changed
     * @param all      whether there should be an event for all the columns after the
    ...
    protected void fireColumnRepositioned(int column, boolean all) {
        final List<Dockable> list = new ArrayList<>();
        ...
        if (list.size() > 0) {
            listeners.fireDockablesRepositioned(list.toArray(new Dockable[0]));
        }
    }
     ...
     * @throws IllegalArgumentException if <code>displayer</code> is not a child of this station
     */
    @Override
    ...
    protected class OverpaintablePanelBase extends SecureContainer {
         ...
         * decorations.
         */
        private final JPanel decorationPane = createBackgroundPanel();
        ...
        public void updateAlignment() {
            ...
            if (orientation != null) {
                ...
                layoutManager =
                        new ToolbarGridLayoutManager<>(dockablePane, orientation, dockables,
                                ToolbarGroupDockStation.this) {
                            @Override
                            ...
                            protected void setShowScrollbar(int column, boolean show) {
                                ...
                                if (show) {
                                    if (!scrollbars.containsKey(column)) {
                                        ColumnScrollBar bar =
                                                scrollbarFactory.getValue().create(ToolbarGroupDockStation.this);
        ...
        protected void paintOverlay(Graphics g) {
            ...
            if (dropInfo != null && dropInfo.hasEffect()) {
                ...
                paint.drawInsertion(g2D, new Rectangle(zero.x, zero.y, dockablePane.getWidth(),
                        dockablePane.getHeight()), gapBounds);
            }
        }
    ...
    public void aside(AsideRequest request) {
        ...
        if (location instanceof ToolbarGroupProperty groupLocation) {
