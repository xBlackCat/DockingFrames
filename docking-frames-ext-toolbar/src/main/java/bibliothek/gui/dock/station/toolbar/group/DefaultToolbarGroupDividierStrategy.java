...
public class DefaultToolbarGroupDividierStrategy implements ToolbarGroupDividerStrategy {
    ...
    public static final ToolbarGroupDividerStrategyFactory FACTORY =
            DefaultToolbarGroupDividierStrategy::new;
    ...
    public void paint(Component parent, Graphics g, ToolbarGridLayoutManager<StationChildHandle> layoutManager) {
        ...
        for (int i = 0, n = model.getColumnCount(); i < n; i++) {
            ...
            for (int j = 0, m = column.getDockableCount(); j < m; j++) {
                ...
                if (j == 0) {
                    switch (orientation) {
                        case HORIZONTAL -> g.drawRect(bounds.x, bounds.y - 1, bounds.width, bounds.height + 1);
                        case VERTICAL -> g.drawRect(bounds.x - 1, bounds.y, bounds.width + 1, bounds.height);
                    }
                } else {
