...
public class ToolbarGroupTitle extends ColumnDockTitle {
     ...
     */
    protected final TitleColor color;
    ...
    protected BasicTitleViewItem<JComponent> createItemFor(DockAction action, Dockable dockable) {
        return dockable.getController().getActionViewConverter().createView(action, ToolbarExtension.TOOLBAR_TITLE,
                dockable);
    }
    ...
    public void paintBackground(Graphics g, JComponent component) {
        ...
        if (getOrientation().isHorizontal()) {
            final RoundRectangle2D rectangleRounded =
                    new RoundRectangle2D.Double(0, 0, getWidth(), getHeight() * 2, 8, 8);
            g2D.fill(rectangleRounded);
        } else {
            final RoundRectangle2D rectangleRounded =
                    new RoundRectangle2D.Double(0, 0, getWidth() * 2, getHeight(), 8, 8);
            g2D.fill(rectangleRounded);
        }
        g2D.dispose();
    }
