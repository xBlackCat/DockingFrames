...
public abstract class ToolbarGridLayoutManager<P extends PlaceholderListItem<Dockable>> implements LayoutManager2 {
     ...
     */
    private final boolean useScrollbars = true;
     ...
     */
    private final List<ToolbarGridLayoutManagerListener> listeners = new ArrayList<>();
    ...
    public ToolbarGridLayoutManager(Container parent, Orientation orientation, DockablePlaceholderToolbarGrid<P> grid
        ...
        this.grid = grid;
        this.parent = parent;
        this.spans = new ToolbarGroupSpanStrategy<>(grid, station) {
            @Override
    ...
    protected Wrapper[][] layout() {
        ...
        if (cachedComponents == null) {
            ...
            for (int i = 0; i < components.length; i++) {
                ...
                components[i] = list.toArray(new Wrapper[0]);
            }
     ...
     */
    protected void layout(Container parent, Wrapper[][] components, Dimension required, Dimension available,
                          Size size) {
        ...
        if (orientation == Orientation.HORIZONTAL) {
            ...
            if (factor < 1) {
                ...
                columns[columns.length - 1].height =
                        Math.min(available.height - sum, columns[columns.length - 1].height);
            } else {
                factor = 1;
            }
            int y = 0;
     ...
     * @param startPoint  where to insert the column, e.g. if the orientation is horizontal, then this point is the y
     *                       coordinate
     */
