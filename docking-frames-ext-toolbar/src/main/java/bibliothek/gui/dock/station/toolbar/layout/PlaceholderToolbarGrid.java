...
public abstract class PlaceholderToolbarGrid<D, S, P extends PlaceholderListItem<D>> {
     ...
     * <code>true</code>
     */
    private final PlaceholderStrategyListener strategyListener = placeholders -> purge();
    ...
    /**
     * Called if the column <code>column</code> was removed from this grid. The column may still contain elements
     * when it
     * is removed.
     *
     ...
     * @param columnIndex the column to add
     * @param item        the item to store, not <code>null</code>
     * @param reuse       if <code>false</code> then a new column will be built in any
    ...
    public void insert(int columnIndex, P item, boolean reuse) {
        ...
        if (reuse) {
            ...
            if (columns.dockables().size() > 0) {
                baseIndex = columns.levelToBase(Math.max(0, Math.min(
                        columns.dockables().size() - 1, columnIndex)), Level.DOCKABLE);
                baseIndex--;
            } else {
                baseIndex = 0;
            }
     ...
     *
     * @param sourceColumn      the column in which to find the item, only includes non-empty
     *                          columns
     ...
     * @param destinationLevel  the level at which to find <code>destinationColumn</code>,
     *                          will be converted to an index from {@link Level#BASE}
     * @throws IllegalArgumentException if any index is out of bounds
     */
    public void move(int sourceColumn, int sourceLine, int destinationColumn, int destinationLine,
        ...
        if ((destinationColumn == -1) || (destinationColumn == columns.list().size())) {
            ...
            final PlaceholderList<ColumnItem<D, S, P>, ColumnItem<D, S, P>, Column<D, S, P>>.Item item =
                    columns.new Item(columns.createColumn(list));
     ...
     *
     * @param column      the column in which to add <code>placeholder</code>, only
    ...
    public void insertPlaceholder(int column, int line, Path placeholder) {
        ...
        PlaceholderList<ColumnItem<D, S, P>, ColumnItem<D, S, P>, Column<D, S, P>>.Item item =
                columns.list().get(column);
        if (item.getDockable() == null) {
    ...
    private void ensureRemoved(PlaceholderList<D, S, P> ignore, Set<Path> placeholders) {
        final Iterator<PlaceholderList<ColumnItem<D, S, P>, ColumnItem<D, S, P>, Column<D, S, P>>.Item> iter =
    ...
    public Iterator<P> getColumnContent(final int index) {
        ...
        return new Iterator<>() {
            private final Iterator<P> delegate = list.dockables().iterator();
            private P current;
            private int currentIndex = -1;
                ...
                if (list.dockables().size() == 0) {
                    item.setPlaceholderMap(list.toMap(new PlaceholderListItemAdapter<D, PlaceholderListItem<D>>() {
                        ...
                        public ConvertedPlaceholderListItem convert(int index, PlaceholderListItem<D> dockable) {
                            throw new IllegalStateException("the list is supposed to have no children, so this " +
                                    "conversion method must never be called");
                        }
                    }));
     ...
     * position.
     *
     * @param identifiers identifiers for all children of the {@link DockStation} using
     *                    this grid
    ...
    public PlaceholderMap toMap(final Map<D, Integer> identifiers) {
        ...
        try {
            return columns.toMap(new PlaceholderListItemAdapter<>() {
                @Override
    ...
    public void fromMap(PlaceholderMap map, final Map<Integer, D> identifiers, final PlaceholderToolbarGridConverter<D, P> converter) {
        ...
        try {
            columns.read(map, new PlaceholderListItemAdapter<>() {
                @Override
