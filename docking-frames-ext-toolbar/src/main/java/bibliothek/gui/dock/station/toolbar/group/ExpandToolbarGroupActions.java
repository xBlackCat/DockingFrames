 ...
 */
public abstract class ExpandToolbarGroupActions<P> extends AbstractToolbarGroupActions<P,
        ExpandToolbarGroupActions<P>.ExpandColumn> {
    /**
    ...
    private final PropertyValue<Boolean> onConflictEnable =
            new PropertyValue<>(AbstractToolbarDockStation.ON_CONFLICT_ENABLE) {
                @Override
    ...
    protected class ExpandColumn extends AbstractToolbarGroupActions<P, ExpandColumn>.Column {
        private final ExpandSource source;
        ...
        public void performAction(Action action) {
            ...
            ExpandedState next = switch (action) {
                case LARGER -> current.larger();
                case LARGEST -> ExpandedState.EXPANDED;
                case SMALLER -> current.smaller();
                case SMALLEST -> ExpandedState.SHRUNK;
            ...
            while (attempts > 0 && current != next && !canPerform[next.ordinal()]) {
                attempts--;
                next = switch (action) {
                    case LARGER, SMALLEST -> next.larger();
                    case LARGEST, SMALLER -> next.smaller();
                };
            }
    ...
    private class ExpandSource extends AbstractDockActionSource {
        private final ExpandColumn column;
        ...
        private void findEnabledActions() {
            ...
            if (actions == null || actions.length != length) {
                ...
                for (int i = 0; i < next.length; i++) {
                    if (next[i] == null) {
                        ...
                        final int index = i;
                        next[i].addActionListener(e -> onAction(index));
                    }
                }
                actions = next;
            }
            ...
            if (canExpand && canShrink && canStretch) {
                ...
                switch (state) {
                    case SHRUNK -> actions[0].setBehavior(Action.LARGER);
                    default -> actions[0].setBehavior(Action.SMALLER);
                }
            } else if (canStretch && canExpand) {
                switch (state) {
                    case EXPANDED -> actions[0].setBehavior(Action.SMALLER);
                    default -> actions[0].setBehavior(Action.LARGER);
                }
            }
        }
