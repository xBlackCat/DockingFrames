...
public class ToolbarContainerDropLayer extends DefaultDropLayer {
    ...
    public boolean contains(int x, int y) {
        ...
        if (station.getOrientation() == Orientation.HORIZONTAL) {
            return point.y >= -side && point.y <= component.getHeight() + side && point.x >= -side &&
                    point.x <= component.getWidth() + side;
        } else {
            return point.x >= -side && point.x <= component.getWidth() + side && point.y >= -side &&
                    point.y <= component.getHeight() + side;
        }
    }
}
