...
@Todo(compatibility = Compatibility.COMPATIBLE, target = Version.VERSION_1_1_2, priority = Priority.MINOR,
        description = "This class is almost identical with 'DialogWindow' from Common, maybe these two classes can be" +
                " merged?")
        ...
        for (GraphicsDevice device : environment.getScreenDevices()) {
            ...
            if (screenBounds.contains(location)) {
                ...
                if (screenBounds.x <= location.x && screenBounds.x + screenBounds.width >= location.x) {
                    ...
                    dx = Math.min(Math.abs(screenBounds.x - location.x), Math.abs(
                            screenBounds.x + screenBounds.width - location.x));
                }

                if (screenBounds.y <= location.y && screenBounds.y + screenBounds.height >= location.y) {
                    dy = 0;
                } else {
                    dy = Math.min(Math.abs(screenBounds.y - location.y), Math.abs(
                            screenBounds.y + screenBounds.height - location.y));
                }
