...
public class DefaultToolbarContainerConverter implements ToolbarContainerConverter {
    ...
    public PlaceholderMap getPlaceholders(ToolbarContainerDockStation station, Map<Dockable, Integer> children) {
        final PlaceholderMap result = new PlaceholderMap(new Path("dock.ToolbarContainerStation"), 0);
        result.put(result.newKey("content"), "list", convert(station.getPlaceholderStrategy(), station.getDockables()
                , children));
        return result;
    }

    @Override
    public PlaceholderMap getPlaceholders(ToolbarContainerDockPerspective station,
    ...
    private PlaceholderMap convert(final PlaceholderStrategy strategy,
                                   DockablePlaceholderList<StationChildHandle> list,
                                   final Map<Dockable, Integer> children) {
        return list.toMap(new PlaceholderListItemAdapter<>() {
            @Override
    ...
    private PlaceholderMap convert(PerspectivePlaceholderList<PerspectiveDockable> list,
                                   final Map<PerspectiveDockable, Integer> children) {
        return list.toMap(new PlaceholderListItemAdapter<>() {
            @Override
    ...
    @Override
    public void setPlaceholders(ToolbarContainerDockStation station, ToolbarContainerConverterCallback callback,
                                PlaceholderMap map, Map<Integer, Dockable> children) {
        convert(station, callback, map.getMap(map.newKey("content"), "list"), children);
    }

    private void convert(final ToolbarContainerDockStation station, final ToolbarContainerConverterCallback callback,
                         PlaceholderMap map, final Map<Integer, Dockable> children) {
        final DockablePlaceholderList<StationChildHandle> list = new DockablePlaceholderList<>();
        callback.setDockables(list);
        list.read(map, new PlaceholderListItemAdapter<>() {
