...
public abstract class AbstractToolbarDockStation extends
    ...
    /**
     * If it is not clear whether an {@link ExpandedState} is enabled, because the involved {@link Dockable}s offer
     * different
     * enabled-states, then the value of this <code>boolean</code> is the result of the operation.
     */
    public static final PropertyKey<Boolean> ON_CONFLICT_ENABLE =
            new PropertyKey<>("ExpandableToolbarGroupActions.on_conflict_enable",
     ...
     */
    private final ExpandableListener expandableListener = new ExpandableListener();

    /**
     * the current strategy to handle {@link ExpandableToolbarItem}s
     */
    private final PropertyValue<ExpandableToolbarItemStrategy> expandableStategy =
            new PropertyValue<>(ExpandableToolbarItemStrategy.STRATEGY) {
                @Override
     ...
     */
    private final PropertyValue<Boolean> onConflictEnable = new PropertyValue<>(ON_CONFLICT_ENABLE) {
        @Override
    ...
    private final boolean[] expandedEnablementStateCache = new boolean[ExpandedState.values().length];
    ...
    protected void fireOrientingEvent() {
        ...
        for (final OrientingDockStationListener listener : orientingListeners
                .toArray(new OrientingDockStationListener[0])) {
     ...
     * @param action if <code>true</code>, then
    ...
    protected ExpandableToolbarItemListener[] expandableListeners() {
        return expandableListeners
                .toArray(new ExpandableToolbarItemListener[0]);
    }
    ...
    public ToolbarStrategy getToolbarStrategy() {
        final SilentPropertyValue<ToolbarStrategy> value = new SilentPropertyValue<>(
     ...
     *
     * @param controller the controller at which the default title factory has to be
     *                   registered
