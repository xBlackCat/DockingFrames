...
public class ToolbarDockStation extends AbstractToolbarDockStation {
     ...
     */
    public static final PropertyKey<Integer> GAP =
            new PropertyKey<>("dock.toolbar.gap", new ConstantPropertyFactory<>(2), true);

    /**
     * Key for setting the size of the gap between the children of a station and the border of the station
     */
    public static final PropertyKey<Integer> SIDE_GAP =
            new PropertyKey<>("dock.toolbar.sidegap", new ConstantPropertyFactory<>(4), true);
     ...
     */
    private final PropertyValue<Integer> gap = new PropertyValue<>(GAP) {
        @Override
        protected void valueChanged(Integer oldValue, Integer newValue) {
            layoutManager.setGap(newValue);
        }
    };
     ...
     */
    private final PropertyValue<Integer> sideGap = new PropertyValue<>(SIDE_GAP) {
        @Override
        protected void valueChanged(Integer oldValue, Integer newValue) {
            layoutManager.setSideGap(newValue);
        }
    };
     ...
     * (Measured in pixel).
     *
     * @param lateralNodropZoneSize the size of the rectangular lateral zones (in pixel)
            ...
            if (dropInfo != null) {
                ...
                if (getOrientation() == Orientation.HORIZONTAL) {
                    if (index == 0) {
                        ...
                        x = dockablePane.getComponent(index - 1).getX() +
                                dockablePane.getComponent(index - 1).getWidth();
                    }
                    ...
                    if (index == 0) {
                        ...
                        y = dockablePane.getComponent(index - 1).getY() +
                                dockablePane.getComponent(index - 1).getHeight();
                    }
                    if (index == dockablePane.getComponentCount()) {
     ...
     * @param target      the item whose position is searched
     * @param index       the location of <code>child</code>
     * @param placeholder the placeholder for <code>target</code> or <code>child</code>
     ...
     *
     * @param property some property created by
    ...
    public void setPlaceholders(PlaceholderMap map, final Map<Integer, Dockable> children) {
        ...
        try {
            ...
            final DockablePlaceholderList<StationChildHandle> next = new DockablePlaceholderList<>();
    ...
    public void setPlaceholders(PlaceholderMap placeholders) {
        ...
        try {
            final DockablePlaceholderList<StationChildHandle> next = new DockablePlaceholderList<>(placeholders);
            if (getController() != null) {
                dockables.setStrategy(null);
                dockables.unbind();
                dockables = next;
    ...
    public void aside(AsideRequest request) {
        ...
        if (location instanceof ToolbarProperty toolbarLocation) {
