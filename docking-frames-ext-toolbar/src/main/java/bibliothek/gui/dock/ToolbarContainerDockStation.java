 ...
 */
public class ToolbarContainerDockStation extends AbstractDockableStation implements OrientingDockStation,
                                                                                    OrientedDockStation {
     ...
     * toolbars
     */
    protected final OverpaintablePanelBase mainPanel;

    /**
     * the background color of {@link #mainPanel}
     */
    private final BackgroundColor mainPanelBackground;
    ...
    /**
     * This {@link LayoutManager} is responsible for updating the boundaries of all {@link Dockable}s and keeping
     * track of {@link Span}s
     */
    ...
    public ToolbarContainerDockStation(Orientation orientation, int maxNumberOfDockables) {
        ...
        displayerFactory =
                new DefaultDisplayerFactoryValue(ThemeManager.DISPLAYER_FACTORY + ".toolbar.container", this);
    ...
    public void aside(AsideRequest request) {
        ...
        if (location instanceof ToolbarContainerProperty toolbarLocation) {
    ...
    public StationDropOperation prepareDrop(StationDropItem item) {
        ...
        if (this.accept(dockable) && dockable.accept(this)) {
            ...
            final ToolbarContainerDropInfo result =
                    new ToolbarContainerDropInfo(dockable, this, dockables, item.getMouseX(), item.getMouseY()) {
                        @Override
                        public void execute() {
                            drop(this);
                        }
     ...
     *
     * @param max the number of dockables accepted
     */
    ...
    protected void fireOrientingEvent() {
        ...
        for (final OrientingDockStationListener listener :
                orientingListeners.toArray(new OrientingDockStationListener[0])) {
    ...
    public boolean drop(Dockable dockable, DockableProperty property) {
        if (property instanceof final ToolbarContainerProperty toolbar) {
    ...
    public ToolbarStrategy getToolbarStrategy() {
        final SilentPropertyValue<ToolbarStrategy> value =
                new SilentPropertyValue<>(ToolbarStrategy.STRATEGY, getController());
     ...
     * @param bind whether the new list should be
     ...
     * @param index    Index where add dockable
     * @return <code>true</code> if dropping was successful
     */
     ...
     *
     * @param displayer the displayer to replace, must actually be shown on this
     *                  station
     */
        ...
        protected void paintOverlay(Graphics g) {
            ...
            if (prepareDropDraw) {
                if (indexBeneathMouse != -1) {
                    ...
                    // the base pane
                    final Rectangle rectComponentBeneathMouse =
                            getDockables().dockables().get(indexBeneathMouse).getDisplayer().getComponent().getBounds();
                    ...
                    switch (getOrientation()) {
                        case VERTICAL -> {
                            ...
                            paint.drawInsertionLine(g, rectComponentBeneathMouse.x, y,
                                    rectComponentBeneathMouse.x + rectComponentBeneathMouse.width, y);
                        }
                        case HORIZONTAL -> {
                            int x;
                            ...
                            paint.drawInsertionLine(g, x, rectComponentBeneathMouse.y, x,
                                    rectComponentBeneathMouse.y + rectComponentBeneathMouse.height);
                        }
                    }
