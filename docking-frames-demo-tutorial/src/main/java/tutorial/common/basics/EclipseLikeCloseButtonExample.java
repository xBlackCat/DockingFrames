...
public class EclipseLikeCloseButtonExample {
    public static void main(String[] args) {
        ...
        control.getController().getActionViewConverter().putClient(FLAT_BUTTON, ViewTarget.TITLE,
                new FlatButtonGenerator());
    ...
    /* We want the buttons for the close action to be flat, for this we first define a new type of action
    "flat_button" */
    private static final ActionType<ButtonDockAction> FLAT_BUTTON = new ActionType<ButtonDockAction>("flat button");

    /* And then we define a factory that creates the view for a "flat_button". We can resuse the BasicMiniButton and
    just
     * change its border a bit.  */
    private static class FlatButtonGenerator implements ViewGenerator<ButtonDockAction,
