 ...
 * Each method should return a new SGlassParameter structure, because these parameters will be modified by a dockable
 * (color).
 * It is allowed that they return <code>null</code>. In this case no glass effect will be rendered for the associated
 * state.
 *
 * @author Thomas Hilbert
 */
public class CDefaultGlassFactory implements IGlassParameterFactory {
    static final CDefaultGlassFactory INSTANCE = new CDefaultGlassFactory();
    ...
    public SGlassParameter getStripBGGlassParameters() {
        return (new SGlassParameter(0.25, 0.0, 0.5, 0.0, 0.0, 0.21, new Color(0, 0, 0), new Color(0, 0, 0),
                new Color(0, 0, 0), 0.0, 1.0, 0.7, 0.2));
    }
