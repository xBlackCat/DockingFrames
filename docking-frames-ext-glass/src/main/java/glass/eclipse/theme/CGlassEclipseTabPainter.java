...
public class CGlassEclipseTabPainter extends BaseTabComponent {
    ...
    protected void initAdditionalColors() {
        colGlassCenterSelected =
                new CGlassColor("glass.selected.center", getStation(), getDockable(), new Color(222, 222, 222));
        colGlassBoundarySelected =
                new CGlassColor("glass.selected.boundary", getStation(), getDockable(), new Color(0, 40, 255));
        colGlassLightSelected =
                new CGlassColor("glass.selected.light", getStation(), getDockable(), new Color(222, 222, 222));

        colGlassCenterUnSelected =
                new CGlassColor("glass.unselected.center", getStation(), getDockable(), new Color(222, 222, 222));
        colGlassBoundaryUnSelected =
                new CGlassColor("glass.unselected.boundary", getStation(), getDockable(), new Color(0, 40, 255));
        colGlassLightUnSelected =
                new CGlassColor("glass.unselected.light", getStation(), getDockable(), new Color(222, 222, 222));

        colGlassCenterFocused =
                new CGlassColor("glass.focused.center", getStation(), getDockable(), new Color(0, 0, 150));
        colGlassBoundaryFocused =
                new CGlassColor("glass.focused.boundary", getStation(), getDockable(), new Color(0, 40, 80));
        colGlassLightFocused =
                new CGlassColor("glass.focused.light", getStation(), getDockable(), new Color(100, 200, 255));

        colGlassCenterDisabled =
                new CGlassColor("glass.disabled.center", getStation(), getDockable(), new Color(150, 150, 150));
        colGlassBoundaryDisabled =
                new CGlassColor("glass.disabled.boundary", getStation(), getDockable(), new Color(0, 40, 80));
        colGlassLightDisabled =
                new CGlassColor("glass.disabled.light", getStation(), getDockable(), new Color(150, 150, 150));

        addAdditionalColors(colGlassBoundaryFocused, colGlassBoundarySelected, colGlassCenterFocused,
                colGlassCenterSelected, colGlassLightFocused, colGlassLightSelected, colGlassLightUnSelected,
                colGlassCenterUnSelected, colGlassBoundaryUnSelected, colGlassCenterDisabled,
                colGlassBoundaryDisabled, colGlassLightDisabled);
    }
    ...
    public Insets getOverlap(TabComponent other) {
        if (other instanceof CGlassEclipseTabPainter painter) {
    ...
    protected void update() {
        ...
        switch (getOrientation()) {
            case TOP_OF_DOCKABLE, BOTTOM_OF_DOCKABLE -> {
                labelInsets = new Insets(iInsets, 5, iInsets, 2);
                buttonInsets = new Insets(1, 0, 1, 5);
            }
            case LEFT_OF_DOCKABLE, RIGHT_OF_DOCKABLE -> {
    ...
    protected boolean isHorizontal() {
        return (getOrientation() == TabPlacement.TOP_OF_DOCKABLE ||
                getOrientation() == TabPlacement.BOTTOM_OF_DOCKABLE);
    }
    ...
    protected void paintUnselected(Graphics g) {
        ...
        if (w > 0 && h > 0) {
            ...
            switch (getOrientation()) {
                case TOP_OF_DOCKABLE -> g2d.drawLine(0, h - 1, w - 1, h - 1);
                case BOTTOM_OF_DOCKABLE -> g2d.drawLine(0, 0, w - 1, 0);
                case LEFT_OF_DOCKABLE -> g2d.drawLine(w - 1, 0, w - 1, h - 1);
                case RIGHT_OF_DOCKABLE -> g2d.drawLine(0, 0, 0, h - 1);
            }
    ...
    protected void paintSelected(Graphics g, boolean bActive) {
        ...
        if (w > 0 && h > 0) {
            ...
            if (getTabIndex() > 0) {
                // paint a little bit of the previous tab, because previous tab should also overlap at the begin of
                // the selected tab
                paintWorkAround(g2d);
            }
    ...
    private void paintWorkAround(Graphics2D g2d) {
        ...
        BufferedImage bimg = null;
        dImg = new Dimension(
                isHorizontal() ? r.width + CORNER_RADIUS : getHeight() + CORNER_RADIUS,
                isHorizontal() ? getHeight() : r.width);
            ...
            if (glassUnSelected != null) {
                ...
                if (!isHorizontal()) {
                    AffineTransform atTrans =
                            AffineTransform.getTranslateInstance(/*r.width*/0, CORNER_RADIUS/*-getHeight()*/);
                    atTrans.concatenate(COutlineHelper.tRot90CCW);
    ...
    protected Shape createUnSelectedTabShape(int w, int h, boolean bFirst, boolean forClip) {
        ...
        switch (getOrientation()) {
            case LEFT_OF_DOCKABLE -> {
                ...
                sTab = COutlineHelper.CreateUnselectedTabShape(CORNER_RADIUS,
                        h + 1, w + 1, bFirst, forClip, bBeforeSelected);
                if (!forClip) {
            ...
            case BOTTOM_OF_DOCKABLE -> {
                ...
                y = 0;
                sTab = COutlineHelper.CreateUnselectedTabShape(CORNER_RADIUS,
                        w + 1, h + 1, bFirst, forClip, bBeforeSelected);
    ...
    protected static class CTabPainter implements TabPainter {
        final CBorderListener placementListener;
         ...
         * @author Thomas Hilbert
         */
        protected static class CBorderListener implements DockPropertyListener<TabPlacement> {
            protected final WeakHashMap<CEclipseBorder, Void> wmap = new WeakHashMap<>();
        ...
        public Border getFullBorder(BorderedComponent owner, DockController controller, Dockable dockable) {
            ...
            CEclipseBorder border = new CEclipseBorder(controller, CORNER_RADIUS, owner,
                    CEclipseBorder.TOP_LEFT | CEclipseBorder.BOTTOM_RIGHT);
